...
bool RecordTask::readCharBase(const Cmd::Record::t_ReadUser_SceneRecord *rev)
{
	...
	if (1 == retcode)
	{
		Xlogger->info("找到合格的角色记录：%u,%s)",ret->charbase.id,ret->charbase.name);
	}
	else
	{
		Xlogger->error("读取档案失败,没有找到记录");
	...
	Xlogger->debug("读取档案服务器数据,压缩数据大小(size = %ld)",ret->dataSize);
...
bool RecordTask::saveCharBase(const Cmd::Record::t_WriteUser_SceneRecord *rev)
{
	...
	if (1 == retcode)
	{
		Xlogger->info("保存档案成功：%u,%u",rev->id,retcode);
	}
	else
	{
		Xlogger->error("保存档案失败：%u,%u",rev->id,retcode);
	}
	*/
...
bool RecordTask::getSelectInfo(DWORD accid, DWORD countryid)
{
	...
	if ((DWORD)-1 == retcode
			|| 0 == retcode)
	{
		//Xlogger->debug("没有找到记录");
		ret.num = 0;
		bzero(ret.info,sizeof(ret.info));
	}
	else
	{
		//Xlogger->debug("找到 %u 条合格的角色记录",retcode);
		ret.num = retcode;
	}
	ret.accid=accid;
	return sendCmd(&ret,sizeof(ret));
...
bool RecordTask::readCharTest(Cmd::Record::t_Read_CharTest_SceneRecord *rev)
{
	...
	if (1 == retcode)
	{
		Xlogger->debug("读取测试统计数据成功(%s)",ret.name);
	}
	else
	{
		Xlogger->debug("未找到统计数据(%s)",ret.name);
		return false;
	}
	return sendCmd(&ret,sizeof(ret));
}
...
bool RecordTask::insertCharTest(Cmd::Record::t_Insert_CharTest_SceneRecord *rev)
{
	...
	if ((DWORD)-1 == retcode)
	{
		Xlogger->error("角色插入测试数据库出错 %s",rev->name);
		return false;
	}
	return true;
}
...
bool RecordTask::updateCharTest(Cmd::Record::t_Update_CharTest_SceneRecord *rev)
{
	...
	if (1 == retcode)
	{
		Xlogger->debug("保存测试数据成功%s,%u",rev->name,retcode);
	}
	else
	{
		Xlogger->error("保存测试数据失败：%s,%u",rev->name,retcode);
	}
	return true;
}
/**
