...
InfoClient::InfoClient(
    const std::string &ip,
    const WORD port) : x_tcp_clientTask(ip,port,true),tempid(++tempidAllocator),netType(NetType_near)
{
}
...
int InfoClient::checkRebound()
{
  ...
  if (retcode > 0)
  {
    ...
    else
    {
      ...
      {
        Xlogger->debug("登陆InfoServer成功，收到区的编号：%u(%u,%u),%s,%u",
            ptCmd->gameZone.id,
            ptCmd->gameZone.game,
            ptCmd->gameZone.zone,
            ptCmd->name,
...
{
#ifdef _MSGPARSE_
  Xlogger->error("?? InfoClient::msgParse(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
#endif 
          ...
          case PARA_CHAT_GMTOOL:
            {
              ...
              if (PARA_BROADCAST_GMTOOL==pNullCmd->para)
              {
                ...
                Xlogger->debug("[GM工具]收到GM公告消息 %s:%s",rev->GM,rev->content);
              }
#endif
