...
void RoleregCache::timeAction(const zTime &ct)
{
	if (actionTimer.elapse(ct) > 10)
	{
		...
		while(!datas.empty())
		{
			...
			Xlogger->debug("处理缓冲中的角色名称指令完成：%u,%s,%u",rd.accid,rd.name,rd.state);
			datas.pop_front();
		}
		mlock.unlock();
		actionTimer = ct;
...
bool RoleregCache::msgParse_loginServer(WORD wdServerID,DWORD accid,char name[MAX_NAMESIZE],WORD state)
{
	...
	if (state & ROLEREG_STATE_TEST)//检测角色名
	{
		...
		Xlogger->debug("accid=%u",  tmpinfo.accid);
		Xlogger->debug("zone=%u",  tmpinfo.zone);
		Xlogger->debug("game=%u",  tmpinfo.game);
		Xlogger->debug("name=%s",  tmpinfo.name);
		if (SetRoleInfo.add(tmpinfo))
		{
			...
			else
			{
				...
				SetRoleInfo.remove(tmpinfo.name);
				state |= ROLEREG_STATE_HAS;
				Xlogger->debug("获取数据库句柄出错");
			}
		}
		else
		{
			state |= ROLEREG_STATE_HAS;
			Xlogger->debug("名字已存在缓存中");
		}
	}
	else if (state & ROLEREG_STATE_WRITE)//回写
	{
		...
		if ((connHandleID)-1 != handle)
		{
				...
				creat_con_define,(BYTE*)&Tmpinfo))
			{
				...
				Xlogger->debug("回写数据库成功");
			}
			else
			{
				state &= ~ROLEREG_STATE_OK;
				Xlogger->debug("回写数据库失败");
			}
			SuperService::dbConnPool->putHandle(handle);
		}
		else
	...
	else if (state & ROLEREG_STATE_CLEAN)//清除
	{
		...
		if ((connHandleID) -1 != handle)
		{
			...
			{
				state &= ~ROLEREG_STATE_OK;
				Xlogger->debug("删除角色%s失败",name);
			}
			else
			{
				state |= ROLEREG_STATE_OK;
				Xlogger->debug("删除角色%s成功",name);
			}
			SuperService::dbConnPool->putHandle(handle);
		}
		else
	...
	if (state & ROLEREG_STATE_WRITE)
	{
		if (state & ROLEREG_STATE_OK)
			Xlogger->error("回写角色成功：%u,%s",accid,name);
		else
			Xlogger->error("回写角色失败：%u,%s",accid,name);
	}
	if (state & ROLEREG_STATE_CLEAN)
	{
		if (state & ROLEREG_STATE_OK)
			Xlogger->error("删除角色成功：%u,%s",accid,name);
		else
			Xlogger->error("删除角色失败：%u,%s",accid,name);
	}
