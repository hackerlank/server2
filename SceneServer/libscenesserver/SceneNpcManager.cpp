...
SceneNpcManager::SceneNpcManager()
{
  if (!loadNpcCommonChatTable())
    Xlogger->error("读取npc说话内容失败");
#ifdef _DEBUG  
  Xlogger->debug("SceneNpcManager::SceneNpcManager");
#endif    
}
...
#ifdef _DEBUG
    //Xlogger->debug("addSceneNpc(): 增加特殊npc %s",sceneNpc->name);
#endif
    rwlock.unlock();
    return true;
  }
...
#ifdef _DEBUG
  //Xlogger->debug("removeSpecialNpc(): 删除特殊npc %s(%u)",sceneNpc->name,sceneNpc->tempid);
#endif
  rwlock.unlock();
}
...
{
#ifdef _DEBUG  
  Xlogger->debug("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!SceneNpcManger::removeNpcInOneScene");
#endif    
...
{
  zXMLParser xml;
  if (!xml.initFile(Seal::global["confdir"] + "NpcCommonChat.xml"))
  {
    Xlogger->error("不能读取NpcCommonChat.xml");
    return false;
  }
  ...
  while (chatNode)
  {
    ...
    if (0!=type)
    {
      ...
      while (contentNode)
      {
        ...
        //Xlogger->debug("loadNpcCommonChatTable:%s",std::string(str).c_str());
        itemCount++;
  ...
  Xlogger->info("加载npc随机聊天文件成功,共%d类%d条",NpcCommonChatTable.size(),itemCount);
  return true;
}
