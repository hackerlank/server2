...
bool SceneManager::init()
{
	...
	// 初始化所有地图
	zXMLParser parser;
	if (parser.initFile(Seal::global["confdir"] + "scenesinfo.xml"))
	{
		...
		if (countryNode)
		{
			...
			while(subnode)
			{
				if (strcmp((char*)subnode->name,"country") == 0)
				{
					...
					Xlogger->info("加载国家名称(%u,%s,%u,%u)",info.id,info.name,info.mapid,info.function);
		...
		while (serverNode) {
			...
			if (ScenesService::getInstance().getServerID() == id) {
				...
				while(countryNode)
				{
					...
					while(mapNode)
					{
						...
						if (loaded)
						{
							...
							Xlogger->info("加载%s(%d,%d)成功",loaded->name,loaded->id,loaded->tempid);
							t_regScene_SceneSession regscene;
							regscene.dwID=loaded->id;
							//Xlogger->info("[地图真实ID]:%d",loaded->id&0x0FFF);
				...
				Xlogger->info("ScenesServer id=%d加载%d张地图.",id,mapCount);
				...
				Xlogger->info("skip id=%d != %d.",id,ScenesService::getInstance().getServerID());
			}
		...
		Xlogger->warn("SceneManager 解析配置文件失败.");
	//刷新玉如意需要的地图信息
...
{
  Xlogger->info("SceneManager::loadScene type=%d,countryid=%d,mapid=%d",type,countryid,mapid);
  zEntry *s=NULL;
  switch(type)
  {
	  case 0://Scene::STATIC:
  ...
  if (ret)
  {
    ret=addEntry(s);
    if (!ret)
      Xlogger->error("SceneManager::loadScene addEntry[%s]失败.",s->name);
    else Xlogger->info("SceneManager::loadScene[%s]成功",s->name);
  }
  else
    Xlogger->error("SceneManager::loadScene init[%s]失败.",s->name);
  rwlock.unlock();
  if (!ret)
  {
    SAFE_DELETE(s);
...
Scene * SceneManager::loadBattleScene(DWORD baseid)
{
	...
	if (ret)
	{
		...
		if (!ret)
			...
			Xlogger->info("SceneManager::loadScene[%s]成功",s->name);
	}
	else
		Xlogger->error("SceneManager::loadScene init[%s]失败.",s->name);
	rwlock.unlock();
...
void SceneManager::unloadScene(std::string &name)
{
  ...
  SAFE_DELETE(ret);
  rwlock.unlock();
  Xlogger->debug("SceneManager::unloadScene");
}
...
void SceneManager::unloadAllScene()
{
  ...
  rwlock.unlock();
  Xlogger->debug("SceneManager::unloadAllScene");
}
...
void SceneManager::checkUnloadOneScene()
{
  for(zEntryTempID::hashmap::iterator it=zEntryTempID::ets.begin();it!=zEntryTempID::ets.end();it++)
  {
    ...
    {
      Xlogger->debug("卸载场景%s",scene->name);
...
void SceneManager::freshEverySceneField()
{
  struct getAllMapExec :public SceneCallBack
  {
    ...
    bool exec(Scene *scene)
    {
      //Xlogger->debug("%d,%d,%d,%s",scene->getCountryID(),_old_scene->getCountryID(),scene->isMainCity(),scene->getFileName());
