...
void PrivateStore::show(SceneUser* target)
{
  ...
  using namespace Cmd;
  std::map<DWORD,SellInfo>::iterator it = _items.begin();
  char buffer[x_socket::MAX_USERDATASIZE];
  stAddObjectSellListUserCmd *cmd = (stAddObjectSellListUserCmd *)buffer;
  constructInPlace(cmd);
  for ( ; it!=_items.end(); ++it) {
    if (sizeof(stAddObjectSellListUserCmd) + (cmd->num + 1) * sizeof(cmd->list[0])>= x_socket::MAX_USERDATASIZE)
    {
...
void TradeOrder::reset()
{
  ...
  if (pUser && (pUser == _target) && _target->tradeorder.target() == _me)
  {
    ...
    Xlogger->info("[交易:玩家<------>玩家]%s取消与%s的交易",_me->name,_target->name);
  }
  else
  {
    if (_targetid)
    {
      Xlogger->debug("[交易:玩家<------>玩家]%s取消与%d的交易,但这个人已经不在了",_me->name,_targetid);
    }
  }
...
*/
    if (_target->packs.addObject(it->second,true,AUTO_PACK)) {
      ...
      Xlogger->info("[交易:玩家<------>玩家]用户%s交易%s给%s成功",_me->name,it->second->data.strName,_target->name);
      ...
      Xlogger->info("[交易:玩家<------>玩家]用户%s交易%s给%s失败",_me->name,it->second->data.strName,_target->name);
    }
  }
...
*/
  if (_money) {
    ...
    Xlogger->info("[交易:玩家<------>玩家]用户%s交易银子%d给%s",_me->name,_money,_target->name);
  }
}
...
{
  if (_me->name[0] && _targetid) Xlogger->info("[交易:玩家<------>玩家]%s的交易状态完成",_me->name);
  _targetid=0; 
  _target = NULL;
  _money = 0;
  commited = false;
...
{
  _money = money;
  Xlogger->info("[交易:玩家<------>玩家]用户%s对%s更改交易银子数量(%d)",_me->name,_target->name,money);
}

void TradeOrder::add(zObject* ob)
{
  _items[ob->data.qwThisID] = ob;
  Xlogger->info("[交易:玩家<------>玩家]用户%s对%s添加交易物品%s(%d)",_me->name,_target->name,ob->data.strName,ob->data.qwThisID);  
}
...
void TradeOrder::remove(DWORD id)
{
  ...
  if (it != _items.end()) {
    Xlogger->info("[交易:玩家<------>玩家]用户%s对%s删除交易物品%s(%d)",_me->name,_target->name,it->second->data.strName,it->second->data.qwThisID);  
    _items.erase(it);
  }
}
...
float get_repair_dur_rate(zObject* ob)
{
  ...
  //Xlogger->debug("get_repair_dur_rate=%f",temp);
  return temp;
...
float get_sell_price(zObject* ob)
{
  ...
  //Xlogger->debug("get_sell_price=%f",money);
  return money;
}
...
bool SceneUser::do_trade_rs_cmd(const Cmd::stTradeUserCmd *rev,DWORD cmdLen)
{
  ...
  if (!target || target->privatestore.step() != PrivateStore::BEGIN) {
    Xlogger->warn("%s(%ld)请求购买摆摊物品的用户不存在或者没有摆摊",name,id);
    return true;
  }
  ...
  if (!sf  || !sf->object() ) {
    Xlogger->debug("%s(%ld)请求购买摆摊物品不存在",name,id);
    return true;
  }
    ...
    case GOLD_GIVE_USERCMD_PARAMETER:
      {
        ...
        Xlogger->debug("%d是收到的类型",rett->type);
        if (rett->type == STORN)
        {
    ...
    case REQUEST_TRADE_USERCMD_PARAMETER:
      {
        ...
        if (tradeorder.canRequest())
        {
          ...
          {
            Xlogger->warn("[交易:玩家<------>玩家]%s(%ld)向自己请求交易",name,id);
            return true;
          }
          ...
          if (pAnswer)
          {
            ...
            //mask.on_trade();
            Xlogger->info("[交易:玩家<------>玩家]%s(%ld)请求%s(%ld)交易",name,id,pAnswer->name,pAnswer->id);
          ...
          Xlogger->warn("[交易:玩家<------>玩家]%s(%ld)不能请求交易时请求交易",name,id);
        return true;
      }
      break;
    case ANSWER_TRADE_USERCMD_PARAMETER:
      {
        ...
        if (tradeorder.canAnswer())
        {
          ...
          Xlogger->info("[交易:玩家<------>玩家]%s(%ld)应答%s(%ld)的交易",name,id,pAsker->name,pAsker->id);
          if (pAsker->tradeorder.canAnswer() && pAsker->tradeorder.target()==this)
          {
            ...
            Xlogger->warn("[交易:玩家<------>玩家]%s(%ld)交易应答另一个用户",name,id);
        }
        else
          Xlogger->warn("[交易:玩家<------>玩家]%s(%ld)不能应答交易时应答交易",name,id);
        return true;
      }
      break;
    case COMMIT_TRADE_USERCMD_PARAMETER:
      {
        if (tradeorder.commit())
        {
          ...
          Xlogger->warn("[交易:玩家<------>玩家]%s(%ld)不能确定交易时确定交易",name,id);
        return true;
      }
      break;
    ...
    case ADD_OBJECT_TRADE_USERCMD_PARAMETER:
      {
        ...
        if (cmd->x > TradeOrder::WIDTH || cmd->y > TradeOrder::HEIGHT) {
          Xlogger->warn("[交易:玩家<------>玩家]%s(%ld)请求交易的物品坐标非法",name,id);  
          return true;  
        }

        zObject* ob = packs.uom.getObjectByThisID(cmd->object.qwThisID);
        if (!ob) {
          Xlogger->warn("[交易:玩家<------>玩家]%s(%ld)请求交易的物品不存在",name,id);  
          return true;
        }

        if (ob->base->kind == ItemType_Money && cmd->object.dwNum > ob->data.dwNum  ){
          Xlogger->warn("[交易:玩家<------>玩家]%s(%ld)请求交易的物品数量非法",name,id);  
          return true;
        }

        if (ob->data.bind || ob->data.dwObjectID == 800 || ob->base->kind == ItemType_Quest) {
          Xlogger->warn("[交易:玩家<------>玩家]%s(%ld)用户试图交易不能交易的物品",name,id);            
          return true;
        }

        if (ob->data.pos.loc() != Cmd::OBJECTCELLTYPE_COMMON) {
          Xlogger->warn("[交易:玩家<------>玩家]%s(%ld)用户试图交易不在主包裹中的物品",name,id);
          return true;
        }
    ...
    case VISITNPC_TRADE_USERCMD_PARAMETER:
      {
        ...
        if ( (sceneNpc && this->canVisitNpc(sceneNpc)) /*|| ( sceneNpc->id == 5281 )*/ )/*( (sceneNpc->id>=5000&&sceneNpc->id<=6000)  || (sceneNpc->scene && (sceneNpc->scene->getCountryID() == charbase.country || changeface)) ) */
        {
		  ...
		  //Xlogger->debug("TODO 检查Npc(%lu)是否在同一个场景,并且检查距离",sceneNpc->id);
		  ...
		  {
			  Xlogger->warn("用户%s(%d)检查Npc(%lu)距离不合法(%u,%d,%d),(%u,%d,%d)",this->name,this->id,sceneNpc->id,this->scene->id,this->getPos().x,this->getPos().y,sceneNpc->scene->id,sceneNpc->getPos().x,sceneNpc->getPos().y);
		  }
		  //          Xlogger->debug("动态菜单(%s)",cmd->menuTxt);
		}
		else
			Xlogger->error("%s(%d)访问不能访问的Npc",this->name,this->id);
    ...
    case BUYOBJECT_NPCTRADE_USERCMD_PARAMETER:
      {
        ...
        {
          Xlogger->debug("[交易:玩家<------商店]%s 交易时,找不到该npc tempID=%u",name,npc_dwNpcTempID);
          return true;
        }
              ...
              if (NpcTrade::getInstance().verifyNpcAction(npc_dwNpcDataID,item))
              {
                if (base->kind == ItemType_HORSE) {
                  ...
                  if (packs.removeMoney(need,"买东西")) {
                    ...
                    Xlogger->info("[交易:玩家<------商店]用户(%s,%u)买马(%d)花费银子%d",name,id,base->id,need);
                  }else {
                    Xlogger->fatal("[交易:玩家<------商店]用户(%s)买马时银子计算错误!",name);
                  }
                  return true;
                }
                ...
                /*if (base->kind == ItemType_Store) 
				{
                  ...
                  if (packs.removeMoney(need,"买东西") ) {
                    Xlogger->info("[交易:玩家<------商店]用户(%s,%u)买储物箱花费银子%d",name,id,need);
				  ...
				  {
                    Xlogger->fatal("[交易:玩家<------商店]用户(%s)买储物箱时银子计算错误!",name);
                  }
                  return true;
                }*/
                ...
                if (o)
                {
                  ...
                  if (callback.num() || free) {
                    ...
                    if (base->kind == ItemType_DoubleExp)
                    {
                      ...
                      {
                        Xlogger->fatal("[交易:玩家<------商店]用户(%s)买%s时荣誉点数计算错误!",name,o->base->name);
                      ...
                      Xlogger->info("[交易:玩家<------商店]用户(%s,%u)买%s(%d)个花费荣誉点%d",name,id,o->name,count,price*count);
                    }
                    else 
                    {
						DWORD taxMoney;
                      ...
                      if (!packs.removeMoney(price*count+taxMoney,"买东西")) {
                        Xlogger->fatal("[交易:玩家<------商店]用户(%s)买%s时银子计算错误!",name,o->base->name);
                      }
                      Cmd::Session::t_taxAddCountry_SceneSession send;
					  if( npc_dwNpcTempID == 100000000 )
					  {
					  ...
					  if( npc_dwNpcTempID == 100000000 )
					  {
						  Xlogger->info("[交易:玩家<------随身商店]用户(%s,%u)买%s(%d)个花费银子%d",name,id,o->name,count,price*count+taxMoney);
					  }
					  else
						  zObject::logger(o->createid,o->data.qwThisID,o->data.strName,o->data.dwNum,count,1,n->id,n->name,this->id,this->name,"buy_npc",o->base,o->data.kind,o->data.upgrade);
                      Channel::sendMoney(this,Cmd::INFO_TYPE_GAME,price*count+taxMoney,"得到物品 %s(%d)个,花费银子",o->name,count);
                      Xlogger->info("[交易:玩家<------商店]用户(%s,%u)买%s(%d)个花费银子%d",name,id,o->name,count,price*count+taxMoney);
                    }
                  } 
                ...
                Xlogger->error("[交易:玩家<------商店]不能在这里买这样物品 %u,%u,%s",npc_dwNpcDataID,base->id,base->name);
            }
            else
            {
              if (base->kind == ItemType_DoubleExp)
          ...
          if (this->getGoodnessState() != Cmd::GOODNESS_6)
          {
            ...
            if (NpcTrade::getInstance().verifyNpcAction(npc_dwNpcDataID,item))
            {
              ...
              if (srcobj->data.price>0)
              {
                ...
                Xlogger->info("[交易:玩家------>商店]用户(%s,%u)卖%s得到银子%d",name,id,srcobj->name,real_price);
                packs.addMoney(real_price,"卖东西");
              }

              if (srcobj->data.exp && srcobj->base->kind != ItemType_Pack ) {
                Xlogger->info("[交易:玩家------>商店]%s(%u) 卖装备增加经验 %u",name,id,srcobj->data.exp);
            ...
            {
              Xlogger->warn("[交易:玩家------>商店]不能在这里卖这样物品 %u,%u,%s",
    ...
    case SELLHORSE_NPCTRADE_USERCMD_PARAMETER:
      {
        ...
        if (this->getGoodnessState() != Cmd::GOODNESS_6)
        {
          ...
          if (!base) {
            Xlogger->warn("[卖马]用户马匹在道具表中不存在,请检查道具基本表!");
            return true;
          }
          NpcTrade::NpcItem item;
          item.id = base->id;
          ...
          if (NpcTrade::getInstance().verifyNpcAction(npc_dwNpcDataID,item)) {
            if (base->price>0)  {
              ...
              Xlogger->info("[卖马]用户(%s,%u)卖%s得到银子%d",name,id,base->name,price);
    ...
    case REPAIROBJECT_GOLD_NPCTRADE_USERCMD_PARAMETER:
      {
        ...
        {
          Xlogger->fatal("用户(%s)修理装备时金子子计算错误!",name);
        }
      }
      break;
    case REPAIROBJECT_NPCTRADE_USERCMD_PARAMETER:
      {
        ...
        {
          Xlogger->fatal("用户(%s)修理装备时银子计算错误!",name);
        }
        /*
        else {
          Xlogger->info("[修理装备]用户(%s)修理装备花费银子(%d)",name,price);
        }
        // */
      }
      break;
...
*/
		...
		sendCmdToMe(&cmd,sizeof(cmd));
        Xlogger->info("[交易:玩家------>摆摊]用户(%s)请求开始摆摊",name);
        return true;
      }
      if (privatestore.step() == PrivateStore::START) {
        ...
        sendMeToNine();
        Xlogger->info("[交易:玩家------>摆摊]用户(%s)开始摆摊",name);
        return true;
      }
//#endif
      break;
    case FINISH_SELL_USERCMD_PARAMETER:
      {
        ...
        Xlogger->info("[交易:玩家------>摆摊]用户(%s)完成摆摊",name);
        sendMeToNine();
      }
      break;
    case REQUEST_ADD_OBJECT_SELL_USERCMD_PARAMETER:
      {
        ...
        for(int i = 0; i < cmd->num && i < 200; i++)
        {
          ...
          if (cmd->list[i].x > PrivateStore::WIDTH || cmd->list[i].y > PrivateStore::HEIGHT) {
            Xlogger->warn("[交易:玩家------>摆摊]%s(%ld)请求摆摊的物品坐标非法",name,id);  
            return true;  
          }

          zObject* ob = packs.uom.getObjectByThisID(cmd->list[i].qwThisID);
          if (!ob) {
            Xlogger->warn("[交易:玩家------>摆摊]%s(%ld)请求摆摊的物品不存在",name,id);  
            return true;
          }
          ...
          if (ob->data.bind || ob->data.dwObjectID == 800 || ob->base->kind == ItemType_Quest) {
            Xlogger->warn("[交易:玩家------>摆摊]%s(%ld)用户试图使用不能摆摊的物品",name,id);            
            return true;
          }


          if (ob->data.pos.loc() != Cmd::OBJECTCELLTYPE_COMMON && ob->data.pos.loc()!=Cmd::OBJECTCELLTYPE_PACKAGE) {
            Xlogger->warn("[交易:玩家------>摆摊]%s(%ld)用户试图摆摊不在主副包裹中的物品",name,id);
            return true;
          }

          privatestore.add(ob,cmd->list[i].price,cmd->list[i].x,cmd->list[i].y);
          Xlogger->info("[交易:玩家------>摆摊]用户%s添加摆摊物品%s(%d)",name,ob->data.strName,ob->data.qwThisID);
        }
      }
      break;
  ...
  /*
        if (ob->base->kind == ItemType_Money && cmd->object.dwNum > ob->data.dwNum  ){
          Xlogger->warn("%s(%ld)请求交易的物品数量非法",name,id);  
          return true;
        }
  */
        if (mask.is_use(ob)) {
    ...
    case REMOVE_OBJECT_SELL_USERCMD_PARAMETER:
      {
        ...
        if (ob) {
          Xlogger->info("[交易:玩家------>摆摊]用户%s移除摆摊物品%s(%d)",name,ob->data.strName,ob->data.qwThisID);
        }
    ...
    case REQUEST_SELL_INFO_USERCMD_PARAMETER:
      {
        ...
        if (cmd->temp_id == tempid) {
          //Xlogger->warn("[交易:玩家<------摆摊]%s(%ld)请求自己的摆摊信息",name,id);
          return true;
        }
        
        SceneUser *target = scene->getUserByTempID(cmd->temp_id);
        if (!target || target->privatestore.step() != PrivateStore::BEGIN) {
          Xlogger->warn("[交易:玩家<------摆摊]%s(%ld)请求摆摊信息的用户不存在或者没有摆摊",name,id);
          return true;
        }
        ...
        //Xlogger->info("[交易:玩家<------摆摊]%s(%ld)请求%s(%ld)摆摊信息",name,id,target->name,target->id);
