...
bool TeamManager::loadTeam(SceneUser *leader,TempArchiveMember *data)
{
  ...
  //    if (!u->team->IsTeamed())
  //    {
  //      //Xlogger->debug("队伍%u恢复成员找到成员,id=%u,tempid=%u,name=%s",leader->tempid,u->id,u->tempid,u->name);
  ...
  //  }
  //  //Xlogger->debug("恢复队伍成员,id=%u,name=%s",pair->id,pair->name);
  ...
  //}
  ////Xlogger->debug("load队伍成员数量=%u",leader->team->getSize());
  return true;
}
...
struct ExpSizeExec : public TeamMemExec
{
  ...
  bool exec(TeamMember &member)
  {
    ...
    if (pUser)
    {
      ...
      {
        //Xlogger->debug("检查9屏成员坐标成功(%d,%d)(%d,%d)",pos,pUser->getPosI(),pUser->getPos().x,pUser->getPos().y);
        size++;
      }
      else
      {
        //Xlogger->debug("检查9屏成员坐标失败(%d,%d)(%d,%d)",pos,pUser->getPosI(),pUser->getPos().x,pUser->getPos().y);
      }
    }
    return true;
  }
