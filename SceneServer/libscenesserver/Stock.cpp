    ...
    case TRANSFER_FUND_SAVE_STOCKPARA:
      {
        ...
        {
          Xlogger->debug("%s(%d)股票冲值时发现金额不够,可能是外挂",this->name,this->id);
        }
        else
        {
...
bool SceneUser::stockSave(DWORD mon,DWORD go)
{
  ...
  if (bret)
  {
    ...
    Xlogger->debug("%s(%d)股票冲值成功,金币:%d,银币:%d",this->name,this->id,go,mon);
  }
  else
  {
    Xlogger->debug("%s(%d)股票冲值失败,金币:%d,银币:%d",this->name,this->id,go,mon);
  }
  return bret;
}
