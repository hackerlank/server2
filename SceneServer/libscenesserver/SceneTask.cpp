...
SceneTask::~SceneTask()
{
	Xlogger->info("网关(%u)关闭",wdServerID);
}
...
int SceneTask::recycleConn()
{
	...
	Xlogger->debug("连接%s使用内存:%d",getIP(),mSocket->getBufferSize());
	return 1;
}
...
bool SceneTask::usermsgParseBill(SceneUser *pUser,const Cmd::t_NullCmd *pNullCmd,const DWORD cmdLen)
{
			...
			case PARA_STOCK_FETCH:
				{
					...
					Xlogger->debug("%s(%d)股票提现,金币:%d,银币:%d",pUser->name,pUser->id,ret->dwGold,ret->dwMoney);
				}
				break;
			}
			return true;
	...
	Xlogger->error("SceneTask::usermsgParseBill(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,cmdLen);
	return false;
}
/**
* \brief 用户命令分派
...
bool SceneTask::usermsgParse(SceneUser *pUser,const Cmd::t_NullCmd *pNullCmd,const DWORD cmdLen)
{
	...
	if (pUser->getState() == SceneUser::SceneEntry_Death)
	{
		...
		default:
			{
				//Xlogger->debug("(%s,%ld)死亡状态过滤指令 (%ld,%ld)",pUser->name,pUser->id,pNullCmd->cmd,pNullCmd->para);
				return true;
			}
			break;
		}
		...
		case MAPSCREEN_USERCMD:
			{
				...
				default:
					{
						Xlogger->debug("(%s,%ld)交易状态过滤指令 (%ld,%ld)",pUser->name,pUser->id,pNullCmd->cmd,pNullCmd->para);
						return true;
					}
					break;
				}
		...
		case MAPSCREEN_USERCMD:
			{
				...
				default:
					{
						Xlogger->debug("(%s,%ld)摆摊状态过滤指令 (%ld,%ld)",pUser->name,pUser->id,pNullCmd->cmd,pNullCmd->para);
						return true;
					}
					break;
				}
	...
	//Xlogger->debug("用户指令(cmd=%u,para=%u,rev->size=%d",pNullCmd->cmd,pNullCmd->para,cmdLen);
	switch(pNullCmd->cmd)
	{
	case DATA_USERCMD:
		{
			...
			case LOADMAPOK_DATA_USERCMD_PARA:
				{
					//pUser->loadMapOK();
					//Xlogger->debug("%s(%d)加载地图完成",pUser->name,pUser->id);
				}
				break;
			}
			return true;
	...
	case MOVE_USERCMD:
		{
			...
			case USERMOVE_MOVE_USERCMD_PARA:
				{
					Xlogger->debug("%s 收到移动消息",pUser->name);
					...
					Xlogger->debug("%s 移动处理完毕",pUser->name);
					return ret;
				}
				break;
...
#ifdef _DEBUG
					Xlogger->error("!!!----收到攻击消息,攻击消息的长度为[%u]",cmdLen);
#endif
			...
			case REDEEM_MONTH_CARD_PARA:
				{
					...
					/*
					Xlogger->debug("%s(%u)兑换月卡",pUser->name,pUser->id);
			...
			case RETURN_CARD_AND_GOLD_PARA:
				{
					...
					Xlogger->debug("%s(%u)查询金币月卡,金币%d,月卡%u",pUser->name,pUser->id,rev->dwGold,rev->dwMonthCard);
					// */
					return true;
				}
				break;
			...
			case REQUEST_CARD_AND_GOLD_PARA:
				{
					...
					Xlogger->debug("%s(%u)请求查询金币和月卡",pUser->name,pUser->id);
					return true;
					// */
				}
				break;
			case REQUEST_POINT_PARA:
				{
					...
					pUser->gatetask->sendCmd(&rb,sizeof(rb));
					//Xlogger->debug("%s(%u)请求查询剩余点卡",pUser->name,pUser->id);
					return true;
				}
				break;
			case RETURN_REQUEST_POINT_PARA:
				{
					...
					//Xlogger->debug("%s(%u)请求查询点卡",pUser->name,pUser->id);
					return true;
				}
				break;
			default:
	...
	case GMTOOL_USERCMD:
		{
			if (pNullCmd->para==Cmd::MSG_GMTOOL_PARA)
			{
				...
				Xlogger->debug("[GM工具]收到GM留言消息 %s",pUser->name);
				if (rev->type==6)
				{
	...
	Xlogger->error("SceneTask::usermsgParse(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,cmdLen);
	return false;
}
	...
	if (pNullCmd->para==Cmd::Scene::PARA_LOGIN_UNREG)
	{
		...
		//if (scene)
		{
			...
			if (pUser)
			{
				...
				pUser->unreg();
				Xlogger->debug("用户%ld注销时还没有进入场景",rev->dwUserID);
			}
			else
			{
				...
				if (pUser)
				{
					...
					//pUser->killAllPets();
					pUser->unreg();
					Xlogger->debug("请求注销(%s,%u)指令:t_Unreg_LoginScene",pUser->name,pUser->id);
				}
				else
					Xlogger->error("注销时,未找到临时编号为%ld的用户",rev->dwUserID);
			}
		}
		return true;
	}
...
bool SceneTask::cmdMsgParse(const Cmd::t_NullCmd *cmd,const DWORD cmdLen)
{
	...
	if (cmd->cmd==Cmd::Scene::CMD_FORWARD && cmd->para==Cmd::Scene::PARA_FORWARD_SCENE)
	{
		...
		//if (scene)
		{
			...
			{
				Xlogger->error("处理用户指令(%d,%d)时,未找到临时编号为%ld的用户",cmd->cmd,cmd->para,rev->dwUserID);
				//        SceneUserManager::getMe().unlock();
				return true;
			}
		}
	...
	Xlogger->error("SceneTask::cmdMsgParse(%u,%u,%u)",cmd->cmd,cmd->para,cmdLen);
	return false;
}
