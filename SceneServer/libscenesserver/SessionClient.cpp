...
bool SessionClient::cmdMsgParse_Gem(const Cmd::t_NullCmd *pNullCmd,const DWORD nCmdLen)
{
	...
	case Cmd::Session::SUMMON_GEMNPC_SCENE_PARA:
		{
			...
			if (!scene)
			{
				Xlogger->debug("SUMMON_GEMNPC: 召唤npc时找不到地图 mapid=%u",rev->dwMapID);
				return false;
			}
			zNpcB *base = npcbm.get(rev->dwBossID);
			if (NULL == base)
			{
				Xlogger->debug("SUMMON_GEMNPC: 召唤npc时找不到NPC id=%d",rev->dwBossID);
				return false;
			}
			...
			if (!boss)
			{
				Xlogger->debug("SUMMON_GEMNPC: 召唤NPC失败 id=%d",base->id);
				return false;
			}
	...
	case Cmd::Session::CLEAR_GEMNPC_SCENE_PARA:
		{
			...
			if (!scene)
			{
				Xlogger->debug("CLEAR_GEMNPC: 清除npc时找不到地图 mapid=%u",rev->dwMapID);
				return false;
			}
	...
	case Cmd::Session::BLAST_GEMNPC_SCENE_PARA:
		{
			...
			if (!pUser)
			{
				Xlogger->debug("BLAST_GEMNPC: npc爆物品时找不到玩家 userID=%u",rev->dwUserID);
				return false;
			}
			...
			if (NULL == base)
			{
				Xlogger->debug("BLAST_GEMNPC: 召唤npc时找不到NPC id=%d",rev->dwBossID);
				return false;
			}
	...
	Xlogger->error("SessionClient::cmdMsgParse_Gem(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
	return false;
}
...
bool SessionClient::cmdMsgParse_Army(const Cmd::t_NullCmd *pNullCmd,const DWORD nCmdLen)
{
	...
	Xlogger->error("SessionClient::cmdMsgParse_Army(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
	return false;
}
...
bool SessionClient::cmdMsgParse_Recommend(const Cmd::t_NullCmd* pNullCmd,const DWORD nCmdLen)
{
	...
	Xlogger->error("SessionClient::cmdMsgParse_Recommend(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
	return false;
...
bool SessionClient::cmdMsgParse_Dare(const Cmd::t_NullCmd *pNullCmd,const DWORD nCmdLen)
{
...
#ifdef _DEBUG        
			Xlogger->debug("收到设置皇城争夺战命令: state:%u defcountry:%u",
...
#ifdef _DEBUG        
			Xlogger->debug("收到设置反攻命令: from:%d to:%d",
	...
	case Cmd::Session::PARA_ENTERWAR:
		{
...
#ifdef _DEBUG        
			Xlogger->debug("收到激活对战命令:type:%d fromrelation:%d torelation:%d state:%d isatt:%d isAntiAtt:%d",              rev->dwWarType,rev->dwFromRelationID,rev->dwToRelationID,
			...
			if (rev->dwStatus == 1)
			{
				...
				else
				{
					if (pUser == NULL)
					{
						Xlogger->error("无效的用户ID(%d),取消激活对战命令",
							rev->dwUserID);
						return true;
					}
			...
			if (pUser)
			{
				Xlogger->debug("%s 已有 %d 条交战记录",pUser->name,pUser->warSize());
			}
#endif        
			if (pUser != NULL)
			{
	...
	case Cmd::Session::PARA_DARE_GOLD:
		{
...
#ifdef _DEBUG        
			Xlogger->debug("%ld 银子操作 : %d",rev->dwWarID,rev->dwNum);
#endif        
			...
			if (rev->dwNum>=0)
			{
				...
				else if (rev->dwType == Cmd::Session::WINNER_GOLD)
				{
					if (rev->dwWarType == Cmd::UNION_DARE 
						|| rev->dwWarType == Cmd::UNION_CITY_DARE)
					{
						Xlogger->info("[夺城战]: %s(%d) 对战奖励 %d 文",
							pUser->name,pUser->charbase.unionid,rev->dwNum);
					}
					else if (rev->dwWarType == Cmd::SEPT_DARE)
					{
						Xlogger->info("[家族日志]: %s(%d) 对战奖励 %d 文",
							pUser->name,pUser->charbase.septid,rev->dwNum);
					}
				}
			...
			else if (rev->dwNum<0)
			{
				...
				{
					//Xlogger->info("对战银子跟踪：%s([%s]帮 [%s]家族) 扣除 %d 文",
					...
					if (rev->dwType == Cmd::Session::DARE_GOLD)
					{
						if (rev->dwWarType == Cmd::UNION_DARE 
							|| rev->dwWarType == Cmd::UNION_CITY_DARE)
						{
							Xlogger->info("[夺城战]: %s(%d) 扣除对战金 %d 文",
								pUser->name,pUser->charbase.unionid,rev->dwNum);
						}
						else if (rev->dwWarType == Cmd::SEPT_DARE)
						{
							Xlogger->info("[家族日志]: %s(%d) 扣除对战金 %d 文",
								pUser->name,pUser->charbase.septid,rev->dwNum);
						}
					}
	...
	Xlogger->error("SessionClient::cmdMsgParse_Dare(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
	return false;
}
...
bool SessionClient::cmdMsgParse_Country(const Cmd::t_NullCmd *pNullCmd,const DWORD nCmdLen)
{
	...
	case Cmd::Session::PARA_REFRESH_GEN:
		{
			...
			if (s)
			{
				...
				if (0==base)
				{
					Xlogger->error("PARA_REFRESH_GEN: 召唤npc时找不到NPC id=%d",COUNTRY_MAIN_GEN+rev->level);
					return true;
				}
				...
				if (!boss)
				{
					Xlogger->debug("PARA_REFRESH_GEN: 召唤NPC失败 id=%d",base->id);
					return true;
				}
				...
				Xlogger->info("刷新大将军 map=%s level=%u pos=(%u,%u)",s->name,rev->level,boss->getPos().x,boss->getPos().y);
			}
			else
				Xlogger->error("刷新大将军时没找到地图 mapID=%u",(rev->dwCountryID<<16)+139);
	...
	case Cmd::Session::PARA_SUMMON_ALLY_NPC:
		{
			...
			if (!scene)
			{
				Xlogger->debug("PARA_SUMMON_ALLY_NPC: 召唤npc时找不到地图 mapName=%s",mapName);
				return false;
			}
			zNpcB *base = npcbm.get(ALLY_GUARDNPC);
			if (NULL == base)
			{
				Xlogger->debug("PARA_SUMMON_ALLY_NPC: 召唤npc时找不到NPC id=%d",ALLY_GUARDNPC);
				return false;
			}
	...
	case Cmd::Session::PARA_UPDATE_TECH:
		{
			...
			{
				Xlogger->info("[国家]: %d 未找到对应国家科技信息。",rev->dwCountryID);
			}
	...
	Xlogger->error("SessionClient::cmdMsgParse_Country(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
	return false;
}
...
bool SessionClient::cmdMsgParse_Temp(const Cmd::t_NullCmd *pNullCmd,const DWORD nCmdLen)
{
	...
	Xlogger->error("SessionClient::cmdMsgParse_Temp(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
	return false;
}
...
bool SessionClient::cmdMsgParse_Union(const Cmd::t_NullCmd *pNullCmd,const DWORD nCmdLen)
{
	...
	case Cmd::Session::PARA_CHECK_USER_CITY:
		{
			...
			Xlogger->debug("SessionClient::cmdMsgParse_Union(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
			return true;
		}
		break;
	default:
	...
	Xlogger->error("SessionClient::cmdMsgParse_Union(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
	return false;
}
...
bool SessionClient::cmdMsgParse_Sept(const Cmd::t_NullCmd *pNullCmd,const DWORD nCmdLen)
{
	...
	Xlogger->error("SessionClient::cmdMsgParse_Sept(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
	return false;
}
...
bool SessionClient::cmdMsgParse_Other(const Cmd::t_NullCmd *pNullCmd,const DWORD nCmdLen)
{
	...
	switch (pNullCmd->para)
	{
		...
		{
		Xlogger->debug("加载特征码文件...%u 字节",ScenesService::updateStampData());
		return true;
		}
		break;
		*/
	...
	case Cmd::Session::PARA_SET_SERVICE:
		{
			...
			DWORD old = atoi(Seal::global["service_flag"].c_str());

			char buf[32];
			bzero(buf,sizeof(buf));
			_snprintf(buf,32,"%u",rev->flag);
			Seal::global["service_flag"] = buf;
			Xlogger->debug("设置服务 flag=%s",buf);
			...
			if (((old^rev->flag)&Cmd::Session::SERVICE_PROCESS) && ScenesService::pStampData)
			{
				...
				Xlogger->debug("%sprocess服务",ScenesService::pStampData->dwChannelID?"打开":"关闭");
			}
			return true;
		}
		break;
	...
	case Cmd::Session::PARA_UNION_DARE_NOTIFY:
		{
			...
			Xlogger->debug("收到帮会状态消息 state=%u",rev->state);
			return true;
		}
		break;
		// 给师父分配积分
	...
	case Cmd::Session::PARA_QUIZ_AWARD:
		{
			...
			if (pUser)
			{
				...
				if (rev->byType == 1)
				{
					...
					Xlogger->info("[个人答题]: %s(%u) 获得经验 %d",pUser->name,pUser->id,addExp);
					struct tm tv1;
					...
					if (!base)
					Xlogger->error("无法获取物品");
					zObject* o = NULL;
					o = zObject::create(base,1,0);
					if (o)
					{
					Xlogger->debug("create");
				...
				{
					Xlogger->info("[全国答题]: %s(%u) 获得经验 %d",pUser->name,pUser->id,addExp);
				}
...
#ifdef _DEBUG
			Xlogger->debug("收到物品查询命令: %s 查询 %s 的(%d)",rev->to_name,rev->from_name,
				rev->dwObjectTempID);
#endif        
	...
	case Cmd::Session::PARA_SCENE_REGSCENE_RET:
		{
			...
			if (scene)
			{
				if (rev->byValue==Cmd::Session::REGSCENE_RET_REGOK)
					Xlogger->info("注册 %s 成功",scene->name);
				else if (rev->byValue==Cmd::Session::REGSCENE_RET_REGERR)
				{
					Xlogger->error("注册 %s 失败",scene->name);
			...
			{
				Xlogger->error("注册场景%ld时未找到他",rev->dwTempID);
				if (rev->byValue==Cmd::Session::REGSCENE_RET_REGOK)
				{
	...
	case Cmd::Session::PARA_SCENE_UNREGUSER:
		{
			...
			if (scene)
			{
				...
				if (pUser)
				{
					...
					pUser->unreg();
					Xlogger->debug("Session请求注销(%s,%u)",pUser->name,pUser->id);
					Xlogger->debug("用户%ld注销时未找到他在地图%s,但是在用户管理器中找到",rev->dwUserID,scene->name);
				}
				else 
				{
					pUser=SceneUserManager::getMe().getUserByID(rev->dwUserID);
					if (pUser)
					{
						...
						{
							Xlogger->debug("收到Session广播消息注销用户(name=%s,id=%u,tempid=%u",pUser->name,pUser->id,pUser->tempid);
						}
				...
				Xlogger->error("未找到地图%ld",rev->dwSceneTempID);
			if (rev->retcode==Cmd::Session::UNREGUSER_RET_LOGOUT)
			{
				// 通知Session 注册失败
				Cmd::Session::t_unregUser_SceneSession ret;
				Xlogger->debug("Session请求注销,但未找到地图");
				...
				if (gate)
				{
					...
					if (SceneRecycleUserManager::getInstance().canReg(rev->dwID))
					{
						...
						if (pUser)
						{
							...
							Xlogger->debug("发现重复用户(%s,%u)",pUser->name,pUser->id);
						...
						{	
							...
							Xlogger->debug("发现重复用户,并且此人正在读档(%s,%u)",pUser->name,pUser->id);
							pUser->destroy();
							SAFE_DELETE(pUser);
							return true;
						...
						if (SceneUserManager::getMe().addUser(pUser))
						{
							...
							Xlogger->fatal("请求读档: 地图名:%s 地图ID:%d RegMapType:%d", 
								scene->name, scene->tempid, send.RegMapType);

							//Xlogger->debug("开始读取%ld(%s)档案",pUser->id,pUser->name);
					...
					{
						Xlogger->warn("退出等待中收到可疑的选择指令 %d",rev->dwID);
					}
				}
				else
				{
					Xlogger->fatal("网关断掉了 %d",rev->dwGatewayServerID);
				}
			}
			else
				Xlogger->fatal("未找地图 %s",(char *)rev->byMapName);
			// 通知会话服务器注册失败
	...
	case Cmd::Session::PARA_SCENE_REQ_ADD_SCENE:
		{
			...
			Xlogger->debug("收到加载地图消息(%u,%u,%u)",rev->dwServerID,rev->dwCountryID,rev->dwMapID);
			if (rev->dwServerID == ScenesService::getInstance().getServerID())
			{
				...
				if (loaded)
				{
					...
					Xlogger->info("加载%s(%ld,%ld)成功",loaded->name,loaded->id,loaded->tempid);
	...
	case Cmd::Session::PARA_SCENE_UNLOAD_SCENE:
		{
			...
			if (scene)
			{
				...
				Xlogger->info("地图%s目前在线人数%u",scene->name 
					,scene->countUser());
				...
				if (NULL !=pUser)
				{
					if (1 == rev->byRetcode)
					{
						...
						if (!pUser->packs.removeMoney(UnionDef::CREATE_UNION_NEED_PRICE_GOLD,"创建帮会")) {
							Xlogger->fatal("用户(%s)创建帮会时银子计算错误",pUser->name);
						}
...
// #endif
						...
						*/
						if (!pUser->packs.removeMoney(SeptDef::CREATE_SEPT_NEED_PRICE_GOLD,"创建家族"))                    {
							Xlogger->fatal("用户(%s)创建家族时银子计算错误",
								pUser->name);
						}
...
//						}
			...
			if (NULL !=pUser)
			{
				...
				*/
				if (!pUser->packs.removeMoney(CREATE_SCHOOL_REQUEST_PRICE_GOLD,"创建师门")) {
					Xlogger->fatal("用户(%s)创建门派时银子计算错误",pUser->name);
				}
	...
	case Cmd::Session::PARA_SCENE_CHANEG_SCENE:
		{
			...
			*/
			...
			Xlogger->info("%s(%d)跨服切换场景(%s-->%s)",pUser->name,pUser->id,pUser->scene->name,pUser->charbase.mapName);
			pUser->charbase.x = rev->x;
			pUser->charbase.y = rev->y;
			//        Xlogger->debug("存储下一场景(%s,%d,%d)",pUser->charbase.mapName,pUser->charbase.x,pUser->charbase.y);
			...
			if (rev->temp_id==189)
				Xlogger->info("%s PK值 %u,送往监牢",pUser->name,pUser->charbase.goodness);
			if (rev->temp_id==203)
				Xlogger->info("%s 恶意杀人,送往牢狱,时间 %u 分钟",pUser->name,pUser->charbase.punishTime);
	...
	case Cmd::Session::PARA_SCENE_FORBID_TALK:
		{
			...
			if (pUser)
			{
				...
				Xlogger->debug("处罚时未找到该玩家 id=%s",rev->name);
			return true;
			break;
		}
	...
	case Cmd::Session::PARA_SEPT_NPCDARE_ITEMBACK:
		{
			...
			if (pUser)
			{
				...
				{
					Xlogger->info("[家族争夺NPC]扣除角色%s40两银子成功",pUser->name);
				}
				else
				{
					Xlogger->info("[家族争夺NPC]扣除角色%s40两银子失败,有作弊嫌疑",pUser->name);
				}
				return true;
				/*
				...
				pUser->sendCmdToMe(&ret,sizeof(ret));
				Xlogger->info("给角色%s返还道具地羽令id=[%u]",pUser->name,item->data.qwThisID);
				}
				else 
				{
				Xlogger->fatal("给角色%s返还道具地羽令失败.",pUser->name);
				}
				}*/
			}
		}
		...
		if (!pUser)
		{
		Xlogger->info("SessionClient::cmdMsgParse_Other: 邮件检查时发送者 %s 离线",rev->mail.fromName);
		return false;
		}
		...
		Xlogger->info("%s 邮件检查成功后银子不足",pUser->name);
		...
		if (!srcobj)
		{
		Xlogger->info("%s 发送邮件时未找到该物品物品 id=%u",pUser->name,rev->itemID);
		return false;
		}
		...
		Xlogger->info("发送邮件 %s->%s",rev->mail.fromName,rev->mail.toName);
		return true;
		}
		else
		{
		Xlogger->error("邮件发送失败 %s->%s",rev->mail.fromName,rev->mail.toName);
		return false;
		}
		}
		break;
	...
	case Cmd::Session::PARA_SCENE_GET_MAIL_ITEM_RETURN:
		{
			...
			if (!pUser)
			{
				Xlogger->error("SessionClient::cmdMsgParse_Other [邮件]: 获取物品时发送者离线 mailID=%u",rev->mailID);
				return false;
			}
			...
			if (rev->item.object.dwObjectID)
			{
				...
				if (objbase==NULL) 
				{	
					//Xlogger->error("加载物品失败,道具基本表中不存在:%d",o->object.dwObjectID);
					return false;
				}
				...
				if (0==ob)
				{
					Xlogger->error("[邮件]%s 加载邮件附件失败 mailID=%u",pUser->name,rev->mailID);
				...
				*/
				...
				{
					Xlogger->error("[邮件]%s 添加邮件附件失败 mailID=%u",pUser->name,rev->mailID);
				...
				//Xlogger->info("[邮件]%s 通过邮件获得物品 %s",pUser->name,ob->data.strName);
			}
			if (rev->sendMoney)
			{
				pUser->packs.addMoney(rev->sendMoney,"邮件得到");
				Xlogger->info("[邮件]%s 通过邮件获得银子 %u",pUser->name,rev->sendMoney);
			}
			/*
			if (rev->sendGold)
			{
			pUser->packs.addGold(rev->sendGold,"邮件得到");
			Xlogger->info("[邮件]%s 通过邮件获得金子 %u",pUser->name,rev->sendGold);
			}
			*/
			pUser->save(OPERATION_WRITEBACK);//立刻存档
			...
			if (rush)
			{
				if (rush->init())
				{
					...
					*/
					...
					Xlogger->info("Session触发怪物攻城 %s",rush->rushName);
					return true;
				}

				SAFE_DELETE(rush);
				Xlogger->info("Session触发的攻城初始化失败");
				return true;
			}
		}
		break;
	...
	case Cmd::Session::PARA_SCENE_SEND_GIFT:
		{
			...
			if (rev->info.itemID)
			{
				...
				if (base)
				{
					...
					Xlogger->error("[Gift]发送奖品时,没找到物品 itemID=%u user=%s(%u)",rev->info.itemID,rev->info.name,rev->info.charID);
			}
			else
...
#ifdef _DEBUG
				Xlogger->debug("%s公告:%s mapID=%u",send.pstrName,send.pstrChat,rev->mapID);
#endif
			}
#ifdef _DEBUG
			else
				Xlogger->debug("%s公告:%s mapID=%u 没找到地图",rev->GM,rev->info,rev->mapID);
#endif
			return true;
		}
		break;
	...
	Xlogger->error("SessionClient::cmdMsgParse_Other(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
	return false;
}
...
bool SessionClient::cmdMsgParse(const Cmd::t_NullCmd *pNullCmd,const DWORD nCmdLen)
{
	...
	//Xlogger->error("收到session转发过来的消息%d",pNullCmd->cmd);
	...
	Xlogger->error("SessionClient::cmdMsgParse(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
	return false;
}
	...
	case PARA_AUCTION_CHECK_BID:
		{
			...
			if (!pUser)
			{
				Xlogger->warn("SessionClient::doAuctionCmd(PARA_AUCTION_CHECK_BID): 竞标检查后玩家已经下线");
				return true;
			}

			if (!pUser->packs.checkMoney(rev->money) || !pUser->packs.checkGold(rev->gold))
			{
				Xlogger->warn("SessionClient::doAuctionCmd(PARA_AUCTION_CHECK_BID): 竞标检查后玩家 %s 银子不足",pUser->name);
	...
	case PARA_AUCTION_CHECK_CANCEL:
		{
			...
			if (!pUser)
			{
				Xlogger->warn("SessionClient::doAuctionCmd(PARA_AUCTION_CHECK_CANCEL): 取消拍卖检查后玩家已经下线");
				return true;
			}
			...
			if (!base)
			{
			Xlogger->error("[拍卖]%s 取消拍卖时物品ID错误 itemID=%u auctionID=%u",pUser->name,rev->itemID,rev->auctionID);
			return false;
			}
...
bool SessionClient::doGmCmd(const Cmd::t_NullCmd *pNullCmd,const DWORD nCmdLen)
{
	...
	case Cmd::Session::GM_COMMAND_LOAD_PROCESS:
		{
			Xlogger->debug("加载特征码文件%u字节",ScenesService::updateStampData());
			break;
		}//case LOAD_PROCESS
		break;
		//新区配置GM指令
	...
	case PARA_CARTOON_DRAW:
		{
			...
			if (!pMaster)
			{
				Xlogger->error("[宠物]提取经验返回时,找不到主人 userID=%u cartoonID=%u exp=%u",rev->userID,rev->cartoonID,rev->num);
				return false;
			}
			...
			Xlogger->info("[宠物]%s(%u) 提取经验 %u cartoonID=%u",pMaster->name,pMaster->id,rev->num,rev->cartoonID);
			return true;
			return true;
		}
		break;
	...
	case PARA_CARTOON_CHARGE_NOTIFY:
		{
			...
			if (!pAdopter)
			{
				Xlogger->error("[宠物]宠物充值返回时找不到领养者 adopter=%s cartoonID=%u time=%u",rev->adopter,rev->cartoonID,rev->time);
				return false;
			}
	...
	case PARA_CARTOON_SALE:
		{
			...
			if (!pUser)
			{
				Xlogger->error("[宠物]出售宠物返回时找不到主人 userID=%u cartoonID=%u",rev->userID,rev->cartoonID);
				return false;
			}
			...
			Xlogger->info("[宠物]%s 出售宠物 %s",pUser->name,pUser->cartoonList[rev->cartoonID].name);
	...
	case PARA_CARTOON_ADD:
		{
			...
			if (pUser->cartoon && pUser->cartoon->getCartoonID()==rev->cartoonID)
			{
					...
					|| rev->data.state==Cmd::CARTOON_STATE_ADOPTED)
				{
					...
					Xlogger->debug("[宠物]%s(%u) 的宠物 %s(%u) 在跟随时被删除,session状态 %u",pUser->name,pUser->id,rev->data.name,rev->cartoonID,rev->data.state);
				}
				else
			...
			else if (rev->data.state==Cmd::CARTOON_STATE_PUTAWAY && !pUser->cartoon && pUser->charbase.petPoint)
			{
				...
				*/
				...
				Xlogger->info("%s 的宠物%s(%u)获得%u秒修炼时间",pUser->name,pUser->cartoonList[rev->cartoonID].name,rev->cartoonID,pUser->charbase.petPoint);
	...
	case PARA_CARTOON_ADOPT:
		{
			...
			for(SceneUser::adopt_it it=pUser->adoptList.begin(); it!=pUser->adoptList.end(); it++)
				...
				if (newPet)
				{
					...
					Xlogger->debug("[宠物]%s(%u级)领养了 %s(%u级) 的宠物(%u),现在有%u只 addExp=%u",pUser->name,pUser->charbase.level,rev->data.masterName,rev->data.masterLevel,rev->cartoonID,pUser->adoptList.size(),rev->data.addExp);
				}
	...
	case Cmd::Session::PARA_USE_SPORTS_REQUESTMAP:
		{
			...
			if (loaded)
			{
				Xlogger->info("动态加载%s(%d,%d)成功",loaded->name,loaded->id,loaded->tempid);
				regscene.dwID=loaded->id;
				regscene.dwTempID=loaded->tempid;
				strncpy(regscene.byName,loaded->name,MAX_NAMESIZE);
				strncpy(regscene.fileName,loaded->getFileName(),MAX_NAMESIZE);
				...
				//sky 通知GateWay注册动态地图
				char Buf[x_socket::MAX_DATASIZE];
				bzero(Buf,sizeof(Buf));
