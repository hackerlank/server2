          ...
          if (scene->zPosShortRange(getPos(),master->getPos(),SCREEN_WIDTH*2,SCREEN_HEIGHT*2))
          {
            if (this!=entry)
            {
              if (!((entry->frenzy)||(frenzy)))
              {
                ...
                {
                  isPlayer = true;
                  //Xlogger->debug("isPetEnemy 119 %s->%s",master->name,entry->name);
                  ret = isUserMasterEnemy(entry);
                }
                else
                {
                  //Xlogger->debug("isPetEnemy 124 %s->%s",master->name,entry->name);
	...
	case zSceneEntry::SceneEntry_NPC:
		{
			...
			if (m)
			{
				//Xlogger->debug("isPetEnemy 183 %s->%s",tm->name,n->getMaster()->name);
...
SceneEntryPk * ScenePet::getMaster()
{
  ...
  {
#ifdef _DEBUG
    //Xlogger->info("%s 找不到主人 masterTpye=%u masterID=%u needClear=%u",name,masterType,masterID,needClear());
#endif

    delCount++;
    if (delCount==100)
      Xlogger->info("%s 找不到主人 masterTpye=%u masterID=%u needClear=%u %s(%u,%u)",name,masterType,masterID,needClear(),scene->name,pos.x,pos.y);
...
bool ScenePet::isRedNamed(bool allRedMode)
{
  ...
  //Xlogger->error("ScenePet::isRedNamed(): %s 没有主人",name);
  return false;
}
...
#ifdef _DEBUG
  Xlogger->debug("发送宠物hp plus=%u getMaxHP=%u - petData.maxhp=%u",petData.maxhp_plus,getMaxHP(),petData.maxhp);
#endif
...
void ScenePet::setMaster(DWORD id,DWORD type)
{
  ...
  {
    Xlogger->info("ScenePet::setMaster(): 企图给 %s 设置非法的主人类型 type=%u",name,type);
    return;
  }
...
bool ScenePet::checkMasterTarget(SceneEntryPk *&ret)
{
  ...
  //Xlogger->error("ScenePet::checkMasterTarget(): %s 没有主人",name);
  ret = 0;
  return false;
}
...
void ScenePet::petDeath()
{
  ...
  if (master)
  {
    /* //马不会死了
       ...
       m->horse.horse(0);
       Xlogger->info("%s 的 %s 死亡",m->name,name);
       }
       */
...
#ifdef _DEBUG
      Xlogger->debug("发送宠物信息 name=%s ai=%x hp=%u maxhp=%u",name,petData.ai,petData.hp,petData.maxhp);
#endif
...
void ScenePet::getAbilityByLevel(DWORD level)
{
  ...
  if (aType>0)
  {
    ...
    Xlogger->debug("npc %s 未知的攻击类型 type=%d",name,aType);
  /*
     std::vector<DWORD> list;
...
{
  if (masterType==zSceneEntry::SceneEntry_Player)
    Xlogger->debug("[宠物]pet %s 因找不到主人(%u)而删除 %s(%u,%u)",name,masterID,scene->name,pos.x,pos.y);
  else
    Xlogger->debug("[宠物]pet %s 因找不到主人而删除 %s(%u,%u)",name,scene->name,pos.x,pos.y);
  setClearState();
}
