...
void GuardNpc::owner(SceneUser* user)
{
  ...
  user->guard = this;
  /*
  Xlogger->debug("[镖车] %s,%u,%s(%u,%u),%s(%u,%u) 接镖 _gold=%u",
          _name.c_str(),
...
#ifdef _DEBUG
  Xlogger->debug("[镖车]%s 镖车 reset time:%d",_name.c_str(),_time.sec()-SceneTimeTick::currentTime.sec());
#endif
}
...
void GuardNpc::check()
{
  ...
  if (_status == 0)
  {
    if (!_owner || abs((long)(pos.x-_owner->getPos().x))>SCREEN_WIDTH || abs((long)(pos.y-_owner->getPos().y))>SCREEN_HEIGHT)
    {
      ...
      moveAction = false;
      /*
      Xlogger->debug("[镖车] %s,%u,%s(%u,%u),%s(%u,%u) _gold=%u,0 -> 1",
              _name.c_str(),
          tempid,name,getPos().x,getPos().y,
  ...
  if (_status == 1)
  {
    if (_owner && _owner->scene==scene && (abs((long)(pos.x-_owner->getPos().x)) < SCREEN_WIDTH && abs((long)(pos.y-_owner->getPos().y)) < SCREEN_HEIGHT))
    {
      ...
      /*
      Xlogger->debug("[镖车] %s,%u,%s(%u,%u),%s(%u,%u) _gold=%u,1 -> 0",
              _name.c_str(),
          tempid,name,getPos().x,getPos().y,
          _owner ? _owner->name : "NULL",_owner ? _owner->getPos().x : 0,_owner ? _owner->getPos().y : 0,
          _gold);
    ...
    {
      /*
      Xlogger->debug("[镖车] %s,%u,%s(%u,%u),%s(%u,%u) _gold=%u,1 -> 2,3分钟没找到主人,运镖失败",
              _name.c_str(),
          tempid,name,getPos().x,getPos().y,
          _owner ? _owner->name : "NULL",_owner ? _owner->getPos().x : 0,_owner ? _owner->getPos().y : 0,
          _gold);
  ...
  */
  ...
  //Xlogger->debug("check: current time:%d,check time:%d",SceneTimeTick::currentTime.msecs(),_time.msecs());
  if (_status == 2  && SceneTimeTick::currentTime > _time) {
    //Xlogger->debug("clear: current time:%d,check time:%d",SceneTimeTick::currentTime.msecs(),_time.msecs());
    setClearState();
    return;
  }
...
/*
    ...
    Xlogger->debug("%s劫镖成功,获得银子%d",m->name,_gold);
*/
  ...
  SceneUser * _owner = (SceneUser *)getMaster();
  /*
  Xlogger->debug("[镖车] %s,%u,%s(%u,%u),%s(%u,%u),死亡,杀手 %s _gold=%u",
              _name.c_str(),
      tempid,name,getPos().x,getPos().y,
      _owner ? _owner->name : "NULL",_owner ? _owner->getPos().x : 0,_owner ? _owner->getPos().y : 0,
      m->name,_gold);
...
void GuardNpc::on_reached()
{
  ...
  /*
  Xlogger->debug("[镖车] %s,%u,%s(%u,%u),%s(%u,%u),成功运到 _gold=%u",
              _name.c_str(),
      tempid,name,getPos().x,getPos().y,
      _owner ? _owner->name : "NULL",_owner ? _owner->getPos().x : 0,_owner ? _owner->getPos().y : 0,
      _gold);
