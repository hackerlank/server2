...
zSkill *zSkill::create(SceneEntryPk *pEntry,DWORD id,DWORD level)
{
	...
	{
		Xlogger->debug("zSkill::create():无法将新技能加到角色身上");
		SAFE_DELETE(ret);
		return NULL;
	}
...
bool zSkill::action(const Cmd::stAttackMagicUserCmd *rev,const DWORD cmdLen)
{
  ...
  {
    Xlogger->error("学习的技能中_entry指针为空");
  }
  return false;
}
        ...
        case 20:
          {
            ...
            DWORD itemNum = 0;
#ifdef _DEBUG
            Xlogger->error("!!!---客户端发过来的攻击指令原始长度[%u] 当前长度[%u]",tempLen,curRevCmdLen);
#endif
            if (curRevCmdLen>tempLen)
            {
              itemNum = (curRevCmdLen-tempLen)/sizeof(DWORD);
#ifdef _DEBUG
              Xlogger->error("!!!---客户端发过来的攻击列表数目[%u]",itemNum);
#endif
...
#ifdef _DEBUG
                Xlogger->error("!!!---客户端发过来的被攻击临时坐标[%u]",curRevCmd->dwTempIDList[i]);
#endif
...
*/    
#ifdef _DEBUG
                Xlogger->error("!!!---客户端发过来的攻击坐标[%u][x=%u,y=%u]",curRevCmd->dwTempIDList[i],pd.x,pd.y);
#endif
