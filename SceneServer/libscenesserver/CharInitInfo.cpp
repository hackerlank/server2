...
bool CharInitInfo::init()
{
  ...
  rwlock.wrlock();
  zXMLParser xml;
  if (!xml.initFile(Seal::global["confdir"] + "initcharinfo.xml"))
  {
    Xlogger->error("加载initcharinfo.xml失败");
    return false;
  }
  ...
  if (root)
  {
    ...
    while(node)
    {
      if (strcmp((char *)node->name,"object") == 0)
      {
        ...
        //Xlogger->debug("%u,%s,%u,%u,%u",object.id,object.name,object.localeID,object.num,object.profession);
        objects.insert(ObjectsContainer::value_type(object.profession,object));
      }

      node = xml.getNextNode(node,NULL);
    }
    Xlogger->info("初始化初始物品系统成功");
    rwlock.unlock();
    return true;
  }
  rwlock.unlock();

  Xlogger->error("加载角色初始信息配置文件失败");
  return false;
}
...
void CharInitInfo::get(const WORD profession,InitObjectVector &objs)
{
  ...
  //Xlogger->debug("%u,%u",profession,objs.size());
}
