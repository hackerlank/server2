...
//#if 0\
  ...
  Xlogger->error("升级成功机率为%d",base->odds + extra_odds);
...
bool EquipMaker::check_material(zObjectB* ob,const std::map<DWORD,DWORD>& list,bool is_resource)
{
  ...
  {
    Xlogger->debug("打造%s需要材料%d",ob->name,it->id);
  }
  // */
...
*/
  ...
  if (((int)id == _1_id && (int)level == _1_level) || ((int)id == _2_id && (int)level == _2_level) ) {
    ...
    //Xlogger->debug("current:%f\tbase:%f",_current,_base);
  }else {
    _current += static_cast<double>( coff*5.0f*num*(level-it->level+1));
    _base += static_cast<double>(coff*5.0f*num);
    //Xlogger->debug("current:%f\tbase:%f",_current,_base);
...
void EquipMaker::bonus_hole(zObject* ob)
{
  ...
  if (bonus_hole>0) {
    ...
    Xlogger->debug("材料等级(%f),奖励孔(%d)",_odds.material_level,bonus_hole);
  }
}
...
{  
  ...
  /*if (user)
    Xlogger->debug("[打造]用户(%s)材料等级(%s:%.10f)",user?user->name:"",base->name,_odds.material_level);

  int white = odds_of_white(base);

  if (user)
    Xlogger->debug("[打造]用户(%s)生成白色装备概率(%s:%f\%)",user?user->name:"",base->name,white*1.0);
...
*/
  ...
  if (!pack || pack->object()->data.dur < 1 || user.tradeorder.in_trade(pack->object()) ) {
    Xlogger->warn("用户(%d:%d:%s)请求合成的包裹不存在,使用次数不够或在交易中",user.charbase.accid,user.charbase.id,user.charbase.name);
    return false;
  }
  ...
  if (!dc.can_compose || !dc.num ) {
    Xlogger->warn("用户(%d:%d:%s)请求合成不可合成物品",user.charbase.accid,user.charbase.id,user.charbase.name);
    return false;      
  }

  if (command->per_num <3 || command->per_num > 5 || (dc.num % command->per_num) || (dc.num > command->per_num*dc.max_num) ) {
    Xlogger->warn("用户(%d:%d:%s)请求合成物品数量错误",user.charbase.accid,user.charbase.id,user.charbase.name);
    return false;
  }
...
//    pack->clear(&user);
    ...
    response(user,1,COMPOSE);      
    Xlogger->error("为用户(%d:%d:%s)添加合成物品时失败",user.charbase.accid,user.charbase.id,user.charbase.name);
    return false;
  }
...
*/
  ...
  if (!dc.can_compose()) {
    Xlogger->warn("用户(%d:%d:%s)请求升级不可升级物品",user.charbase.accid,user.charbase.id,user.charbase.name);
    return false;      
  }
...
bool RebuildObject::hole(SceneUser& user,const Cmd::stPropertyUserCmd* cmd)
{
  ...
  if (!up_ob || !Hole::can_hole(up_ob)) {
    Xlogger->warn("用户(%d:%d:%s)请求对不能打孔装备打孔",user.charbase.accid,user.charbase.id,user.charbase.name);
    return false;      
  }
  ...
  if (!ob || ob->data.dwObjectID != HOLE_SONTE_ID || Hole::get_hole_num(up_ob) == Hole::INVALID_NUM || ob->data.upgrade != (Hole::get_hole_num(up_ob)-1) ) {
    Xlogger->warn("用户(%d:%d:%s)对装备打孔时缺少宝石",user.charbase.accid,user.charbase.id,user.charbase.name);
    return false;      
  }
...
bool RebuildObject::enchance(SceneUser& user,const Cmd::stPropertyUserCmd* cmd)
{
  ...
  if (!dc.can_compose()) {
    Xlogger->warn("用户(%d:%d:%s)请求镶嵌不可镶嵌物品",user.charbase.accid,user.charbase.id,user.charbase.name);
    return false;      
  }
...
bool RebuildObject::decompose(SceneUser& user,const Cmd::stPropertyUserCmd* cmd)
{
  ...
  if (!up_ob || up_ob->base->make != 1) {
    Xlogger->warn("用户(%d:%d:%s)请求分解不能分解装备",user.charbase.accid,user.charbase.id,user.charbase.name);
    return false;
  }
...
/*zObject *RebuildObject::remake(SceneUser& user,const Cmd::stReMakObjectUserCmd* cmd,DWORD id)
{
  ...
  if (!up_ob || up_ob->make != 1) {
    Xlogger->warn("用户(%d:%d:%s)请求打造不可打造物品",user.charbase.accid,user.charbase.id,user.charbase.name);
    return NULL;  
  }
...
int RebuildObject::make(SceneUser& user, DWORD dwID, int num, BYTE kind)
{
	...
	if(NULL == up_ob)
	{
		...
		Xlogger->debug("用户%s打造普通装备id = %u失败",user.name,dwID);
		return 0;
	}
	...
	{
		Xlogger->debug("用户%s打造普通装备%s失败,可能是npctrade填写有问题",user.name,up_ob->name);
		return 0;
	}
