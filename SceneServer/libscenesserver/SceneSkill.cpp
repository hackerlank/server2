...
bool SceneUser::upgradeSkill(DWORD dwSkillID,bool needSkillPoint)
{
  ...
  if (!skill)
  {
	...
	{
		Xlogger->error("用户:[%s]企图学习该用户职业不可以学习的技能:[%s]", charbase.name, skill->base->name);
		return false;
	}
...
#ifdef _DEBUG
  Xlogger->error("技能升级检查前提技能点数 当前点[%u] 需要点[%u] 当前BASE技能等级[%u] dwSkillID=[%u]",this->usm.getPointInTree(skill->base->subkind),skill->base->needpoint,skill->base->level,dwSkillID);
#endif
