...
/**
 * \brief 重载x_tcp_buffer_client的run方法
 *
 *
 */
...
bool RecordClient::cmdMsgParse(const Cmd::t_NullCmd *pNullCmd,const DWORD nCmdLen)
{
		...
		case PARA_SCENE_USERINFO:
			{
				...
				if (scene)
				{
					//Xlogger->debug("得到场景%s",scene->name);
					if (pUser)
					{
						bool dataOk=false;
						//Xlogger->trade("读取%ld档案成功",rev->id);
						...
						//Xlogger->debug("%u,%u",rev->charbase.x,rev->charbase.y);
						// load Object
						//设置宠物包裹的大小
						...
						if (pack)
						{
							...
							//Xlogger->debug("%s 宠物包裹大小 %u",pUser->name,pUser->charbase.petPack);
						}
						if (rev->dataSize != (DWORD)PARA_SCENE_USER_READ_ERROR)
						{
							...
							{       
								...
								if (!scene)
								{
									Xlogger->fatal("读取%u档案失败,其等级与地图不符合,但未找到新手村 地图",rev->id);
									return true;
								}
							}   
							if (dataOk)
							{
								...
								if(rev->RegMapType == 1)
								{
									...
									Xlogger->fatal("进入老地图的 name:%s x:%d y:%d RegMapType:%d",scene->name, pUser->charbase.x, pUser->charbase.y, rev->RegMapType);
								}

								Xlogger->fatal("进入新地图的 name:%s x:%d y:%d RegMapType:%d",scene->name, pUser->charbase.x, pUser->charbase.y, rev->RegMapType);
								...
								if (pUser->intoScene(scene,true,pUser->getPos()))
								{
									...
									//检查新邮件
									if (Seal::global["mail_service"]=="on")
									{
							...
							{
								Xlogger->fatal("加载%u档案失败,可能是表格错误",rev->id);
							}
						}
						else
						{
							Xlogger->fatal("读取%u档案失败,无效用户数据",rev->id);
						}
					}
					else
						Xlogger->fatal("读取%u档案失败,未找到用户数据",rev->id);
				}
				else
					Xlogger->fatal("读取%u档案失败,未找到地图,maptempid=%u",rev->id,rev->dwMapTempID);
				/*
				// 通知Session 注册失败
				...
				Xlogger->debug("档案失败注销(%s,%u)",pUser->name,pUser->id);
				pUser->destroy();
				SAFE_DELETE(pUser);
				}
				// */
      ...
      case PARA_SCENE_USER_EXIST:
        {
          ...
          if (!pUser)
          {
            Xlogger->warn("[邮件]检查目标玩家 %s 后,发送者已不在线",rev->sm.toName);
            return false;
          }
          ...
          if (rev->sm.itemID && rev->sm.itemID != 0xffffffff)//INVALID_THISID
          {
            ...
            if (!srcobj)
            {
              Xlogger->info("%s 发送邮件时未找到该物品物品 id=%u",pUser->name,rev->sm.itemID);
              return false;
            }
            if (!srcobj->canMail())
            {
          ...
          if (sessionClient->sendCmd(&sm,sizeof(Cmd::Session::t_sendMail_SceneSession)))
          {
            ...
            Xlogger->info("发送邮件 %s->%s",pUser->name,rev->sm.toName);
            return true;
          }
          else
          {
            Xlogger->error("邮件发送失败 %s->%s money=%u gold=%u",pUser->name,rev->sm.toName,rev->sm.sendMoney,rev->sm.sendGold);
            return false;
          }
          return true;
        }
  ...
  Xlogger->error("RecordClient::cmdMsgParse(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
  return false;
}
