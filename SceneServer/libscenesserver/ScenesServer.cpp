...
bool ScenesService::init()
{
	...
	Xlogger->info("ScenesService::init()");
	...
	taskPool = new zTCPTaskPool(atoi(Seal::global["threadPoolServer"].c_str()),state,5000);

	max_level = atoi(Seal::global["maxlevel"].c_str());
	max_exp = atoi(Seal::global["maxExp"].c_str());

	char *duplist = const_cast<char*>(Seal::global["duplicate"].c_str());
	...
	trun_point_rate = atoi(Seal::global["trun_point_rate"].c_str());
	trun_skill_rate = atoi(Seal::global["trun_skill_rate"].c_str());

	//sky 从配置文件中读取极品掉落基本配置
	g_blue_changce = atoi(Seal::global["g_blue_changce"].c_str());		//蓝色
	g_green_changce = atoi(Seal::global["g_green_changce"].c_str());		//绿色
	g_purple_changce = atoi(Seal::global["g_purple_changce"].c_str());	//紫色
	g_orange_changce = atoi(Seal::global["g_orange_changce"].c_str());	//橙色

	//sky 从配置文件中读取打坐恢复系数
	recoverRate = atoi(Seal::global["recoverRate"].c_str());

	fprintf(stderr,"maxlevel %ld\n",atoi(Seal::global["maxlevel"].c_str()));
	fprintf(stderr,"maxlexp %ld\n",atoi(Seal::global["maxExp"].c_str()));
	...
	{
		Xlogger->error("ScenesService::init 连接档案服务器失败");
		return false;
	}
	if (recordClient->start())
	{
	}
	//Xlogger->info("初始化档案服务器模块(%s:%d)成功",serverEntry->pstrIP,serverEntry->wdPort);
	...
	{
		Xlogger->error("ScenesService::init 连接会话服务器失败");
		return false;
	}
	if (sessionClient->start())
		Xlogger->info("初始化会话服务器模块(%s:%d)成功",serverEntry->pstrIP,serverEntry->wdPort);
	...
	{
		Xlogger->error("ScenesService::init 连接小游戏服务器失败");
		return false;
	}
	if (!miniClient->start())
	{
		Xlogger->warn("初始化Mini服务器模块失败");
	}
	else{
		Xlogger->info("初始化Mini服务器模块(%s:%d)成功",serverEntry->pstrIP,serverEntry->wdPort);
	}

	if (!SceneNpcManager::getMe().init())
	{
		Xlogger->warn("初始化NPC管理器失败");
	}
	else{
		Xlogger->info("初始化NPC管理器成功");
	}

	if (!SceneTimeTick::getInstance().start())
	{
		Xlogger->warn("初始化TimeTick模块失败");
	}
	else{
		Xlogger->info("初始化TimeTick模块成功");
	}

	//加载基本数据
	if (!loadAllBM())
	{
		Xlogger->error("初始化基本数据模块失败");
		return false;
	}
	Xlogger->info("初始化基本数据模块成功");

	char srv[256];
	bzero(srv,sizeof(srv));
	sprintf(srv,"WS[%d]",getServerID());
	objlogger = new zLogger(srv);
	objlogger->setLevel(Seal::global["log"]);
	//设置写本地日志文件
	if ("" != Seal::global["objlogfilename"])
	{
		bzero(srv,sizeof(srv));
		char sub[256];
		bzero(sub,sizeof(sub));
		_snprintf(srv,sizeof(srv),"%s",Seal::global["objlogfilename"].c_str());
		char *tok = strstr(srv,".");
		if (tok != NULL)
		{
	...
	gmlogger->setLevel(Seal::global["log"]);
	if ("" != Seal::global["gmlogfilename"]){
		gmlogger->addLocalFileLog(Seal::global["gmlogfilename"]);
		gmlogger->removeConsoleLog();
	}

	wglogger = new zLogger("wglog");
	wglogger->setLevel(Seal::global["log"]);
	if ("" != Seal::global["wglogfilename"]){
		wglogger->addLocalFileLog(Seal::global["wglogfilename"]);
		wglogger->removeConsoleLog();
	}

	Xlogger->info("加载特征码文件,大小%u",updateStampData());

	if (!SceneManager::getInstance().init())
	{
		Xlogger->error("初始化场景管理器失败");
		return false;
	}
	Xlogger->info("初始化场景管理器成功");

	if (!NpcTrade::getInstance().init())
	{
		Xlogger->error("初始化NPC交易配置模块失败");
		return false;
	}
	Xlogger->info("初始化NPC交易配置模块成功");
	...
	{
		Xlogger->error("初始化任务模块失败");
		return false;
	}
	Xlogger->info("初始化任务模块成功");
	...
	{
		Xlogger->error("初始化攻击范围定义模块失败");
		return false;
	}
	Xlogger->info("初始化攻击范围定义模块成功");
	...
	Xlogger->info("ScenesService::init() OK");
...
bool ScenesService::msgParse_SuperService(const Cmd::t_NullCmd *pNullCmd,const DWORD nCmdLen)
{
	...
	Xlogger->error("ScenesService::msgParse_SuperService(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
	return false;
}
...
void ScenesService::final()
{
	...
	Xlogger->debug("ScenesService::final");
...
{
	reload=true;
	Xlogger->debug("ScenesService::reloadConfig");
}
...
void ScenesService::checkAndReloadConfig()
{
	...
	{
		reload=false;
		Xlogger->debug("ScenesService::checkAndReloadConfig");
		SceneConfile sc;
		...
		//定时存档配置
		if (atoi(Seal::global["writebacktimer"].c_str()))
		{
			ScenesService::getInstance().writeBackTimer = atoi(Seal::global["writebacktimer"].c_str());
		}
		else
		{
		...
		ScenesService::getInstance().ExpRate       = atoi(Seal::global["ExpRate"].c_str());
		ScenesService::getInstance().DropRate      = atoi(Seal::global["DropRate"].c_str());
		ScenesService::getInstance().DropRateLevel = atoi(Seal::global["DropRateLevel"].c_str());
...
DWORD ScenesService::updateStampData()
{
	...
	process_file = Seal::global["confdir"] + "process.dat";
	// f = open(process_file.c_str(),O_RDONLY);
	/* if (f != -1)
	{
	pStampData = (Cmd::stChannelChatUserCmd *)malloc(x_socket::MAX_DATASIZE);
	bzero(pStampData,x_socket::MAX_DATASIZE);
	...
	pStampData->dwFromID = read(f,(void *)(pStampData->tobject_array),x_socket::MAX_DATASIZE-sizeof(Cmd::stChannelChatUserCmd));
	close(f);

	pStampData->dwChannelID = atoi(Seal::global["service_flag"].c_str()) & Cmd::Session::SERVICE_PROCESS;
...
{
	Xlogger=new zLogger("ScenesServer");


	//设置缺省参数
	Seal::global["datadir"]         = "data/";
	Seal::global["confdir"]         = "conf/";
	Seal::global["questdir"]        = "quest/";
	Seal::global["cmdswitch"]       = "true";
	Seal::global["writebacktimer"]  = "600";
	Seal::global["ExpRate"]         = "1";
	Seal::global["DropRate"]        = "1";
	Seal::global["DropRateLevel"]   = "0";
	Seal::global["mail_service"]    = "on";
...
bool ScenesService::LoadSuitInfo()
{
	...
	{
		Xlogger->error("加载套装配置文件 %s 失败",sFileName);
		return false;
	}
	xmlNodePtr _root;
	xmlNodePtr _suits;
	...
	Xlogger->error("读取套装配置文件 %s 失败",sFileName);
	return false;
}
//End Shx
...
bool ScenesService::LoadItmeCoolTime()
{
	...
	{
		Xlogger->error("加载物品冷却文件 %s 失败",CoolFileName);
		return false;
	}
