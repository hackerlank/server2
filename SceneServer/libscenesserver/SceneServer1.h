  ...
  if (pDefine)
  {
    ...
    if (sceneNpc)
    {
      ...
      if (sceneNpc->getState() == zSceneEntry::SceneEntry_Normal)
      {
        ...
        Xlogger->debug("%s 初始状态 %u",sceneNpc->name,sceneNpc->getState());
#endif
      return sceneNpc;
    }
    else
    {
      Xlogger->fatal("Scene::summonOneNpc:SceneNpc分配内存失败");
      SAFE_DELETE(pDefine);
    }
  }
  else
  {
    Xlogger->fatal("Scene::summonOneNpc:t_NpcDefine分配内存失败");
  }
  return NULL;
}
 ...
 *
 */
class SceneTaskManager : private boost::noncopyable
{
 ...
 * 
 */
class RecordClient : public x_tcp_buffer_client,public MessageQueue
{
  ...
  public:
    ...
    RecordClient(const std::string &name,const std::string &ip,const WORD port)
      : x_tcp_buffer_client(name,ip,port,false) {};
...
class SessionClient : public x_tcp_buffer_client,public MessageQueue
{
  ...
  public:
    ...
    */
    SessionClient(
        ...
        const std::string &ip,
        const WORD port)
      : x_tcp_buffer_client(name,ip,port) {};
  ...
  private:
    ...
    template <typename T>
      bool additive(T& ret,const luckRangeValue & lv,int& property)
      {
        ...
        //    Xlogger->debug("属性产生概率%f,%f",lv.per*1.0,odds*1.0);
        if (selectByTenTh(odds) )  {
          ++property;
...
public:
  ...
  {
    Operation op;
//    Xlogger->debug("id:%d\tname:%s\tcondition:%d\toperation:%s",_id,_name.c_str(),_condition,op.name().c_str());
    if (_id) {
...
//    Xlogger->debug("id:%d\tname:%s\tcondition:%d\toperation:%s",id,_name.c_str(),_condition,op.name().c_str());
...
public:  
  ...
  bool check_valid(const SceneUser* user,const Vars* vars) const
  {
    ...
    //Xlogger->debug("Time_START:%02d:%02d:%02d",_start.tm_hour,_start.tm_min,_start.tm_sec);
    //Xlogger->debug("Time_NOW  :%02d:%02d:%02d",now.tm_hour,now.tm_min,now.tm_sec);
    //Xlogger->debug("Time_END  :%02d:%02d:%02d",_end.tm_hour,_end.tm_min,_end.tm_sec);
...
public:
  ...
  {
    Operation op;
//    Xlogger->debug("id:%d\tname:%s\taction:%d\toperation:%s",_id,_name.c_str(),_action,op.name().c_str());          
    if (_id) {
      Vars* v = user->quest_list.vars(_id);
      if (v) {
  ...
  int done (SceneUser* user,Vars* vars)
  {
    ...
    if (op.name() == "Sub") {
      if (!user->packs.removeMoney(_value,"任务减少")) {
        Xlogger->fatal("任务银子计算错误: 用户(%s),任务(%d)",user->name,vars->quest_id());
      }
...
public:
  ...
  { 
    p.value("value",_level);
    Xlogger->debug("初始化_level%d",_level);
  }
  ...
  int done (SceneUser* user,Vars* vars)
  {
    ...
    Xlogger->debug("发送_level%d",_level);
...
public:
  ...
  { 
    ...
    if (!_s) {
      Xlogger->warn("NpcAction时请求的场景(%s)不存在",map.c_str());
    }
...
public:
  ...
  { 
    ...
    if (/*(regex.compile("(.*)[ ,](.*)") && */regex.match(pos.c_str()))) {
      ...
      //Xlogger->debug("AddGuardAction::AddGuardAction(): _dest=%u,%u",_dest.x,_dest.y);
    }
    else
      Xlogger->info("AddGuardAction::AddGuardAction(): _dest匹配失败 %s",pos.c_str());
...
inline int execute_script_event(SceneUser * user,const char* func)
{
  ...
  {
    Xlogger->debug("CATCHED Luabind EXCEPTION:%s\n%s",func,e.what());
    return 0;
  }          
  catch (const char* msg)
  {
    Xlogger->debug("CATCHED (...) EXCEPTION:%s\n%s",func,msg);
    return 0;
  }
...
int execute_script_event(SceneUser * user,const char* func,P1& p1,P2& p2,P3& p3)
{
  if (user == NULL )
  {
    ...
    current_user=user;
  Xlogger->debug("execute_script_event:%s,%s,%u,%u",func,p1->name,p2,p3);
  try {
    LuaVM* vm = ScriptingSystemLua::instance().getVM(0);
