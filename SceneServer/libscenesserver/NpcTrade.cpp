...
bool NpcTrade::init()
{
  ...
  zXMLParser xml;
  if (!xml.initFile(Seal::global["confdir"] + "npctrade.xml"))
  {
    Xlogger->error("加载npctrade.xml失败");
    return false;
  }
  ...
  {
    Xlogger->debug("%u,%s",it->second.id,it->second.menu);
    for(NpcItemMultiMap::iterator item = it->second.items.begin(); item != it->second.items.end(); item++)
    {
      Xlogger->debug("%u,%u,%u",item->second.id,item->second.kind,item->second.action);
    }
  }
#endif

  Xlogger->error("加载npc交易配置文件失败");
  return false;
}
...
bool NpcTrade::verifyNpcAction(const DWORD npcid,const NpcItem &item)
{
  ...
  for(NpcDialogMultiMap::const_iterator npcDialog = dialogs.begin(); npcDialog != dialogs.end(); npcDialog++)
  {
    if (npcDialog->second.npcid == npcid)
    {
      ...
      {
        //Xlogger->debug("%u,%u,%u,%u,%u",it->second.id,it->second.kind,it->second.lowLevel,it->second.level,it->second.action);
