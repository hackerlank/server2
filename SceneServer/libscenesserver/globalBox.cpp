...
{
	//Xlogger->error("_boxItem.qualityID = %u\n",_boxItem.qualityID);
...
int globalBox::init()
{
	...
	while(NULL != currentNode)
	{
		if(!xmlStrcmp(currentNode->name,(const xmlChar *)"item"))
		{
			...
			//Xlogger->debug("quality %d",_boxItem.qualityID);
			...
			//for( ; it != end; ++it)
			//{
			//	Xlogger->debug("needlevel %d",it->_obj->needlevel);
			//}
