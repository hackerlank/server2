...
box *boxCircle::generateOneBox(bool isGloden,int sex,int level)
{
	...
	else
	{
		for(int i = 0; i < 17;++i)
		{
			...
			//Xlogger->error("quality = %u",quality);
			std::map<unsigned int,struct boxItem> *_boxItems = &(globalBox::getInstance()._boxItems);
			struct boxItem _boxItem = (*_boxItems)[quality];
			if(sex == 1)
			{
			...
			//Xlogger->error("len = %u",len);
			//unsigned int index = getRandomIndex(len,i+1);
			//Xlogger->error("index = %u",index);
			//_box->items[i] = _boxItem._boxObjects[index].obj_id;
			//Xlogger->error("id = %u",_boxItem._boxObjects[index].obj_id);
	...
	/*for(int i = 0; i < 17; ++i)
	{
		Xlogger->error("print id = %u",_box->_boxObjects[i].obj_id);
	}*/
...
void boxCircle::createBoxCircle()
{
	...
	for(int i = 0; i < 11; ++i)
	{
		...
		//Xlogger->error(" index %u ",index1);
		//Xlogger->error(" index %u ",index2);
		_swap(silverBoxCircle[index1],silverBoxCircle[index2+1]);
		_swap(silverBoxCircle[index1+1],silverBoxCircle[index2]);
	}

	/*Xlogger->error("------silver box----------- ");

	for(int i = 0; i < 11; ++i)
	{
		Xlogger->error(" %d ",silverBoxCircle[i]);
	}
	Xlogger->error("------end silver box----------- ");
*/
	...
	/*Xlogger->error("------glod box----------- ");

	for(int i = 0; i < 100; ++i)
	{
		Xlogger->error(" %d ",glodenBoxCircle[i]);
	}

	Xlogger->error("------end glod box----------- ");
*/
