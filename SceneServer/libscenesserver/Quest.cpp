    ...
    case Cmd::CART_CONTROL_QUEST_PARA:
      {
        if (user.guard)
        {
          ...
          if (user.guard->canMove())
            Xlogger->debug("%s 开始行走",user.guard->name);
          else
            Xlogger->debug("%s 停止行走",user.guard->name);
        }
        return true;
      }
      break;
...
int Vars::save(BYTE* dest) const
{
  ...
  for (const_var_iterator it=_vars.begin(); it!=_vars.end(); ++it) {
    if (!it->second.is_tmp()) {
      ...
      //Xlogger->debug("存储变量(%s:%s)",it->first.c_str(),it->second.value().c_str());
    }
  }
  //store count
...
int Vars::load(BYTE* dest)
{
  ...
  while ( (*count)-- > 0) {
    ...
    //Xlogger->debug("读取变量(%s:%s)",name.c_str(),value.c_str());
  }
...
int Vars::notify(SceneUser& user) const
{
  ...
  for (const_var_iterator it=_vars.begin(); it!=_vars.end(); ++it) {
    ...
    if (offset >= (int)(x_socket::MAX_DATASIZE - sizeof(Cmd::stQuestVarsUserCmd)
          - (Cmd::stQuestVarsUserCmd::MAX_NSIZE + Cmd::stQuestVarsUserCmd::MAX_VSIZE) ) ) {
      Xlogger->error("任务(%d)变量数太多",_quest_id);
      break;
    }
    //name
    ...
    //Xlogger->debug("var:(%s,%s)",it->first.c_str(),it->second.value().c_str());
  }
...
bool UserVar::save() const
{
  ...
  for (const_vars_iterator it=_vars.begin(); it!=_vars.end(); ++it) {
    ...
    {
      Xlogger->fatal("存储全局变量时，缓冲区过短越界（%u,%u）",MAX_BUF_SIZE,len);
      of.write(buf,len);
      bzero(buf,sizeof(buf));
      len = 0;
    }
...
bool UserVar::load()
{
  ...
  if (length > 0)
  {
    ...
    if (buf)
    {
      ...
      Xlogger->fatal("加载任务变量文件分配内存失败：%u",length);
  }
...
void QuestList::add_quest(DWORD id,const Vars& vars,SceneUser& user,bool notify)
{
  ...
  if (quest ) {
    ...
    //Xlogger->debug("quest time(%d)",info->start);
    ...
    //Xlogger->debug("quest info(%s)",info->info);
...
int QuestList::get_menu(char* menu,int& status)
{
  int len;
  if (!_menu.empty() ) {
    len =  x_socket::MAX_DATASIZE - strlen(menu) - sizeof(Cmd::stVisitNpcTradeUserCmd) - strlen(HAS_TASK_DIALOG);
...
int QuestList::notify(SceneUser& user) const
{
  for (const_quest_iterator it=_quests.begin(); it!=_quests.end(); ++it) {
    ...
    if (quest ) {
      ...
      //Xlogger->error("用户(%d,%d)带有不存在的任务(%d)",user.accid,user.charbase.id,it->first);
    }
  }
