...
SceneUserManager &SceneUserManager::getMe()
{
  if (sum==NULL)
  {
    ...
    //Xlogger->debug("读写锁计数:(读=%u,写=%u)",sum->rwlock.rd_count,sum->rwlock.wr_count);
  }
  return *sum;
}
...
void SceneUserManager::removeUserByTask(SceneTask *task)
{
  ...
  for(std::vector<DWORD>::iterator iter = exec.del_vec.begin() ; iter != exec.del_vec.end() ; iter ++)
  {
    ...
    if (pUser)
    {
      ...
      //pUser->killAllPets();
      pUser->unreg();
      Xlogger->info("用户%s(%ld)因卸载网关注销",pUser->name,pUser->id);
    }
    else
    {
      ...
      if (pUser)
      {
        Xlogger->info("用户%s(%ld)因卸载网关注销,但这个人正在读取档案",pUser->name,pUser->id);
...
void SceneUserManager::removeAllUser()
{
  ...
  for(std::vector<DWORD>::iterator iter = exec.del_vec.begin() ; iter != exec.del_vec.end() ; iter ++)
  {
    ...
    if (pUser)
    {
      ...
      Xlogger->info("用户%s(%ld)因服务器关闭卸载",pUser->name,pUser->id);
      OnQuit event(1);
      EventTable::instance().execute(*pUser,event);
      execute_script_event(pUser,"quit");
...
void SceneUserManager::removeUserInOneScene(Scene *scene)
{
  ...
  for(std::vector<DWORD>::iterator iter = exec.del_vec.begin() ; iter != exec.del_vec.end() ; iter ++)
  {
    ...
    if (pUser)
    {
      //pUser->save(Cmd::Record::LOGOUT_WRITEBACK);
      //SceneUserManager::getMe().removeUser(pUser);
      Xlogger->info("用户%s(%ld)因卸载场景注销",pUser->name,pUser->id);
      OnQuit event(1);
      EventTable::instance().execute(*pUser,event);
      execute_script_event(pUser,"quit");
  ...
  struct EverySceneUserAction : public execEntry<SceneUser>
  {
    ...
    bool exec(SceneUser *su)
    {
      if (su->scene && su->charbase.country == ptCmd->dwFromRelationID)
      {
        if (ptCmd->dwStatus == 1)
        {
          Xlogger->debug("当前对战记录数: %u",su->warSize());
          Xlogger->debug("加入国战: toRelation:%u,isAtt:%u",ptCmd->dwToRelationID,ptCmd->isAtt);
        ...
        {
          Xlogger->debug("当前对战记录数: %u",su->warSize());
          Xlogger->debug("删除国战: toRelation:%u,isAtt:%u",ptCmd->dwToRelationID,ptCmd->isAtt);
...
bool SceneRecycleUserManager::canReg(DWORD id)
{
  ...
  {
    Xlogger->debug("等待回收时再次登陆:%s",ret->name);
    rwlock.wrlock();
