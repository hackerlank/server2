...
bool SceneEntryPk::AttackMe(SceneEntryPk *pAtt,const Cmd::stAttackMagicUserCmd *rev,bool physics,SWORD rangDamageBonus)
{
...
#ifdef _DEBUG
		Xlogger->debug("攻击者[%s]---防御者[%s]----------------------------------------------------------",pAtt->name,this->name);
		Xlogger->debug("物理攻击数值  =%d",(int)pAtt->pkValue.pdamage);
		Xlogger->debug("物理防御数值  =%d",(this->pdeftozero?0:this->pkValue.pdefence));
		Xlogger->debug("物理伤害值为dwDam=%d",dwDam);
#endif
	}
	else
	{
...
#ifdef _DEBUG
		Xlogger->debug("攻击者[%s]---防御者[%s]----------------------------------------------------------",pAtt->name,this->name);
		Xlogger->debug("法术攻击数值  =%d",(int)pAtt->pkValue.mdamage);
		Xlogger->debug("法术防御数值  =%d",(this->mdeftozero?0:this->pkValue.mdefence));
		Xlogger->debug("法术伤害值为%d",dwDam);
#endif
	}
...
#ifdef _DEBUG
	Xlogger->debug("根据技能增加伤害值计算出来的结果累加值dwDamDef:%ld",dwDamDef);
#endif
	dwDamDef += (int)(dwDam * ((this->skillValue.dvaluep>100?this->skillValue.dvaluep-100:0)/100.0f));
#ifdef _DEBUG
	Xlogger->debug("根据技能增加伤害率计算出来的结果累加值dwDamDef:%ld",dwDamDef);
#endif
...
#ifdef _DEBUG
	Xlogger->debug("中间值dwDamDef:%d",dwDamDef);
#endif
...
#ifdef _DEBUG
			Xlogger->debug("根据随机转移伤害率扣除后计算结果值dwDamDef:%d",dwDamDef);
#endif
		}
		//--------------------------------------------
		this->hp2mp(dwDamDef);
...
#ifdef _DEBUG
	Xlogger->debug("最终值dwDamDef:%ld",dwDamDef);
#endif  
...
#ifdef _DEBUG
	Xlogger->debug("技能加额外伤害appenddam:%d 猎手印记额外伤害%d",this->skillValue.appenddam,this->skillValue.brappenddam);
#endif
	...
	if (!this->processDeath(pAtt))
	{
		...
		*
		if (0==curTargetID)
		{
		//Xlogger->debug("%s 被 %s 攻击,进入战斗",name,pAtt->name);
...
bool SceneEntryPk::checkMagicFlyRoute(zSceneEntry *pTarget,BYTE aType)
{
	...
	case 0: // 近身
		{
			...
			{
#ifdef  _DEBUG 
				Xlogger->debug("超出攻击范围(%s(%ld) x间距=%u y间距=%u)",this->name,this->id,abs((long)(this->pos.x - pTarget->getPos().x)),abs((long)(this->pos.y - pTarget->getPos().y)));
				...
				{
#ifdef  _DEBUG 
					Xlogger->debug("超出攻击范围(%s(%ld) oldx间距=%u oldy间距=%u)",this->name,this->id,abs((long)(this->pos.x - pTarget->getOldPos1().x)),abs((long)(this->pos.y - pTarget->getOldPos1().y)));
					Channel::sendSys(tempid,Cmd::INFO_TYPE_GAME,"穿越方式：近身");
#endif
					if (abs((long)(this->pos.x - pTarget->getOldPos2().x)) > 1 || abs((long)(this->pos.y - pTarget->getOldPos2().y)) > 1)
					{
#ifdef  _DEBUG 
						Xlogger->debug("超出攻击范围(%s(%ld) oldx间距=%u oldy间距=%u)",this->name,this->id,abs((long)(this->pos.x - pTarget->getOldPos2().x)),abs((long)(this->pos.y - pTarget->getOldPos2().y)));
						Channel::sendSys(tempid,Cmd::INFO_TYPE_GAME,"穿越方式：近身");
#endif
						return false;
					}
	...
	default:
		{
			Xlogger->debug("(%s,%ld)缺少攻击类型",this->name,this->tempid);
#ifdef  _DEBUG 
...
bool SceneEntryPk::setCurTarget(DWORD tempid,DWORD type,bool force)
{
	...
	setEndBattleTime(SceneTimeTick::currentTime,10*1000);
#ifdef _DEBUG
	//Xlogger->debug("%s 设置攻击对象 %u",name,tempid);
#endif
	return true;
}
...
bool SceneEntryPk::setCurTarget(SceneEntryPk * target,bool force)
{
	...
	setEndBattleTime(SceneTimeTick::currentTime,10*1000);
	//Xlogger->debug("%s 设置攻击对象 %s",name,target->name);
	return true;
}
...
bool SceneEntryPk::setDefTarget(DWORD tempid,DWORD type,bool force)
{
	...
	setEndBattleTime(SceneTimeTick::currentTime,10*1000);
#ifdef _DEBUG
	//Xlogger->debug("%s 设置防御对象 %u",name,tempid);
#endif
	return true;
}
...
bool SceneEntryPk::setDefTarget(SceneEntryPk * target,bool force)
{
	...
	setEndBattleTime(SceneTimeTick::currentTime,10*1000);
#ifdef _DEBUG
	//Xlogger->debug("%s 设置防御对象 %s",name,target->name);
#endif
	return true;
}
