...
{
  Xlogger->debug("EventTable::init %s,%s",type.c_str(),file.c_str());
...
bool QuestTable::init()
{
  ...
  if (!xml.initFile(Seal::global["questdir"] + "quests.xml"))
  {
    Xlogger->error("加载任务文件 %s 失败",Seal::global["questdir"].c_str());
    return false;
  }
    ...
    while (node) {
      ...
      if (0 == strcmp((char *)node->name,"quest")) {
        ...
        if (!xml.getNodePropNum(node,"id",&quest._id,sizeof(quest._id))) {
          Xlogger->error("加载任务文件 %squests.xml 失败，任务缺少id",Seal::global["questdir"].c_str());
          return false;
        }
...
/*
          if (0 == strcmp((char *)d_e_node->name,"title")) {
            ...
            Xlogger->debug("任务(%d),%s",quest._id,quest._title.c_str());
          }
*/
          if (0 == strcmp((char *)d_e_node->name,"description")) {
            ...
            Xlogger->debug("任务(%d),%s",quest._id,quest._description.c_str());
          }
...
Xlogger->debug("EventTable::instance().init");
        ...
        Xlogger->warn("QuestTable::init skip %s",node->name);
      }
      node = xml.getNextNode(node,NULL);
    }
    
    Xlogger->info("初始化任务系统成功");
    return true;
  }
