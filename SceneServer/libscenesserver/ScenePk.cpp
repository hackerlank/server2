...
bool SceneUser::attackMagic(const Cmd::stAttackMagicUserCmd *rev,const DWORD cmdLen)
{
  ...
  if (Cmd::ATTACKTYPE_U2U==rev->byAttackType)
  {
	  ...
	  if(!def)
	  {
		  Xlogger->error("%s 用户攻击拉一个无效目标!", this->name);
		  return false;
	  }
  }
		  ...
		  case Cmd::ATTACKTYPE_U2U:
			  {
				  ...
				  else
				  {
					  ...
					  else
					  {
						  ScenePk::attackUserCmdToNine(rev,this);
						  ScenePk::attackFailToMe(rev,this);
						  //Xlogger->debug("错误的dwDefenceTempID.(%ld,%ld)",rev->dwUserTempID,rev->dwDefenceTempID);
					  }
				  }
		  }  
		  break;
		  ...
		  case Cmd::ATTACKTYPE_U2N:
			  {
				  ...
				  {
					  ScenePk::attackFailToMe(rev,this);
					  //Xlogger->debug("错误的dwDefenceTempID.(AID=%ld,DID=%ld)",rev->dwUserTempID,rev->dwDefenceTempID);
				  }
			  }
			  break;
	...
	default:
		{
			...
			{
				ScenePk::attackFailToMe(rev,this);
				Xlogger->debug("(%s,%ld)使用自己没有练的技能(%ld)",this->name,this->id,rev->wdMagicType);
				return true;
			}
		}
  }
...
//  }
  ...
  if (npcdareflag)
  {
    Xlogger->info("[家族争夺NPC]家族[%u]中的角色%s战死",this->charbase.septid,this->name);
  }
...
bool SceneUser::switchPKMode(const Cmd::stPKModeUserCmd *rev)
{
...

  {
    ...
    //Xlogger->debug("switchPkMode=%u",this->pkMode);
...
bool SceneUser::checkGoodness()
{
  ...
  if ((charbase.goodness & 0x0000FFFF) != Cmd::GOODNESS_2_1)
  {
    if (ctv > pkState.tGood)
    {
            ...
            case Cmd::GOODNESS_6:
              {
                ...
                //Xlogger->debug("%s(%d)pk状态改变(%d-->%d)",this->name,this->id,temp,this->charbase.goodness);
                //this->reSendMyMapData();
      ...
      //Xlogger->debug("当前善恶度(%s(%ld),%X)",this->name,this->id,charbase.goodness);
      pkState.tGood = ctv;
  ...
  if (charbase.goodness & 0x00FF0000)
  {
    if (ctv > pkState.tProtect)
    {
      if (pkState.cancelProtect(this))
      {
        ...
        //Xlogger->debug("取消自卫状态(%s(%ld),%X)",this->name,this->id,this->charbase.goodness);
      }
...
bool ScenePkState::cancelProtect(SceneUser *pThis,DWORD time)
{
  ...
  //Xlogger->debug("%s(%d)灰名时间:%d",pThis->name,pThis->id,protect_time);
  if (!protect_time)
  {
...
#define bitMask  0x00FF0000
        ...
        case Cmd::GOODNESS_2_1: // 普通
          {
            if (pDef->getGoodnessState() <= 60)
            {
              ...
              Xlogger->debug("%s(%d)pk状态改变(%d-->%d)",pAtt->name,pAtt->id,temp,pAtt->charbase.goodness);
            }
            else//杀红名
            {
              pDef->lastKiller = 0;
        ...
        case Cmd::GOODNESS_0://英雄
          {
            ...
            {
              Xlogger->debug("%s(%d)pk状态改变(%d-->%d)",pAtt->name,pAtt->id,pAtt->charbase.goodness,(DWORD)Cmd::GOODNESS_3 + pDef->getPkAddition());
              pAtt->charbase.goodness = (DWORD)Cmd::GOODNESS_3 + pDef->getPkAddition();
            }
          }
          break;
  ...
  if (attGood>=1500 && attGood<=(SDWORD)MAX_GOODNESS && pAtt->scene->getRealMapID()!=189)
  {
    ...
    if (s)
    {
      ...
      if (!suc)
        Xlogger->error("%s PK值 %u,送往监狱失败,目的 %s (%d,%d)",pAtt->name,pAtt->charbase.goodness,s->name,100,100);
      else
        Xlogger->error("%s PK值 %u,送往监狱",pAtt->name,pAtt->charbase.goodness);
    }
    else
    {
...
#ifdef _DEBUG
  Xlogger->debug("%s 被 %s 杀死,凶手tempid",pDef->name,pAtt->name,pDef->lastKiller);
#endif
  return true;
}
...
{
  if (Seal::global["pet_point"]!="true") return;
  DWORD hour = 0;
  switch (charbase.level)
  {
    ...
    case 120:
      ...
      if (cartoonList.begin()->second.state==Cmd::CARTOON_STATE_PUTAWAY && !cartoon)
      {
        ...
        Xlogger->info("[宠物]%s %u级,其宠物%s(%u)获得 %u 秒修炼时间",name,charbase.level,cartoonList.begin()->second.name,cartoonList.begin()->first,hour*3600);
      }
      break;
    default:
      return;
...
bool SceneUser::upgrade(DWORD num)
{
  ...
  if (num > 0)
  {
    while(num--)
    {
      ...
      if (30==charbase.level && (atoi(Seal::global["service_flag"].c_str())&Cmd::Session::SERVICE_HORSE))//30级送枣红马
      {
          horse.horse(3000);
  ...
  Xlogger->info("用户(%s(%ld))升到%ld级",this->name,this->id,charbase.level);
...
{
#ifdef _DEBUG
  //Xlogger->debug("%s 攻击失败",pAtt->name);
#endif
...
#ifdef _DEBUG
  Xlogger->info("[发送经验增加通知]获得经验：%u 用户当前经验：%u",ret.dwExp,ret.dwUserExp);
#endif
  pUser->sendCmdToMe(&ret,sizeof(ret));
}
*/
...
bool ScenePk::checkAttackSpeed(SceneUser *pAtt,const Cmd::stAttackMagicUserCmd *rev)
{
  ...
  switch(rev->wdMagicType)
  {
	...
	case SERVER_SKILL_HANDS_ATTACK_NORMAL:	//双手武器普通攻击
    {
      ...
      {
        //Xlogger->info("攻击速度过快(%s(%ld),%ld)",pAtt->name,pAtt->id,rev->byAction);
        return false;
      }
    }
	case SERVER_SKILL_DART_ATTACK_NORMAL:	//飞镖武器普通攻击:
    ...
    default:
    {
      ...
      //除物理攻击之外的所有攻击视作法术攻击
...
#endif
				//Xlogger->info("技能攻击速度过快或处于冷却期间(%s(%ld),%ld)",pAtt->name,pAtt->id,rev->byAction);
				return false;
			}
		...
		{
			Xlogger->info("外挂:试图使用没有学习的技能(%s(%ld),%ld) 技能id = %ld",pAtt->name,pAtt->id,rev->byAction,rev->wdMagicType);
			return false;
		}
	  }
	  else
...
#ifdef _DEBUG
		  Xlogger->info("1-----------------------------------------------------------");
		  Xlogger->info("pkpreValue.fivedam=%u,pkpreValue.fivemaxdam=%u",pAtt->pkpreValue.fivedam,pAtt->pkpreValue.fivemaxdam);
		  Xlogger->info("预处理pkValue.pdamage=[%u] percent=[%f]",pAtt->pkValue.pdamage,percent);
#endif
		  checkholyp(percent);
#ifdef _DEBUG
		  Xlogger->info("爆击处理预处理pkValue.pdamage=[%u] 物理爆[%s] 魔法爆[%s]",pAtt->pkValue.pdamage,pAtt->isPhysicBang?"爆":"否",pAtt->isMagicBang?"爆":"否");
#endif
		  //pAtt->pkValue.pdamage= (DWORD)(pAtt->pkValue.pdamage*(1.0f+pAtt->packs.equip.getEquips().getAttFivePoint()/100.0f));
...
#ifdef _DEBUG
              Xlogger->info("1-----------------------------------------------------------");
          Xlogger->info("pkpreValue.nofivedam=%u,pkpreValue.nofivemaxdam=%u",pAtt->pkpreValue.nofivedam,pAtt->pkpreValue.nofivemaxdam);
              Xlogger->info("预处理pkValue.pdamage=[%u] percent=[%f]",pAtt->pkValue.pdamage,percent);
#endif
        checkholyp(percent);
#ifdef _DEBUG
...
#ifdef _DEBUG
              Xlogger->info("1-----------------------------------------------------------");
              Xlogger->info("预处理pkValue.mdamage=[%u]",pAtt->pkValue.mdamage);
#endif
        checkholym(percent);
#ifdef _DEBUG
              Xlogger->info("爆击处理预处理pkValue.mdamage=[%u] 物理爆[%s] 魔法爆[%s]",pAtt->pkValue.mdamage,pAtt->isPhysicBang?"爆":"否",pAtt->isMagicBang?"爆":"否");
#endif
...
#ifdef _DEBUG
              Xlogger->info("2------------------------------------------------[%u]-----------",pDef->npc->five);
              Xlogger->info("预处理pkValue.mdamage=[%u]",pAtt->pkValue.mdamage);
#endif
        checkholym(percent);
#ifdef _DEBUG
...
bool SceneUser::reliveReady(const Cmd::stOKReliveUserCmd *rev,bool isOrigin)
{
  ...
  if (!isOrigin)
  {
     ...
     && this->scene->getRealMapID() == 139)
    {
      ...
      {
        Xlogger->info("查找重生点成功：%s,%u,%u",pToScene->name,newPos.x,newPos.y);
        mapchange = false;
      }
    ...
    if (!finddare)
    {
      if (this->charbase.country == this->scene->getCountryID())
      {
        ...
        {
          Xlogger->info("查找重生点成功：%s,%u,%u",this->scene->name,newPos.x,newPos.y);
          mapchange = false;
        }
        else
        {
          ...
          Xlogger->info("切换地图重生%s,%u,%u",this->scene->name,newPos.x,newPos.y);
        }
      }
      else
      {
        ...
        mapchange = (retcode!=0);
        changeService = (2==retcode);
        Xlogger->info("切换国外地图重生%s,%u,%u",this->scene->name,newPos.x,newPos.y);
      }
      if (!mapchange)
      {
        zPos findedPos;
...
//  skillStatusM.clearActiveSkillStatus();
  ...
  //Xlogger->debug("复活状态(%s(%ld),%X) ",this->name,this->id,this->charbase.goodness);
...
bool SceneUser::relive(const int relive_type,const int delaytime,const int data_percent)
{
	...
	if (npcdareflag)
	{
		Xlogger->info("[家族争夺NPC]家族[%u]中的角色%s被技能复活重新获得对战资格",this->charbase.septid,this->name);
	}
		...
		if (pAtt && pAtt->scene->getRealMapID()!=189 && pAtt->scene->getRealMapID()!=203)
		{
			...
			if ((packs.checkMoney(cost) && packs.removeMoney(cost,"报案")))
			{
				...
				if (s)
				{
					bool suc = pAtt->changeMap(s,zPos(80,70));
					if (!suc)
						Xlogger->error("%s 杀死 %s,送往监牢失败,目的 %s (%d,%d) 时间 %u 分钟",pAtt->name,name,s->name,80,70,punishTime);
					else
						Xlogger->error("%s 杀死 %s,送往监牢,时间 %u 分钟",pAtt->name,name,punishTime);
				}
				else
				{
					//if (pAtt->guard && pAtt->guard->canMove()) pAtt->saveGuard = true;//使镖车跟随指令使用者
