...
bool SceneNpc::doChangeMapAI()
{
  ...
  if (!s)
  {
    Xlogger->info("doChangeMapAI:npc跳转地图失败,未找到地图 %s",mapName.c_str());
    return false;
  }
...
#ifdef _DEBUG
      Xlogger->debug("定时NPC死亡timeCount=%u 过期时间[%u]当前时间[%u]",dwStandTimeCount,dwStandTime,SceneTimeTick::currentTime.sec());
#endif
      return true;
    }
#ifdef _DEBUG
    Xlogger->debug("定时NPC持续timeCount=%u 过期时间[%u]当前时间[%u]",dwStandTimeCount,dwStandTime,SceneTimeTick::currentTime.sec());
#endif
  }
    ...
    case NPC_TYPE_TRAP:        /// 陷阱类型
      {
        if (canFight())
        {
          ...
          if (enemy)
          {
            ...
            {
              Xlogger->error("[怪物AI]NPC[%s]的技能没有加载无法使用！",this->name);
            }
          }
        }
      }
...
bool SceneNpc::canRush()
{
  ...
  {
    Xlogger->info("%s 怪物攻城的几率 %d%%",name,define->rushRate);
...
bool NpcAIController::loadScript(DWORD id)
{
  ...
  zXMLParser xml;
  if (!xml.initFile(Seal::global["confdir"] + "NpcAIScript.xml"))
  {
    Xlogger->error("打开NpcAIScript.xml失败");
    return false;
  }
  ...
  while (scriptNode)
  {
    ...
    if (num==id)
    {
      ...
      while (phaseNode)
      {
        ...
        //Xlogger->debug("loadScript:%s",ad.str);
      ...
      if (!phaseVector.empty())
      {
        ...
        nextPhase(0);
        //Xlogger->info("[AI]%s 读取NPC智能脚本 id=%d phase=%d",npc->name,id,phaseVector.size());
        return true;
      }
      else
      {
        Xlogger->info("读取空智能脚本 id=%d",id);
        return false;
      }
    }
...
SceneNpcAIType NpcAIController::parseAction(char * action)
{
  ...
  Xlogger->error("parseAction : 未知的脚本动作 %s",action);
...
void NpcAIController::setAI(const t_NpcAIDefine ai,const bool setTime)
{
  ...
  //Xlogger->debug("setAI(): %s AI=%d",npc->name,ai.type);
}
...
bool SceneNpc::doDropItemAI()
{
  ...
  if (!ob)
  {
    Xlogger->debug("%s 找不到要丢的物品 id=%u num=%u",name,AIDefine.pos.x,AIDefine.pos.y);
    return false;
  }
...
bool SceneNpc::moveToEnemy(SceneEntryPk * enemy)
{
  ...
  if (checkMoveTime(SceneTimeTick::currentTime) && canMove())
  {
    if (aif&AIF_WARP_MOVE)
      ...
      {
#ifdef _DEBUG
        //Xlogger->debug("%s 寻路失败 closeCount=%u",name,closeCount);
#endif
...
bool SceneNpc::getEntries(int radius,SceneEntryPk_vec & entries,int state)
{
  ...
  //  Xlogger->debug("%s getEntries",name);
#endif
