...
void Rush::process()
{
  ...
  else
  {
    ...
    if (t>=900 && t<=3600 && t%900<2)
    {
      ...
      Xlogger->debug(endText.c_str(),tex);
    }
  }
  ...
  if (boss)
  {
    ...
    if (zSceneEntry::SceneEntry_Death==boss->getState())
    {
      if (curPhase != phaseVector.size()-1)
      {
        ...
        Xlogger->debug("boss死亡,阶段%d",curPhase);
      }
    }
  }
    ...
    case RUSH_RELIVE:
      {
        ...
        {  
          ...
          Xlogger->debug(startText.c_str(),tex);
          rushDelay -= 900;
        }
      }
      break;
...
bool Rush::summonServant()
{
  ...
  for (DWORD index=0; index<servantVector.size(); index++)
  {
    ...
    if (NULL == base)
    {
      Xlogger->debug("召唤 %s 的仆人时,未找到该怪物 id=%d",boss->name,servantVector[index].id);
      return false;
    }
    ...
    Xlogger->debug("%s summonServant:summon %u 次,servant %u 个",rushName,count,servants.size());
  }
...
void Rush::deleteBoss()
{
  if (boss)
  {
    ...
    */
    ...
    boss = 0;
    Xlogger->debug("%s : 删除boss",rushName);
  }
  else
    Xlogger->debug("%s : boss已经删除",rushName);
}
...
void Rush::clearServants()
{
  ...
  Xlogger->debug("%s : 删除 %u 个servants",rushName,servants.size());
  servants.clear();
}
