...
{
  Xlogger->debug("doCountryCmd receive byParam:[%d]",rev->byParam);
    ...
    case Cmd::CONTRIBUTE_COUNTRY_MATERIAL:
      {
        ...
        if (cmd->itemID && cmd->itemID != 0xffffffff)//INVALID_THISID
        {
          ...
          if (!srcobj)
          {
            Xlogger->info("%s 国家捐献时未找到该物品 id=%u",
          ...
          if (send.dwValue>0)
          {
            ...
            Xlogger->info("[国家捐献]: 当前功勋:%d 本次获得功勋:%d 当前荣誉值:%d 本次获得荣誉值:%d",
            ...
            BUFFER_CMD(Cmd::stAddUserAndPosMapScreenStateUserCmd,send2,x_socket::MAX_USERDATASIZE);
