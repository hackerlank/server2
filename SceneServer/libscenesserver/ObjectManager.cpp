...
bool GlobalObjectIndex::addObject(zObject * o)
{
  ...
  if (o)
  {
    ...
    if (ret)//物品重复处理
    {
      ...
      {
        bret=true;
        Xlogger->debug("全局物品管理器中发现有重复添加物品(%s,%d)",ret->data.strName,ret->data.qwThisID);
      }
      else if (ret->createtime==o->createtime && ret->data.dwObjectID==o->data.dwObjectID && ret->data.qwThisID == o->data.qwThisID)
      {
        Xlogger->warn("复制物品:%s 创建时间:%I64u",ret->name,ret->createid);
        ret=false;
      }
      else
      {
        Xlogger->debug("id冲突%s(%d)",ret->name,ret->id);
        do {
          //重新生成ID
          o->generateThisID();
          bret = addEntry((zEntry *)o);
        } while (!bret);
        //if (!bret) Xlogger->fatal("添加物品到物品表失败1");
      }
    }
    else
    {
      bret=addEntry((zEntry *)o);
      if (!bret) Xlogger->fatal("添加物品到物品表失败");
    }
    mlock.unlock();
  }else {
    Xlogger->fatal("添加非法物品");
  }
...
//  Obj_vec a,b;
  ...
  for(hashmap::iterator it=ets.begin();it!=ets.end();it++)
  {
    ...
    //      Xlogger->debug("物品%s(%d,%d,%d,%d)",tmp->name,tmp->data.pos.dwLocation,tmp->data.pos.dwTableID,tmp->data.pos.x,tmp->data.pos.y);
    if (tmp->data.pos.tab()) {
      /*
...
//    Xlogger->debug("物品%s(%d,%d,%d)",(*it)->name,(*it)->data.pos.dwLocation,(*it)->data.pos.dwTableID,(*it)->data.pos.x,(*it)->data.pos.y);    
    if (!exec.exec(*it))
      return;
  }
/*
...
int UserObjectM::space(const SceneUser* user) const
{
  ...
  for(hashmap::const_iterator it=ets.begin();it!=ets.end();it++) {
    ...
    //Xlogger->debug("space: %s,%d",ob->name,ob->data.pos.dwLocation);
    --free;
  }
#endif
