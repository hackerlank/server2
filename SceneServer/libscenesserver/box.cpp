	...
	case HORSETRAINING_USERCMD_TEY:
		{
			...
			case 4:
				{
					DWORD Ret	= atoi(Seal::global["ExploitRate"].c_str());
					DWORD dwExp;
					if( Ret > 0 )
					{
	...
	case SURPLUS_ATTRIBUTE_USERCMD_ADD:
		{
			...
			if( obj != NULL )
			{
				...
				{
					Xlogger->error("玩家 %s 想分配的自由点数大过装备所剩余的自由点数！分配失败！！\n", this->name );
					return true;
				}
	...
	case RT_MAKE_TURRET_USERCMD:	//sky 这个是制造炮台消息和上面的分配消息已经完全没关系拉^_^
		{
			...
			{
				Xlogger->debug("建造:物品已经不存在或者物品类型不是建筑类物品,用户:%s 物品ID:%u", name, pCmd->dwThisID);
				return true;
			}
			...
			if(base)
			{
				...
				if(!scene->checkBlock( pos ) && !scene->checkBlock(pos, TILE_NOCREATE) )
				{
					...
					if(npc)
					{
						...
						Xlogger->debug("用户:%s 企图建造一个不存在的NPC！", name);
				}
				else
	...
	case RT_NPC_START_CHANGE_USERCMD:	//sky  这个又变成NPC变身的处理拉，我承认我懒的加父消息拉otz
		{
			...
			if(sceneNpc)
			{
				if(!sceneNpc->ChangeNpc())
					Xlogger->debug("NPC %s 变身失败!",sceneNpc->define->name);
			}
			else
				Xlogger->debug("无法找到唯一ID为 %u 的NPC",pCmd->npcid);
		}
		break;
	case PACK_BUYTAB_NUM_USERCMD:	//sky 购买包袱页的处理,反正都已经做拉怎么多无关的消息拉,再加一个也没关系吧 ^_^
		{
			...
			if(pCmd->PackType == PACKE_TYPE)
			{
				...
				default:
					{
						Xlogger->error("玩家:%s 购买包袱的时候 发现错误的可用页数量%d", name, packs.main.TabNum);
						return true;
					}
					break;
				}
			...
			else if(pCmd->PackType == SAVEBOX_TYPE)
			{
				...
				default:
					{
						Xlogger->error("玩家:%s 购买仓库的时候 发现错误的可用页数量%d", name, packs.main.TabNum);
						return true;
					}
					break;
				}
