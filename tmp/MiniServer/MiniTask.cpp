...
bool MiniTask::cmdMsgParse(const Cmd::t_NullCmd *pNullCmd,const DWORD nCmdLen)
{
  ...
  Xlogger->error("MiniTask::cmdMsgParse(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
  return false;
}
    ...
    case PARA_GATE_USER_LOGIN:
      {
        ...
        if (!user)
        {
          Xlogger->error("用户登录失败！id=%u name=%s",rev->userID,rev->name);
          return false;
        }
    ...
    case PARA_GATE_USER_DELETE:
      {
        ...
        if (fs)
        {
          ...
          {   
            Xlogger->error("newUser()不能获取数据库句柄");
            return false;
          }
          ...
          if (MiniService::dbConnPool->exeDelete(handle,fs,&where))
            Xlogger->info("玩家删号,清除得分记录 id=%u",rev->userID);
    ...
    case PARA_SCENE_SET_SCENE:
      {
        ...
        Xlogger->info("%s(%u) 切换场景服务器 %u",u->name,u->id,rev->serverID);
    ...
    case PARA_SCENE_DEPOSIT:
      {
        ...
        if (!u)
        {
          ...
          {
            Xlogger->info("玩家 userID=%u 直接写数据库充值 money=%u 成功",rev->userID,rev->num);
            return true;
          }
          else
            return false;
        ...
        Xlogger->info("%s(%u) 充值 %u,现有 %u",u->name,u->id,rev->num,u->getMoney());
      }
      break;
    case PARA_SCENE_CHECK_DRAW:
      {
        ...
        if (u->save())
          Xlogger->info("%s(%u) 兑换银两 %u,现有 %u",u->name,u->id,rev->num,u->getMoney());
      }
      break;
    case PARA_SCENE_DRAW_RET:
      {
        ...
        if (rev->ret==1)
        {
          ...
          send.ret = 1;//成功
          u->sendCmdToMe(&send,sizeof(send));
          Xlogger->info("兑换银两完成 userID=%u rev->num=%u",rev->userID,rev->num);
          return true;
        }
        else
        {
          ...
          {
            u->addMoney(rev->num);
            Xlogger->info("%s(%u) 兑换仙丹失败,补偿成功 num=%u 现有 %u",u->name,u->id,rev->num,u->getMoney());
            return true;
          }
          else
          {
            if (addDBMoney(rev->userID,rev->num))
            {
              Xlogger->info("兑换仙丹失败,补偿成功 userID=%u money=%u ret=%u",rev->userID,rev->num,rev->ret);
              return true;
            }
            else
            {
              Xlogger->error("兑换仙丹失败,补偿也失败 userID=%u money=%u ret=%u",rev->userID,rev->num,rev->ret);
              stDrawRetCommonMiniGameCmd send;
              send.ret = 0;//失败
              u->sendCmdToMe(&send,sizeof(send));
              return true;
...
bool MiniTask::addDBMoney(DWORD userID,DWORD num)
{
  ...
  if (fs)
  {
    ...
    if ((connHandleID)-1 == handle)
    {   
      Xlogger->error("addDBMoney()不能获取数据库句柄");
      return false;
    }   
    ...
    if (recordset)
    {
      ...
      {
        Xlogger->error("往数据库增加金钱时写数据库失败! userID=%u money=%u",userID,num);
        return false;
      }
      ...
      Xlogger->error("往数据库增加金钱时没找到记录 userID=%u money=%u",userID,num);
