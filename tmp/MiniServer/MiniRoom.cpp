...
void MiniRoom::userLeave(MiniUser *u)
{
  ...
  if (MUS_ROOM==u->getGameState(gameType()))
  {
    ...
    */
    ...
    Xlogger->debug("%s(%u) 离开房间 %u",u->name,u->id,id.id());
  }
}
...
bool MiniRoom::userEnter(MiniUser *u)
{
  ...
  Xlogger->error("%s(%u) 进入房间 roomID=%u",u->name,u->id,id.roomID());
  return true;
}
...
void MiniRoom::sendRoomToUser(MiniUser *u)
{
  ...
  for (it=userList.begin(); it!=userList.end(); it++)
  {
    ...
    cmd->num++;
    //Xlogger->debug("sendRoomToUser 给 %s(%u) 发送玩家信息 %s(%u)",u->name,u->id,(*it)->name,(*it)->id);
  }
  if (cmd->num)
  ...
  if (send->num)
  {
    ...
    //Xlogger->debug("sendRoomToUser 给 %s(%u) 发送 %u 个座位信息",u->name,u->id,send->num);
  }
}
...
void MiniRoom::sendUserToRoom(MiniUser *u)
{
  ...
  //Xlogger->debug("sendUserToRoom 广播 %s(%u) 的信息",u->name,u->id);
...
void MiniRoom::sendUserStateToRoom(MiniUser *u)
{
  ...
  //Xlogger->debug("sendUserStateToRoom %s(%u) state=%u pos=%u",u->name,u->id,send.state,send.pos.id());
