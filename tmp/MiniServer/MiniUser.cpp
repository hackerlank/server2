...
void MiniUser::setScene(MiniTask *s)
{
  if (s)
    ...
    Xlogger->error("MiniUser::setScene %s(%u)试图切换不存在的场景",name,id);
}
...
{
  gameList[t].pos = p;
  Xlogger->debug("%s(%u) 位置 %u",name,id,p.id());
}
...
bool MiniUser::save()
{
  ...
  {
    Xlogger->error("MiniUser::save 不能获取数据库句柄");
    return false;
  }
  ...
  if ((DWORD)-1==ret)
  {
    Xlogger->error("用户保存,写数据库失败! %s(%u) score=%u money=%u ret=%d",name,id,score.score,score.money,ret);
    return false;
  }
