...
void DDZCardGame::deal()
{
  ...
  for (BYTE i=1; i<=maxUserNum; i++)
  {
    ...
    userCount++;
    Xlogger->debug("%u 发牌 %s(%u):\t%u %u %u %u %u %u %u %u %u %u %u %u %u %u %u %u %u",id.id(),seatList[i].user->name,cmd->num,cmd->cards[0].number(),cmd->cards[1].number(),cmd->cards[2].number(),cmd->cards[3].number(),cmd->cards[4].number(),cmd->cards[5].number(),cmd->cards[6].number(),cmd->cards[7].number(),cmd->cards[8].number(),cmd->cards[9].number(),cmd->cards[10].number(),cmd->cards[11].number(),cmd->cards[12].number(),cmd->cards[13].number(),cmd->cards[14].number(),cmd->cards[15].number(),cmd->cards[16].number());
  }
}
    ...
    case PUT_CARD_MINI_PARA:
      {
        ...
        //else
        //  Xlogger->debug("%s 无法出牌 %u",u->name,makeSeatID(seat).id());
      }
      break;
    case POINT_DDZ_MINI_PARA:
      {
        ...
        if (rev->num)
        {
          ...
          Xlogger->debug("%s 叫 %u 分 seatID=%u",u->name,rev->num,makeSeatID(curPointSeat).id());
        }
        else
          Xlogger->debug("%s 不叫分 seatID=%u",u->name,makeSeatID(curPointSeat).id());
        ...
        if (rev->num==3 || (newSeat==lastPointSeat))//叫牌完毕
        {
          ...
          else
          {
            ...
            Xlogger->debug("%s 成为地主 seatID=%u",seatList[lordSeat].user->name,makeSeatID(lordSeat).id());
          }
        }
        else
        {
...
void DDZCardGame::putCards(BYTE seat,Cmd::stPutCardMiniGameCmd *cmd,DWORD len)
{
  if (cmd->num)
  {
    ...
    if (seat==lordSeat)
      ...
      otherPutTime++;
    //Xlogger->debug("%s(%u) 出牌 seatID=%u",seatList[seat].user->name,seatList[seat].user->id,makeSeatID(seat).id());
  }
  //else
  //  Xlogger->debug("%s(%u) 不跟 seatID=%u",seatList[seat].user->name,seatList[seat].user->id,makeSeatID(seat).id());
...
void DDZCardGame::calcFleeScore(BYTE seat)
{
  ...
  Xlogger->debug("%s 逃跑\twin:%u lose:%u score:%d",seatList[seat].user->name,s.win,s.lose,s.score);
}
...
void DDZCardGame::calcNormalScore(BYTE seat)
{
  ...
  send.score = s;
  sendCmdToAll(&send,sizeof(send));
  Xlogger->debug("%s\t地主得分\twin:%u lose:%u score:%d",seatList[lordSeat].user->name,s.win,s.lose,s.score);
  ...
  for(BYTE i=1; i<=maxUserNum; i++)
  {
    if (i!=lordSeat && seatList[i].user)
    {
      ...
      Xlogger->debug("%s\t农民得分\twin:%u lose:%u score:%d",seatList[i].user->name,s.win,s.lose,s.score);
    }
  }
}
