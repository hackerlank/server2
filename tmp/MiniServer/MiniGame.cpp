...
bool MiniGame::userEnter(MiniUser *u,Cmd::MiniUserPosition seatID)
{
  ...
  Xlogger->debug("%s(%u) 坐下 seatID=%u",u->name,u->id,seatID.id());
  return true;
}
...
void MiniGame::userLeave(MiniUser *u)
{
  ...
  */
  ...
  Xlogger->debug("%s(%u) 离开桌子 %u",u->name,u->id,makeSeatID(seat).id());
}
...
void MiniGame::sendUserToGame(MiniUser *u)
{
  ...
  Xlogger->debug("sendUserToGame 广播 %s(%u) 的信息",u->name,u->id);
}
...
void MiniGame::sendGameToUser(MiniUser *u)
{
  ...
  if (cmd->num)
  {
    ...
    Xlogger->debug("sendGameToUser 给 %s(%u) 发送 %u 个玩家信息",u->name,u->id,cmd->num);
  }
}
*/
...
void MiniGame::setHost(BYTE seat)
{
  ...
  if (seat>maxUserNum)
  {
    Xlogger->error("gameID=%u 设置房间主人错误 seat=%u",id.id(),seat);
    return;
  }
  ...
  if (hostSeat)
  {
    ...
    Xlogger->debug("清除桌主 gameID=%u seat=%u",id.id(),hostSeat);
  }
  ...
  if (hostSeat)
  {
    ...
    Xlogger->debug("设置桌主 gameID=%u seat=%u",id.id(),seat);
  }
  */
}
...
void MiniGame::toggleSeat(MiniUser *host,Cmd::MiniSeatID seatID)
{
  ...
  Xlogger->debug("%s(%u) 设置座位状态 %u state=%u",host->name,host->id,seatID.id(),seatList[seatID.seat].open);
  */
}
...
void MiniGame::kickUser(MiniUser *host,MiniUser *u)
{
  ...
  Xlogger->debug("%s 被 %s 踢出游戏，进入房间 %u",u->name,host->name,id.roomID());
  */
}
...
bool MiniGame::toggleReady(MiniUser *u)
{
  ...
  */
  ...
  Xlogger->debug("%s(%u) %s %u",u->name,u->id,u->getGameState(id.type)==Cmd::MUS_READY?"准备":"取消准备",makeSeatID(seat).id());
...
bool MiniGame::start()
{
  ...
  Xlogger->debug("%u 游戏开始",id.gameID());
  return true;
}
...
bool MiniGame::end()
{
  ...
  for (BYTE i=1; i<=maxUserNum; i++)
    if (seatList[i].user && !seatList[i].user->checkMoney(money*30))
    {
      ...
      Xlogger->info("%s(%u) 金钱不足被踢出游戏 gameID=%u",seatList[i].user->name,seatList[i].user->id,id.id());
      userLeave(seatList[i].user);
    }

  Xlogger->debug("%02u%02u%02u%02u 游戏结束",id.type,id.room,id.game,id.seat);
  return true;
}
