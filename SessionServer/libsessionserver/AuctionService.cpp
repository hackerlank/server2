    ...
    case PARA_AUCTION_SALE:
      {
        ...
        if (!pUser)
        {
          Xlogger->error("[拍卖]doAuctionCmd(PARA_AUCTION_SALE): 添加拍卖记录时未找到拍卖者 %s",rev->info.owner);
          Xlogger->error("[拍卖]添加拍卖记录时未找到拍卖者 %s 物品: %s",rev->info.owner,rev->item.object.strName);
          return false;
        }
        */
        ...
        if ((connHandleID)-1 == handle)
        {
          ...
          Xlogger->error("[拍卖]添加拍卖记录错误 %s 物品: %s 价格：%u-%u",rev->info.owner,rev->item.object.strName,rev->info.minMoney,rev->info.maxMoney);
        ...
        {       
          ...
          Xlogger->error("[拍卖]:%s 拍卖物品 %s auctionID=%u",rev->info.owner,rev->item.object.strName,retcode);
        }
    ...
    case PARA_AUCTION_CHECK_BID:
      {
        ...
        if (!pUser)
        {
          Xlogger->error("[拍卖]doAuctionCmd(PARA_AUCTION_CHECK_BID): 竞标时未找到竞标者");
          return false;
        }
        ...
        if (1 != retcode)
        {
          ...
          //Xlogger->warn("[拍卖]%s 竞标没有找到记录 auctionID=%u retCode=%d",pUser->name,rev->auctionID,retcode);
          return true;
        }
    ...
    case PARA_AUCTION_BID:
      {
        ...
        if (!pUser)
        {
          Xlogger->error("[拍卖]doAuctionCmd(PARA_AUCTION_CHECK_BID): 竞标时未找到竞标者,丢失%u文",rev->money);
          return false;
        }
        ...
        if ((connHandleID)-1 == handle)
        {
          ...
          Xlogger->error("[拍卖]竞标失败 %s 丢失 money=%u gold=%u",rev->money,rev->gold);
          return false;
        }
        ...
        if (1 != retcode)
        {
          if (MailService::getMe().sendMoneyMail("征途拍卖行",0,pUser->name,pUser->id,rev->money,"竞拍失败返还的银子",(DWORD)handle,MAIL_TYPE_AUCTION,bid.itemID))
            ...
            Xlogger->error("[拍卖]%s 竞标失败返还银子失败 auctionID=%u retCode=%d 丢失 money=%u gold=%u",pUser->name,rev->auctionID,retcode,rev->money,rev->gold);
          SessionService::dbConnPool->putHandle(handle);
          return true;
        }
        ...
        if (1 != retcode)
        {
          ...
          {
            pUser->sendSysChat(Cmd::INFO_TYPE_FAIL,"竞标失败");
            Xlogger->error("[拍卖]%s 竞标时没找到记录,退回邮件失败 auctionID=%u retCode=%d,丢失 money=%u gold=%u",pUser->name,rev->auctionID,retcode,rev->money,rev->gold);
            SessionService::dbConnPool->putHandle(handle);
            return false;
          }
          SessionService::dbConnPool->putHandle(handle);
            ...
            || (1==bid.bidType && bid.maxGold && rev->gold>=bid.maxGold))
        {
          ...
          {
            Xlogger->error("[拍卖]%s 竞价成功发送物品失败 auctionID=%u",pUser->name,rev->auctionID);
            SessionService::dbConnPool->putHandle(handle);
            return false;
          }
        }
    ...
    case PARA_AUCTION_QUERY:
      {
        ...
        if ((DWORD)-1 == retcode)
        {
          Xlogger->error("[拍卖]%s 查询拍卖物品时错误 retCode=%d",pUser->name,retcode);
          return false;
        }
        ...
        if (retcode)
        {
          ...
          for (DWORD i=0; i<retcode && i<10; i++)
          {
            ...
            pUser->sendCmdToMe(&al,sizeof(al));
#ifdef _DEBUG
        Xlogger->debug("[拍卖]查询到 %u 条记录 %s",count,where.c_str());
#endif
          }
        }
      }
    ...
    case PARA_AUCTION_CHECK_CANCEL:
      {
        ...
        if (!pUser)
        {
          Xlogger->error("[拍卖]doAuctionCmd(PARA_AUCTION_CHECK_CANCEL): 取消拍卖检查时未找到拍卖者");
          return false;
        }
    ...
    case PARA_AUCTION_CANCEL:
      {
        ...
        if (!pUser)
        {
          Xlogger->error("[拍卖]doAuctionCmd(PARA_AUCTION_CANCEL): 取消拍卖时未找到拍卖者");
          return false;
        }
            ...
            || bid.state!=AUCTION_STATE_NEW)
        {
          if (MailService::getMe().sendMoneyMail("征途拍卖行",0,pUser->name,pUser->id,rev->charge,"取消拍卖失败返还的费用",(DWORD)handle,MAIL_TYPE_AUCTION,bid.itemID))
            ...
            Xlogger->error("%s 取消拍卖时没找到记录,退回银子失败 auctionID=%u retCode=%d charge=%u",pUser->name,rev->auctionID,retcode,rev->charge);
          SessionService::dbConnPool->putHandle(handle);
          return true;
        }
        ...
        {
          Xlogger->error("%s 取消拍卖发送物品失败 auctionID=%u charge=%u itemID=%u",pUser->name,rev->auctionID,rev->charge,bid.itemID);
          SessionService::dbConnPool->putHandle(handle);
          return false;
        }
        SessionService::dbConnPool->putHandle(handle);
        ...
        Xlogger->info("[拍卖]%s 取消拍卖 %s auctionID=%u itemID=%u",pUser->name,bid.item.object.strName,rev->auctionID,bid.itemID);
      }
      break;
    case PARA_AUCTION_GET_LIST:
      {
        ...
        if (!pUser)
        {
          Xlogger->error("doAuctionCmd(PARA_AUCTION_GET_LIST): 查询时未找到玩家");
          return false;
        }
...
void AuctionService::checkDB()
{
  ...
  if (bidList)
  {
    ...
    for (DWORD i=0; i< retcode; i++)
    {
      ...
      if (sendAuctionItem((DWORD)handle,tempPoint->auctionID,AUCTION_STATE_TIMEOVER,toOnwer))
        Xlogger->info("[拍卖]拍卖时间结束,%s物品 %s auctionID=%u itemID=%u",toOnwer?"退回":"发送",tempPoint->item.object.strName,tempPoint->auctionID,tempPoint->itemID);
      else
        Xlogger->error("[拍卖]时间结束退回物品失败 auctionID=%u itemID=%u",tempPoint->auctionID,tempPoint->itemID);
      tempPoint++;
    }
  ...
  if (retcode) Xlogger->debug("[拍卖]删除 %u 条10天之前的拍卖记录",retcode);
...
bool AuctionService::sendAuctionItem(DWORD h,DWORD auctionID,BYTE newState,bool toOwner)
{
  ...
  if (!toOwner)//卖出,给竞标者
  {
    if (strcmp(bid.bidder,""))
    {
      ...
      Xlogger->info("[拍卖]%s 赢得物品 %s auctionID=%u itemID=%u",bid.bidder,bid.item.object.strName,auctionID,bid.itemID);
    }
    else
    {
      Xlogger->error("[拍卖]sendAuctionItem: 竞标者的名字为空 auctionID=%u,itemID=%u",auctionID,bid.itemID);
      return false;
    }
  }
  else
  {
    ...
    Xlogger->info("[拍卖]%s 取回物品 %s auctionID=%u itemID=%u",bid.owner,bid.item.object.strName,auctionID,bid.itemID);
  }
  ...
  {
    Xlogger->error("[拍卖]sendAuctionItem: 拍卖结束发送物品失败 auctionID=%u itemID=%u",auctionID,bid.itemID);
    return false;
  }
}
...
void AuctionService::delAuctionRecordByName(char * name)
{
  ...
  Xlogger->info("[拍卖]删除角色所有拍卖记录：name=%s",name);
}
