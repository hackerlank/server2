...
bool CArmyM::load()
{
  ...
  {
    Xlogger->error("军队数据加载失败,ARMY表不存在");
    return false;
  }
        ...
        if (rev->byStatus == Cmd::YES_ARMY_GEN)
        {
          ...
          if (pArmy)
          {
            ...
            pUser->sendSysChat(Cmd::INFO_TYPE_EXP,"恭喜您成为 %s 军队将军",
                pArmy->name);
            Xlogger->info("[军队]: %s 军队建立成功",pArmy->name);
...
void CArmyM::processFireCaptain(UserSession* pUser,Cmd::stFireArmyCaptainUserCmd* rev)
{
  ...
  if (pCaptain!=NULL) 
  {
    if (pCaptain->myArmy && pCaptain->myArmy->dwGenID == pUser->id && pUser->id != rev->dwUserID)
    {
      ...
      {
        Xlogger->info("[军队]: %s 离开 %s 军队",pUser->name,pCaptain->myArmy->name);
...
void CArmyM::timer()
{
  ...
  {
    CArmy* temp = *vIter;
    Xlogger->info("[军队]: %s 军队因将军拒绝任命,撤消军队",temp->name);
...
void CArmy::writeDatabase()
{
  ...
  {
    Xlogger->error("军队数据保存失败,ARMY表不存在");
    return;
  }  
}
...
bool CArmy::insertDatabase()
{
  ...
  {
    Xlogger->error("添加军队记录失败,ARMY表不存在");
    return false;
  }
...
bool CArmy::loadCaptainFromDB()
{
  ...
  {
    Xlogger->error("军队详细数据加载失败,CAPTAIN表不存在");
    return false;
  }
  ...
  if (pUser)
  {
    ...
    if (pCaptain)
    {
      ...
      if (pCaptain->insertDatabase())
      {
        ...
        captains.push_back(pCaptain);
        rwlock.unlock();
        Xlogger->info("[军队]: %s 参加 %s 军队",pUser->name,this->name);
...
void CCaptain::writeDatabase()
{
  ...
  {
    Xlogger->error("军队详细数据保存失败,CAPTAIN表不存在");
    return;
  }  
}
...
bool CCaptain::insertDatabase()
{
  ...
  {
    Xlogger->error("添加军队记录失败,CAPTAIN表不存在");
    return false;
  }
