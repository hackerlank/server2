...
/*#ifndef HAVE_STRUCT_DIRENT_D_TYPE
  ...
  rootpath = Seal::global["offlineMsgPath"];
...
#endif //!HAVE_STRUCT_DIRENT_D_TYPE  
    {
      Xlogger->debug("unlink(%s)",(rootpath + record->d_name).c_str());
      if (0 != unlink((rootpath + record->d_name).c_str()))
      {
        Xlogger->error("清空离线消息目录[%s]失败",rootpath.c_str());
        return false;
      }
    }
  }
...
//  Xlogger->debug("写入一个新的离线消息[%s]",myPath.c_str());
  int fd;
  ...
  {
    Xlogger->error("无法写入角色[%u]的离线消息",id);
  }*/
...
/*
  ...
  for (tIterator = filelist.begin(); tIterator != filelist.end(); tIterator++)
  {
    ...
    {
      cmdLen = read(fd,buf,x_socket::MAX_DATASIZE);
      pUser->sendCmdToMe(buf,cmdLen);
      close(fd);
    }
    else
    {
      Xlogger->error("无法读取角色[%s]的离线消息",pUser->name);
    }
