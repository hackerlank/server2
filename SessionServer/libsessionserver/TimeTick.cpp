	...
	void exec(CCountry * c)
	{
		if (rushID && cityID && c)
		{
			...
			if (ss)
			{
				...
				Xlogger->warn("向 %s 发送怪物攻城消息",c->name);
			}
			else
				Xlogger->warn("向 %s 发送怪物攻城消息失败",c->name);
		}
	}
};
...
#ifdef _DEBUG
									Xlogger->debug("GM公告:%s:%s mapID=%u",send.GM,send.info,send.mapID);
#endif
								}
							}
							else//国家公告
...
Xlogger->warn("向 无国籍国家 发送怪物攻城消息失败");

rush ++;
Xlogger->info("运营活动，9点全国怪物攻城");
break;
}
}
else if (3==rush)
...
{
rush++;
Xlogger->info("运营活动，2分钟后福神拜年");
}
break;
case 1:
{
rush++;
Xlogger->info("运营活动，1分钟后福神拜年");
}
break;
case 2:
{
...
rush ++;
Xlogger->info("运营活动，18点福神拜年");
}
break;
case 3:
{
rush++;
Xlogger->info("运营活动，4分钟后财神拜年");
}
break;
case 4:
{
rush++;
Xlogger->info("运营活动，3分钟后财神拜年");
}
break;
case 5:
{
rush++;
Xlogger->info("运营活动，2分钟后财神拜年");
}
break;
case 6:
{
rush++;
Xlogger->info("运营活动，1分钟后财神拜年");
}
break;
case 7:
{
