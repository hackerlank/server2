...
bool UserSessionManager::init()
{
  if (inited)
  {
    Xlogger->warn("User Session管理器已经被初始化过...");
    return true;
  }
  inited=true;
  Xlogger->info("初始化User Session管理器成功...");
  return inited;
}
...
void UserSessionManager::removeAllUserByTask(SessionTask *task)
{
  ...
  {
    Xlogger->debug(" 清理此服务器(%ld,%ld)的所有用户信息",task->getID(),task->getType());
    rust.task=task;
    offline.task=task;
  }
  else
  {
    Xlogger->debug(" 未知服务器(%ld,%ld)",task->getID(),task->getType());
    rust.task=NULL;
    offline.task=NULL;
  }
...
bool SceneSessionManager::init()
{
  if (inited)
  {
    Xlogger->warn("Scene Session管理器已经被初始化过...");
    return true;
  }
  inited=true;
  Xlogger->info("初始化Scene Session管理器成功...");
  return inited;
}
