...
bool SessionService::init()
{
  ...
  // 初始化对战系统相关参数
  dare_active_time = atoi(Seal::global["dare_active_time"].c_str());
  if (dare_active_time<=0)
  {
    dare_active_time = 3600; // 默认3600秒
  }
  
  dare_ready_time  = atoi(Seal::global["dare_ready_time"].c_str());
  ...
  dare_need_gold   = atoi(Seal::global["dare_need_gold"].c_str());
  if (dare_need_gold<=0)
  {
    dare_need_gold = 5000; // 五十两
  }
  
  dare_winner_gold = atoi(Seal::global["dare_winner_gold"].c_str());
  if (dare_winner_gold<=0)
  {
    dare_winner_gold = 10000; // 奖励一百两
  }

  dare_need_gold_sept = atoi(Seal::global["dare_need_gold_sept"].c_str());
  ...
  dare_winner_gold_sept = atoi(Seal::global["dare_winner_gold_sept"].c_str());
  ...
  // 初始化竞赛系统相关参数
  quiz_active_time = atoi(Seal::global["quiz_active_time"].c_str());
  if (quiz_active_time<=0)
  {
    quiz_active_time = 60; // 默认60题
  }
  
  quiz_ready_time  = atoi(Seal::global["quiz_ready_time"].c_str());
  ...
  {
    Xlogger->error("SessionService::init 连接档案服务器失败");
    return false;
  }
  if (recordClient->start())
  {
        Xlogger->info("初始化档案服务器模块成功");
...
//    Xlogger->info("初始化TimeTick模块成功");
  ...
  {
    Xlogger->error("初始化CITY数据失败");
    return false;
  }
  
  if (!CCountryM::getMe().init())
  {
    Xlogger->error("初始化COUNTRY数据失败");
    return false;
  }

  if (!CDareRecordM::getMe().init())
  {
    Xlogger->error("初始化DARERECORD数据失败");
    return false;
  }

  if (!CSortM::getMe().init())
  {
    Xlogger->error("初始化角色排名系统失败");
    return false;
  }
  
  if (!CVoteM::getMe().init())
  {
    Xlogger->error("初始化投票系统失败");
    return false;
  }
  
  if (!CArmyM::getMe().init())
  {
    Xlogger->error("初始化军队数据失败");
    return false;
  }
  
  if (!CGemM::getMe().init())
  {
    Xlogger->error("初始化护宝数据失败");
    return false;
  }
  ...
  {
    Xlogger->error("初始化国家联盟数据失败");
    return false;
  }
  
  if (!RecommendM::getMe().init())
  {
    Xlogger->error("初始化推荐人数据失败");
    return false;
  }
  ...
  if (((tmValue.tm_hour==23) && (tmValue.tm_min>50)) || donow)
  {
    ...
    if (!userMap.empty())
    {
      for(BYTE j=0; j<13; j++)
      {
        ...
        Xlogger->info("国家强弱：%d=%u",j,countryLevel[j]);
      }
    
      Cmd::Session::t_countryPowerSort_SceneSession send;
      bzero(send.country,sizeof(send.country));
#if _DEBUG
      Xlogger->info("sizeof(send.country)=%u",sizeof(send.country));
#endif
...
bool SessionService::msgParse_SuperService(const Cmd::t_NullCmd *pNullCmd,const DWORD nCmdLen)
{
          ...
          case PARA_BROADCAST_GMTOOL:
            {
              ...
              Xlogger->debug("[GM工具]收到公告 %s:%s id=%u time=%u country=%u mapID=%u",rev->GM,rev->content,rev->id,rev->time,rev->country,rev->mapID);
...
#ifdef _DEBUG
                        Xlogger->debug("GM公告:%s mapID=%u mapName=%s",send.info,send.mapID,scene->name);
#endif
                      }
#ifdef _DEBUG
                      else
                        Xlogger->debug("GM公告:%s mapID=%u 没找到地图",rev->content,(rev->country<<16)+rev->mapID);
#endif
                    }
                  else
  ...
  Xlogger->error("SessionService::msgParse_SuperService(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
  return false;
}
...
bool SessionService::checkShutdown()
{
  ...
  if (timValue >= shutdown_time.time)
  {
    ...
    Xlogger->debug("停机维护时间%d年%d月%d日%d时%d分%d秒",tm_1.tm_year+1900,tm_1.tm_mon+1,tm_1.tm_mday,tm_1.tm_hour,tm_1.tm_min,tm_1.tm_sec);
    return true;
  }
  return false;
}
...
{
  Xlogger=new zLogger("SessionServer");
