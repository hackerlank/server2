...
bool CArhat::refreshNPC()
{
  ...
  if (pScene)
  {
    ...
    if (this->dwID==1000)
    {
      SessionChannel::sendCountryInfo(Cmd::INFO_TYPE_GAME,
        ...
        this->x,this->y);
#ifdef _DEBUG
  Xlogger->debug("[护宝任务]: 降龙罗汉刷新地点: country:%d mapid:%d x:%d y:%d",
    ...
    else
    {
      SessionChannel::sendCountryInfo(Cmd::INFO_TYPE_GAME,
        ...
        this->x,this->y);
#ifdef _DEBUG
  Xlogger->debug("[护宝任务]: 伏虎罗汉刷新地点: country:%d mapid:%d x:%d y:%d",
      this->dwCountryID,this->dwMapRealID,this->x,this->y);
#endif  
    }
  ...
  {
    Xlogger->error("[护宝]：未找到罗汉出生地图 %d,%d",this->dwCountryID,this->dwMapRealID);
  }
...
{
  Xlogger->info("[护宝跟踪]:%d(%s)",this->dwCountryID,str_gem_state[this->state]);
}
