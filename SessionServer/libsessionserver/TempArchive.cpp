...
bool GlobalTempArchiveIndex::readTempArchive(DWORD id,char *out,DWORD &outSize)
{
  ...
  //Xlogger->debug("临时读档数据内容大小%u",outSize);
...
void GlobalTempArchiveIndex::checkOverdue()
{
  ...
  {
    //Xlogger->debug("临时存档数据离失效时间还剩%u秒",ctv.sec() - iter->second->createtime.sec());
    if (ctv.sec() - iter->second->createtime.sec() > 120)
    {
      //Xlogger->info("临时存档数据过期失效%u",iter->second->id);
      del_iter = iter;
      iter ++;
      remove(del_iter);
      continue;
...
{
  //Xlogger->debug("临时存档数据内容大小%u",dwSize);
  checkOverdue();
