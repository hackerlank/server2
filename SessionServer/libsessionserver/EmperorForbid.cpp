...
void EmperorForbid::loadDB()
{
  ...
  {               
    Xlogger->error("EmperorForbid::loadDB(): 得到数据库句柄失败");
    return;
  }
  ...
  Xlogger->info("加载 %u 个皇帝禁言信息",list.size());
}
...
void EmperorForbid::writeDB()
{
  ...
  if ((connHandleID)-1 == handle)
  {               
    Xlogger->error("EmperorForbid::writeDB(): 得到数据库句柄失败");
    return;
  }
  ...
  SessionService::dbConnPool->putHandle(handle);
  Xlogger->info("保存 %u 个皇帝禁言信息",list.size());
}
