...
void CAlly::writeDatabase()
{
  ...
  {
    Xlogger->error("国家联盟数据保存失败，ALLY表不存在");
    return;
  }
}
...
bool CAlly::insertDatabase()
{
  ...
  {
    Xlogger->error("国家联盟数据新建失败，ALLY表不存在");
    return false;
  }
...
bool CAlly::deleteMeFromDB()
{
  ...
  {
    Xlogger->error("国家联盟数据删除失败，ALLY表不存在");
    SessionService::dbConnPool->putHandle(handle);
    return false;
  }
...
bool CAllyM::loadAllyFromDB()
{
    ...
    if (recordset)
    {
      for (DWORD i=0; i<recordset->size(); i++)
      {
        ...
        if (pAlly)
        {
          ...
          {
            Xlogger->error("[国家联盟]: %d 与 %d 有重复的联盟数据.",
  ...
  {
    Xlogger->error("国家联盟数据加载失败，ALLY表不存在");
    return false;
  }
    ...
    case Cmd::QUESTION_COUNTRY_ALLY:
      {
        ...
        if (!pCountry) 
        {
          Xlogger->error("[国家联盟]:国家数据不完整,不能使用请求结盟");
          return;
        }
...
void CAllyM::timer()
{
  ...
  for (pos = allies.begin(); pos!=allies.end(); )
  {
    ...
    {
#ifdef _DEBUG
      Xlogger->debug("[国家联盟]: %d,%d 邀请超时,关闭",pTemp->dwCountryID,pTemp->dwAllyCountryID);
#endif      
      rwlock.wrlock();
  ...
  if (tv1.tm_hour == 0 && (tv1.tm_min>=0 && tv1.tm_min<=4))
  {
    for (pos = allies.begin(); pos!=allies.end(); pos++)
    {
      CAlly* pTemp = *pos;
#ifdef _DEBUG
      Xlogger->debug("[国家联盟]: 每日更新联盟友好度: (%d,%d) %d,ct:%u lasttime:%u",
          pTemp->dwCountryID,
