...
bool CNpcDareM::load()
{
...
#ifdef _DEBUG
      Xlogger->debug("dwCountry=%u dwMapID=%u dwNpcID=%u dwHoldSeptID=%u dwDareSeptID=%u dwGold=%u",
    info.dwCountry,
    info.dwMapID,
    info.dwNpcID,
    info.dwHoldSeptID,
  ...
  {
    Xlogger->error("NPC争夺战目标数据加载失败,exeSelect 返回无效buf指针");
  }
  return false;
}
...
#ifdef _DEBUG
  Xlogger->debug("时间：%u:%u",tv1.tm_hour,tv1.tm_min);
#endif
...
void CNpcDareObj::writeDatabase()
{
  ...
  if (1 != retcode)
  {
    Xlogger->error("修改NPC对战档案失败：country=%u,mapid=%u npcid=%u",_dwCountry,_dwMapID,_dwNpcID);
  }
}
...
void CNpcDareObj::processResult(DWORD septid)
{
  if (this->_dwDareSeptID == septid)
  {
    ...
    _dwResultDare++;
#ifdef _DEBUG
  Xlogger->debug("收到交战结果家族id=%u _dwResultDare=%u",septid,_dwResultDare);
#endif
  }
  if (this->_dwHoldSeptID == septid)
  {
    ...
    _dwResultHold++;
#ifdef _DEBUG
  Xlogger->debug("收到交战结果家族id=%u _dwResultHold=%u",septid,_dwResultHold);
#endif
  }
...
void CNpcDareObj::doResult()
{
  ...
  {
    Xlogger->info("[家族]家族NPC争夺战控制者[%u]家族剩余[%d]人挑战者[%u]家族剩余[%d]人MAPID=[%u] NPCID=[%u]",this->_dwHoldSeptID,this->_dwResultHold,this->_dwDareSeptID,this->_dwResultDare,this->_dwMapID,this->_dwNpcID);
...
void CNpcDareObj::processGetGold(UserSession *pUser,DWORD septid,DWORD dwNpcID,DWORD dwMapID,DWORD dwCountryID)
{
    ...
    this->_dwNpcID == dwNpcID)
  {
    if (this->_dwGold >0)
    {
      ...
      send.dwGold = _dwGold;
      pUser->scene->sendCmd(&send,sizeof(send));
      Xlogger->info("[家族]角色%s领取了NPC税金%u",pUser->name,_dwGold);
      _dwGold = 0;
      this->writeDatabase();
    }
    else
