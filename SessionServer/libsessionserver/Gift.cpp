...
bool Gift::init()
{
  Xlogger->debug("[Gift]加载运营活动 %u 个",loadActList());
  Xlogger->debug("[Gift]加载获奖列表 %u 个",loadGiftList());
...
DWORD Gift::loadActList()
{
  ...
  {               
    Xlogger->error("[Gift]loadActList: 得到数据库句柄失败");
    return 0;
  }
...
DWORD Gift::loadGiftList()
{
  ...
  if ((connHandleID)-1 == handle)
  {               
    Xlogger->error("[Gift]loadGiftList: 得到数据库句柄失败");
    return 0;
  }
    ...
    case GET_ITEM_GIFT_PARA:
      {
          ...
          if (i)
          {
              ...
              if ((connHandleID)-1 == handle)
              {               
                  Xlogger->error("[Gift]doGiftCmd(GET_ITEM_GIFT_PARA): 得到数据库句柄失败");
                  return false;
              }
