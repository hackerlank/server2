...
bool RecommendM::load()
{
  ...
  {
    Xlogger->error("推荐人数据加载失败，RECOMMEND表不存在");
    return false;
  }
...
void Recommend::writeDatabase()
{
  ...
  {
    Xlogger->error("推荐人数据保存失败，RECOMMEND表不存在");
    return;
  }  
...
bool Recommend::insertDatabase()
{
  ...
  {
    Xlogger->error("添加推荐人记录失败，RECOMMEND表不存在");
    return false;
  }
...
bool Recommend::loadRecommendSubFromDB()
{
  ...
  {
    Xlogger->error("被推荐人详细数据加载失败，RECOMMENDSUB表不存在");
    return false;
  }
...
void Recommend::pickupBounty(UserSession* pUser)
{
  if (pUser && pUser->scene && this->dwBalance>0)
  {
    ...
    Xlogger->info("[推荐人]: %s 提取了推荐人奖励金:%u 文",pUser->name,this->dwBalance);
    this->dwBalance = 0;
...
void RecommendSub::writeDatabase()
{
  ...
  {
    Xlogger->error("被推荐人详细数据保存失败，RECOMMENDSUB表不存在");
    return;
  }  
...
bool RecommendSub::insertDatabase()
{
  ...
  {
    Xlogger->error("添加被推荐人记录失败，RECOMMENDSUB表不存在");
    return false;
  }
...
void RecommendSub::pickupBounty(UserSession* pUser)
{
  ...
  if (pUser && pUser->scene && this->lastLevel!=pUser->level)
  {
    ...
    if (send.dwMoney)
    {
      send.byType = 1;
      pUser->scene->sendCmd(&send,sizeof(send));
      Xlogger->info("[推荐人]: %s(%u,%u) 提取了被推荐人奖励金:%u 文",
