    ...
    case PARA_CARTOON_SET_REPAIR:
      {
        ...
        */
        Xlogger->info("[自动修理]%s(%u) %s自动修理",cartoonPetList[rev->cartoonID].masterName,cartoonPetList[rev->cartoonID].masterID,rev->repair?"打开":"关闭");
        return true;
      }
      break;
    case PARA_CARTOON_DRAW:
      {
        ...
        {
          Xlogger->error("[宠物]%s(%u) 提取宝宝经验失败 cartoondID=%u addExp=%u state=%u"
        ...
        {
          Xlogger->error("[宠物]%s(%u) 提取宠物经验失败 cartoonID=%u addExp=%u",pUser->name,pUser->id,rev->cartoonID,rev->num);
    ...
    case PARA_CARTOON_SALE:
      {
        ...
        {                       
          Xlogger->error("[宠物]PARA_CARTOON_SALE:得到数据库句柄失败");
          return false;
        }
    ...
    case PARA_CARTOON_BUY:
      {
        ...
        if ((connHandleID)-1 == handle) 
        {                       
          Xlogger->error("[宠物]PARA_CARTOON_BUY:得到数据库句柄失败");
          return false;
        }
        ...
        */
        if (cartoonPetMap[pUser->id].size()>=2)
        {
          pUser->sendSysChat(Cmd::INFO_TYPE_FAIL,"你的宠物太多了");
          SessionService::dbConnPool->putHandle(handle);
          Xlogger->info("[宠物]检测到 %s 重复购买宠物,丢失银子 %u 文",pUser->name,rev->data.npcID==9005?5000:1000);
          return true;
        }
        ...
        if ((DWORD)-1 == retcode)
        {                       
          Xlogger->error("[宠物]PARA_CARTOON_BUY: 添加数据库失败 retcode=%d",retcode);
          return false;
        }
    ...
    case PARA_CARTOON_SAVE:
      {
        ...
        switch (rev->type)
        {
          ...
          case SAVE_TYPE_SYN:
            ...
            {
              Xlogger->error("[宠物]保存指令被丢弃 cartoonID=%u state=%u saveType=%u",rev->cartoonID,cartoonPetList[rev->cartoonID].state,rev->type);
              return false;
            }
        ...
        if (rev->type!=SAVE_TYPE_SYN && rev->type!=SAVE_TYPE_TIMETICK)
          Xlogger->debug("[宠物]保存宠物 %s(%u) addExp=%u time=%u saveType=%u state=%u level=%u userName=%s",rev->data.masterName,rev->cartoonID,cartoonPetList[rev->cartoonID].addExp,rev->data.time,rev->type,cartoonPetList[rev->cartoonID].state,cartoonPetList[rev->cartoonID].lv,rev->userName);
    ...
    case PARA_CARTOON_ADOPT:
      {
        ...
        */
        ...
        Xlogger->debug("[宠物]%s 领养了 %s 的替身宠物 %s(%u)",pUser->name,cd.masterName,cd.name,rev->cartoonID);
    ...
    case PARA_CARTOON_CHARGE:
      {
        ...
        if (cartoonPetList.find(rev->cartoonID)==cartoonPetList.end())
        {
          Xlogger->error("[宠物]给宠物充值时找不到宠物 cartoonID=%u time=%u",rev->cartoonID,rev->time);
          return false;
        }
        ...
        if (!writeDB(rev->cartoonID,cartoonPetList[rev->cartoonID]))
        {
          Xlogger->error("[宠物]%s 给宠物充值写数据库失败 cartoonID=%u time=%u",cartoonPetList[rev->cartoonID].masterName,rev->cartoonID,rev->time);
          return false;
        }
        ...
        Xlogger->info("[宠物]%s 给宠物 %s(%u) 充值 %u 秒",cartoonPetList[rev->cartoonID].masterName,cartoonPetList[rev->cartoonID].name,rev->cartoonID,rev->time);
        return true;
      }
      break;
      /*
    case PARA_CARTOON_CONSIGN:
      {
        ...
        if (cartoonPetList.find(rev->cartoonID)==cartoonPetList.end())
        {
          Xlogger->error("[宠物]委托领养宠物时找不到宠物 cartoonID=%u",rev->cartoonID);
          return false;
        }
...
bool CartoonPetService::writeDB(DWORD cartoonID,Cmd::t_CartoonData& data)
{
  ...
  if ((connHandleID)-1 == handle)
  {               
    Xlogger->error("[宠物]writeDB: 得到数据库句柄失败");
    return false;
  }
  char where[128];
  bzero(where,sizeof(where));
...
void CartoonPetService::checkAdoptable(DWORD id)
{
  ...
  for (hash_set<DWORD>::iterator it=waitingList.begin(); it!=waitingList.end(); it++)
  {
    if (cartoonPetList[*it].state==Cmd::CARTOON_STATE_WAITING)
    {
      ...
      Xlogger->error("[宠物]等待列表中宠物状态错误 cartoonID=%u master=%s",*it,cartoonPetList[*it].masterName);
  }
}
...
void CartoonPetService::sendCmdToItsFriendAndFamily(DWORD id,const char * name,const void *cmd,DWORD len,const char * except)
{
  ...
  else
  {
    ...
    if ((connHandleID)-1 == handle)
    {               
      Xlogger->error("[宠物]sendCmdToItsFriendAndFamily: 得到数据库句柄失败");
      return;
    }
    char where[128];
    bzero(where,sizeof(where));
...
void CartoonPetService::repairData()
{
  ...
  if ((connHandleID)-1 == handle)
  {               
    Xlogger->error("[宠物]repairData: 得到数据库句柄失败");
    return;
  }
  char where[128];
  bzero(where,sizeof(where));
...
void CartoonPetService::delPetRecordByID(DWORD masterID)
{
  ...
  if ((connHandleID)-1 == handle)
  {               
    Xlogger->error("[宠物]delPetRecordByID: 得到数据库句柄失败");
    return;
  }
  char where[128];
  bzero(where,sizeof(where));
  ...
  Xlogger->info("[宠物]玩家删号,删除所有宠物 masterID=%u",masterID);
}
/*
...
bool CartoonPetService::loadAllFromDB()
{
  ...
  if ((connHandleID)-1 == handle)
  {               
    Xlogger->error("[宠物]loadAllFromDB: 得到数据库句柄失败");
    return false;
  }
  char where[128];
  bzero(where,sizeof(where));
  ...
  if (dataList)                          
  {
    ...
    Xlogger->info("[宠物]加载 %u 个替身宠物信息成功",retcode);
  }
  return true;
}
...
DWORD CartoonPetService::writeAllToDB(bool groupflag)
{
  ...
  if (ret)
    if (groupflag)
      Xlogger->debug("[宠物]保存 %u 条替身宠物记录 group=%u",ret,group);
    else
      Xlogger->debug("[宠物]保存 %u 条替身宠物记录",ret);
#endif
  return ret;
}
...
void CartoonPetService::userOnline(UserSession * pUser)
{
    ...
    Xlogger->error("[宠物]%s 上线,修复 %u 只宠物的状态",pUser->name,adoptedPetMap[pUser->name].size());
}
