...
bool SessionTask::verifyLogin(const Cmd::Session::t_LoginSession *ptCmd)
{
      ...
      && (SCENESSERVER == ptCmd->wdServerType || GATEWAYSERVER == ptCmd->wdServerType))
  {
    ...
    if (NULL != entry){
      Xlogger->debug("SessionTask::verifyLogin %s,%d,%d for %s,%d,%d",entry->pstrIP,entry->wdServerID,entry->wdServerType,strIP,ptCmd->wdServerID,ptCmd->wdServerType);
    }
    else{
      Xlogger->error("SessionTask::verifyLogin NULL for %s,%d,%d",strIP,ptCmd->wdServerID,ptCmd->wdServerType);
    }
    ...
    Xlogger->error("!SessionTask::verifyLogin cmd=%d,para=%d,wdServerID=%d,wdServerType=%d",ptCmd->cmd,ptCmd->para,ptCmd->wdServerID,ptCmd->wdServerType);
  return false;
}
...
bool SessionTask::del_role(const Cmd::t_NullCmd* cmd,const DWORD cmdLen)
{
  ...
  if (rev->status == 0)
  {
    Xlogger->info("已解除社会关系，继续删除角色的处理");
  }
  else
  {
    Xlogger->info("存在不能解除的社会关系，删除角色操作被取消");
  }
    ...
    case Cmd::Session::PARA_SCENE_REGSCENE:
      {
		  ...
		  if (scene->reg(reg) && SceneSessionManager::getInstance()->addScene(scene))
		  {
			  ...
			  Xlogger->info("注册地图%u(%s %s) 成功",reg->dwID,reg->byName,reg->fileName);
		  ...
		  else
		  {
			  ...
			  Xlogger->error("注册地图%u(%s %s)失败",reg->dwID,reg->byName,reg->fileName);
		  }
    ...
    case Cmd::Session::PARA_SCENE_UNREGUSER:
      {
        ...
        if (pUser)
        {
          ...
          if (unreg->retcode==Cmd::Session::UNREGUSER_RET_ERROR)
            Xlogger->info("ScenesServer错误，注销%s(%ld)",pUser->name,pUser->id);
          else
            Xlogger->info("场景请求用户%s(%ld)注销",pUser->name,pUser->id);
          SAFE_DELETE(pUser);
        }
        return true;
      }
...
//        CDareM::getMe().userOnline(pUser);
    ...
    case Cmd::Session::PARA_USER_ARCHIVE_REQ:
      {
        ...
        if (buf)
        {
          ...
          if (GlobalTempArchiveIndex::getInstance()->readTempArchive(ret->id,ret->data,ret->dwSize))
          {
            ...
            Xlogger->info("发送临时读档数据包%u",sizeof(Cmd::Session::t_ReadUser_SceneArchive) + ret->dwSize);
          }
        }
        return true;
      }
    ...
    case Cmd::Session::PARA_USER_ARCHIVE_WRITE:
      {
        ...
        //Xlogger->debug("temp->dwSize=%u",temp->size);
        //Xlogger->debug("rev->dwSize=%u",rev->dwSize);
        //Xlogger->debug("收到临时存档数据包%u",cmdLen);
    ...
    case Cmd::Session::PARA_SCENE_CHANEG_SCENE:
      {
        ...
        if (scene->level>0)
        {
          if (pUser)
          {
            if (pUser->level < scene->level)
            {
              ...
              Xlogger->info("[GOMAP]玩家%s等级不够跳到地图[%s]失败!",pUser->name,scene->name);
              return true;
            }
          }
          //else return true;
        ...
        //Xlogger->debug("场景信息%s,%u,%u",scene->name,scene->id,scene->tempid);
        //Xlogger->debug("场景信息%s,%s,%u",rev->map_file,rev->map_name,rev->map_id);
    ...
    case Cmd::Session::PARA_SCENE_REMOVE_SCENE:
      {
        ...
        if (scene)
        {
          ...
          Xlogger->info("卸载地图%u(%s) 成功",scene->id,scene->name);
          SAFE_DELETE(scene);
        }
        return true;
      }
    ...
    case Cmd::Session::PARA_SCENE_REQ_ADD_SCENE:
      {
        ...
        Xlogger->info("转发加载地图消息(%u,%u,%u)",rev->dwServerID,rev->dwCountryID,rev->dwMapID);
    ...
    case Cmd::Session::PARA_SCENE_UNLOAD_SCENE:
      {
        ...
        if (scene)
        {
          ...
          Xlogger->info("地图%s因卸载地图停止注册",scene->name);
          /*
             SceneSessionManager::getInstance()->removeScene(scene);
             ...
             if (pUser)
             {
             Xlogger->info("用户%s(%ld)因卸载地图注销",pUser->name,pUser->id);
             UserSessionManager::getInstance() ->removeUser(pUser);
             SAFE_DELETE(pUser);
             }
             }
             Xlogger->debug("卸载地图%u(%s) 成功",scene->id,scene->name);
             SAFE_DELETE(scene);
          // */
        }
        return true;
  ...
  Xlogger->error("SessionTask::msgParse_Scene(%u,%u,%u)",cmd->cmd,cmd->para,cmdLen);
  return false;
}
...
bool SessionTask::msgParse_Gate(const Cmd::t_NullCmd *cmd,const DWORD cmdLen)
{
    ...
    case Cmd::Session::PARA_GATE_REGUSER:
      {
        ...
        if (scene)
        {
            ...
            if (!pUser)
            {
              ...
              if (pUser && pUser->reg(reg))
              {
                ...
                if (UserSessionManager::getInstance()->addUser(pUser))
                {
                  ...
                  Xlogger->info("用户%s(%ld)注册成功",pUser->name,pUser->id);
                ...
                else
                {
                  ...
                  if (pUser)
                  {
                    Xlogger->debug("角色id重复(id=%u,name=%s,tempid=%u" 
                        ,pUser->id,pUser->name,pUser->tempid);
                  }
                  pUser=UserSessionManager::getInstance()->getUserByTempID(reg->dwTempID);
                  if (pUser)
                  {
                    Xlogger->debug("角色tempid重复(id=%u,name=%s,tempid=%u" 
                        ,pUser->id,pUser->name,pUser->tempid);
                  }
                  Xlogger->error("添加角色失败，可能是角色重复登陆");
                }
                SAFE_DELETE(pUser);
              }
              else
                Xlogger->fatal("注册用户时候分配内存失败(%u,%u,%x)",reg->accid,reg->dwID,reg->byName);
            }
            else
            {
              CSortM::getMe().offlineCount(pUser);
              ...
              Xlogger->info("发现重复用户注销%s(%ld)注销",pUser->name,pUser->id);
              SAFE_DELETE(pUser);
              return true;
            }
          }
          ...
          {
            Xlogger->info("场景%s现在不可以注册用户",(char *)reg->byMapName);
          }
        }
        else
          Xlogger->error("未找到角色所在地图 %s",(char *)reg->byMapName);
        //通知网关注册失败
        Xlogger->error("用户(%lu,%lu,%s,%lu)注册失败",reg->accid,reg->dwID,reg->byName,reg->dwTempID);
        Cmd::Session::t_unregUser_GateSession ret;
        ret.dwUserID=reg->dwID;
        if (scene)
          ret.dwSceneTempID=scene->tempid;
    ...
    case Cmd::Session::REQUEST_GATE_COUNTRY_ORDER:
      {
        ...
        {
          Xlogger->debug("国家:%d,在线人数:%d",ret_gate->order.order[i].country,ret_gate->order.order[i].count);
        }
    ...
    case Cmd::Session::PARA_GATE_CHANGE_SCENE_USER:
      {
        ...
        if (scene)  {
          ...
          if (pUser)   {
            ...
            Xlogger->info("用户%s(%ld)切换场景成功",pUser->name,pUser->id);
            ...
            Xlogger->error("未找到用户%s(%ld)",(char*) reg->byName,reg->dwID);
          }
        }
        else
          Xlogger->error("未找到角色所在地图 %s",(char *)reg->byMapFileName);
        //通知网关注册失败
        Xlogger->error("用户(%lu,%lu,%s,%lu)注册失败",reg->accid,reg->dwID,reg->byName,reg->dwTempID);
        Cmd::Session::t_unregUser_GateSession ret;
        ret.dwUserID=reg->dwID;
    ...
    case Cmd::Session::PARA_GATE_UNREGUSER:
      {
        ...
        if (pUser)
        {
          ...
          {
            Xlogger->error("用户%s(%ld)因网关错误注销",pUser->name,pUser->id);
            SAFE_DELETE(pUser);
            return true;
          }
          else if (reg->retcode==Cmd::Session::UNREGUSER_RET_LOGOUT)
          {
            if (scene)
            {
              ...
              scene->sendCmd(&send,sizeof(send));
              Xlogger->info("网关请求用户%s(%ld)注销",pUser->name,pUser->id);
              SAFE_DELETE(pUser);
              return true;
            }
            else
            {
              ...
              Xlogger->error("用户%s注销时发生错误,发送广播消息注销场景用户",pUser->name);
            }
          }
          SAFE_DELETE(pUser);
        }
        else
          Xlogger->error("注销时未找到用户%ld",reg->dwUserID);
  ...
  Xlogger->error("SessionTask::msgParse_Gate(%u,%u,%u)",cmd->cmd,cmd->para,cmdLen);
  return false;
}
...
bool SessionTask::msgParse_Forward(const Cmd::t_NullCmd *pNullCmd,const DWORD cmdLen)
{
  ...
  {
    Xlogger->error("msgParse_Forward");
          ...
          if (pUser && pUser->unionid>0 && chatCmd->dwType != Cmd::CHAT_TYPE_COUNTRY_PK && chatCmd->dwType != Cmd::CHAT_TYPE_COUNTRY_MARRY)
                ...
                || chatCmd->dwSysInfoType == Cmd::INFO_TYPE_CASTELLAN)
                  ...
                  if (pUser)
                  {
                    ...
                    {
                      Xlogger->error("怀疑玩家%s使用外挂利用自定义消息刷屏",pUser->name);
                    }
    ...
    else
    {
      switch(cmd->byCmd)
      {
        case Cmd::CHAT_USERCMD:
          ...
          break;
        default:
          Xlogger->error("处理用户指(%d,%d)令时,未找到角色为%ld的用户",pNullCmd->cmd,pNullCmd->para,rev->dwID);
          break;
      }
      return true;
    }
  }
  Xlogger->error("SessionTask::msgParse_Forward(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,cmdLen);
  return false;
}
...
bool SessionTask::cmdMsgParse(const Cmd::t_NullCmd *cmd,const DWORD cmdLen)
{
	...
	if (Cmd::Session::CMD_FORWARD == cmd->cmd && Cmd::Session::PARA_FORWARD_USER == cmd->para)
	{
		...
		Xlogger->error("SessionTask::cmdMsgParse(%u,%u,%u)",pNUC->byCmd,pNUC->byParam,cmdLen);

		if (Cmd::CHAT_USERCMD == pNUC->byCmd && ALL_CHAT_USERCMD_PARAMETER == pNUC->byParam)
		{
			pCCUC = (Cmd::stChannelChatUserCmd*)pNUC;
			Xlogger->error("SessionTask::cmdMsgParse dwType=%d,dwSysInfoType=%d,dwChannelID=%d,size=%d,pstrChat=%s",pCCUC->dwType,pCCUC->dwSysInfoType,pCCUC->dwChannelID,pCCUC->size,pCCUC->pstrChat);
		}
...

		{
			...
			Xlogger->error("isEmpty = %d",isEmpty);
			...
			case Cmd::Session::PARA_SHUTDOWN:
				{
					...
					Xlogger->info("系统当前时间%d年%d月%d日%d时%d分%d秒",tm_1.tm_year+1900,tm_1.tm_mon+1,tm_1.tm_mday,tm_1.tm_hour,tm_1.tm_min,tm_1.tm_sec);
					if (sss->time)
					{
						...
						Xlogger->debug("系统将于%d年%d月%d日%d时%d分%d秒停机维护",tm_1.tm_year+1900,tm_1.tm_mon+1,tm_1.tm_mday,tm_1.tm_hour,tm_1.tm_min,tm_1.tm_sec);
