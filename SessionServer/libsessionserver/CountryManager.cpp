...
bool CCountryM::load()
{
			...
			for (DWORD i=0; i<recordset->size(); i++)
			{
				...
				if (pCountry)
				{
					...
					{//如果拥有者
						...
						Xlogger->info("帮会已不存在,%s:国王清空",pCountry->name);
					}
				}
	...
	{
		Xlogger->error("国家数据加载失败,COUNTRY表不存在");
		return false;
	}
	...
	case KING_PUNISH_COUNTRY_PARA:
		{
			...
			Xlogger->info("用户%s(%ld)被国王关进监狱",u->name,u->id);
		...
		if (strncmp(pEmperor->kingName,pUser->name,MAX_NAMESIZE) == 0)
		{
			...
			if ((ct-pEmperor->dwLastDailyMoney) >= 24*60*60)
			{
				...
				{       
					...
					Xlogger->info("角色 %s 领取了皇帝税金%u文",pUser->name,send.dwNum);
...
void CCountryM::processUpTech(UserSession* pUser,Cmd::stUpTechDegreeUserCmd* rev)
{
	...
	if (!pCountry)
	{
		Xlogger->error("[国家]:国家信息获取失败,请检查国家信息完整性");
	}
...
void CCountryM::processSetDiplomat(UserSession* pUser,Cmd::stAppointDiplomatCmd* rev)
{
	if (this->isKing(pUser))
	{
		...
		{
			Xlogger->error("%d 国家数据不存在",pUser->country);
		}
	}
	else
	{
...
void CCountryM::userOnline(UserSession *pUser)
{
	if (pUser)
	{
		...
		{
		#ifdef _DEBUG
		Xlogger->debug("[国战]:%s 进入国战状态",pUser->name);
		#endif     
...
void CCountryM::broadcastTech(DWORD dwCountryID)
{
	...
	{
		Xlogger->info("[国家]: %d 国家数据不存在。请检查国家数据完整性",dwCountryID);
	}
}
...
void CCountryM::refreshGeneral(DWORD country)
{
	...
	#ifdef _DEBUG
	Xlogger->info("刷新大将军 map=%s level=%u",scene->name,pCountry->gen_level);
	#endif
	}
	else
	Xlogger->error("刷新大将军时未找到地图 mapID=%u",(country<<16)+KING_CITY_ID);
	*/
}
...
void CCountry::writeDatabase()
{
	...
	{
		Xlogger->error("国家数据保存失败,COUNTRY表不存在");
		return;
	}
}
...
bool CCountry::insertDatabase()
{
	...
	{
		Xlogger->error("国家数据新建失败,COUNTRY表不存在");
		return false;
	}
...
void CCountry::loadTechFromDB()
{
	...
	{
		Xlogger->error("科技详细数据加载失败,TECH表不存在");
		return;
	}
...
bool CCountry::changeDiplomat(UserSession* pUser)
{
	...
	{
#ifdef _DEBUG
		Xlogger->debug("[国家]:%s 被任命为外交官",pUser->name);
#endif    
		UserSession *u = UserSessionManager::getInstance()->getUserSessionByName(this->diplomatName);
		if (u && u->scene)
		{
...
void CCountry::addGeneralExp(DWORD num)
{
	...
	while (gen_exp>=gen_maxexp && gen_level<10)
	{
		...
		Xlogger->info("大将军升级 country=%u level=%u",dwID,gen_level);
	}
...
void CCountry::generalLevelDown()
{
	...
	Xlogger->info("大将军降级 country=%u level=%u",dwID,gen_level);
}
...
void CCountry::refreshGeneral()
{
	...
	if (scene)
	{
		...
		gen_refreshTime = 0;
#ifdef _DEBUG
		Xlogger->info("刷新大将军 map=%s level=%u",scene->name,gen_level);
#endif
	}
	else
		Xlogger->error("刷新大将军时未找到地图 mapID=%u",(dwID<<16)+KING_CITY_ID);
}
...
void CTech::writeDatabase()
{
	...
	{
		Xlogger->error("国家科技数据保存失败,TECH表不存在");
		return;
	}
}
