...
bool CCityM::load()
{
  ...
  {
    Xlogger->error("CCityM::load 城市数据加载失败,CITY表不存在");
    return false;
  }
  ...
  if (pCity)
  {
    if (pCity->dwGold>0)
    {
      ...
      if (pUser->scene) 
      {
        ...
        Xlogger->info("角色 %s 领取了城市税金%u文",pUser->name,pCity->dwGold);
      }
...
void CCityM::beginDare()
{
  ...
  for(vIterator = citys.begin(); vIterator!=citys.end(); vIterator++)
  {
    ...
    {
      Xlogger->info("[对战]: (UNION_CITY_DARE,%d,%d) 今天无对战",
...
void CCity::writeDatabase()
{
  ...
  {
    Xlogger->error("CCity::writeDatabase 城市数据加载失败,CITY表不存在");
    return;
  }
}
...
bool CCity::insertDatabase()
{
  ...
  {
    Xlogger->error("CCity::insertDatabase 城市数据加载失败,CITY表不存在");
    return false;
  }
...
void CCity::beginDare()
{
  if (this->dareSize()>0)
  {
    ...
    if (pDefUnion == NULL)
    {
      Xlogger->info("[对战]: 城市:%d 拥有帮会已不存在,取掉对战",this->dwCityID);
      if (this->dareSize()>0)
      {
    ...
    for (DareSet::iterator pos = vDareList.begin(); pos!=vDareList.end(); ++pos)
    {
      ...
      if (pAttUnion)
      {
        ...
        Xlogger->info("[对战]: 帮会夺城战在 %s 与 %s 之间举行",
