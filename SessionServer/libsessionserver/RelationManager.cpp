...
void CRelationManager::deleteDBRecord(const DWORD dwID)
{
  ...
  if ((DWORD)-1 == retcode)
  {
    Xlogger->debug("%s删除关系记录失败 %u",user->name,dwID);
    return;
  }
  return;
}
...
void CRelationManager::updateDBRecord(const CRelation *relation)
{
  ...
  if ((DWORD)-1 == retcode)
  {
    Xlogger->error("CRelationManager::updateDBRecord() 修改好友档案失败CHARID = %u RELATIONID = %u retcode =%u",user->id,relation->id,retcode);
  }
}
...
void CRelationManager::setFriendDegree(Cmd::Session::t_CountFriendDegree_SceneSession *rev)
{
  ...
  for (int i=0; i<rev->size; i++)
  {
    ...
    {
#ifdef _DEBUG
      Xlogger->info("设置友好度：设置%s 与 %s 的 %u 友好度为 %d",this->user->name,rel->name,rel->type,rev->namelist[i].wdDegree);
#endif
