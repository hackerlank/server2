...
class PrintSet
{
  public:
    ...
    {
      Xlogger->debug("题目跟踪[全国竞赛]:本轮已答过的题目:%d",subid);
    }
};
...
{
  int db_answer_count = 0; //atoi(Seal::global["answer_count"].c_str());
...
//    Xlogger->debug("[题库跟踪[选题]]:(%d,%d),title:%s",i,
...
{
  //Xlogger->info("竞赛跟踪:%d(%s)",this->tempid,str_state[this->state]);
}
...
void CQuizWorld::setSendQuestionState()
{
  ...
  Xlogger->debug("----题库跟踪[发题]:subject:%d title:%s answer:%d(全国)",cur_subject+1,
...
int CQuizWorld::answer(Cmd::stAnswerQuiz* pCmd,DWORD dwUserID)
{
  ...
  /*Xlogger->debug("----题目跟踪[答题]:(%d)subject:%d title:%s answer:%d user_answer:%d(全国)",
      dwUserID,
      cur_subject+1,
  ...
  {
    Xlogger->debug("题目跟踪[全国竞赛]:(%d,%d) curid:%d answerid:%d title:%s answer:%d user_answer:%d",
      dwUserID,
      question_count,
      cur_subject+1,
      pCmd->dwID,
...
/*  Xlogger->debug("----题目跟踪[答题](%d):subject:%d title:%s answer:%d user_answer:%d(个人)",
...
//  CSubject temp_subject = CSubjectM::getMe().gsubjects[subjects[cur_subject]];
  ...
  Xlogger->debug("----题库跟踪[发题]:(%d)subject:%d title:%s answer:%d(个人)",pothunter.dwUserID,
...
void CQuizPersonal::setReadyOverState()
{
  ...
  if (pUser)
  {
    ...
    Xlogger->info("[个人答题]:%s 结束个人答题",pUser->name);
    rwlock.wrlock();
    pUser->sendCmdToMe(&send,sizeof(send));
    if (pUser->scene)
    {
    ...
    if (findWorldQuiz() == NULL)
    {
      ...
      if (pQuiz)
      {
        ...
        if (pCmd->active_time<60)
        {
          ...
          Xlogger->debug("---本轮总题目数:%d,原始设定:%d",pQuiz->total_subject,
              pCmd->active_time);
        }
        else
        {
    ...
    if (findPersonalQuiz(pCmd->dwUserID) == NULL)  
    {
      ...
      if (pQuiz)
      {
        if (pUser)
        {
          rwlock.wrlock();
          addEntry(pQuiz);
          Xlogger->info("[个人答题]:%s 开始个人答题",pUser->name);
...
{
//  Xlogger->info("答题目前对象数:%d",this->size());
}
...
CSubjectM::CSubjectM()
{
/*  if (atoi(Seal::global["answer_count"].c_str())>=1)
  {
    gsubjects.resize(atoi(Seal::global["answer_count"].c_str()));
  }
  else
  {
...
bool CSubjectM::init()
{
  ...
  int db_answer_count = atoi(Seal::global["answer_count"].c_str());
  if (db_answer_count == 0)
  {
  ...
  Xlogger->debug("--答题跟踪:题库题目总数：g:%d,p:%d l:%d",gsubjects.size(),personals.size(),levels.size());
  return true;
}
