...
bool CVoteM::load()
{
  ...
  {
    Xlogger->error("国家数据加载失败,VOTE表不存在");
    return false;
  }
...
void CVote::writeDatabase()
{
  ...
  {
    Xlogger->error("投票数据保存失败,VOTE表不存在");
    return;
  }  
}
...
bool CVote::insertDatabase()
{
  ...
  {
    Xlogger->error("添加投票记录失败,VOTE表不存在");
    return false;
  }
...
bool CVote::loadItemFromDB()
{
  ...
  {
    Xlogger->error("投票详细数据加载失败,VOTEITEM表不存在");
    return false;
  }
...
void CVote::setReadyState()
{
  ...
  rwlock.unlock();
  Xlogger->info("[投票]:(%d,%d) VOTE_READY",this->dwCountryID,this->dwType);
}
...
void CVote::setActiveState()
{
  ...
  this->dwStatus = CVote::VOTE_ACTIVE;
  rwlock.unlock();
  Xlogger->info("[投票]:(%d,%d) VOTE_ACTIVE",this->dwCountryID,this->dwType);
  this->writeDatabase();
}
...
void CVote::setOverState()
{
  ...
  rwlock.unlock();
  Xlogger->info("[投票]:(%d,%d) VOTE_OVER",this->dwCountryID,this->dwType);
  this->writeDatabase();
}
...
bool CVote::addVoted(DWORD dwCharID)
{
  ...
  {
    Xlogger->error("添加玩家投票记录失败,VOTEDPLAYER表不存在");
    return false;
  }
...
bool CVote::clearVoted()
{
  ...
  {
    Xlogger->error("清除玩家投票记录失败,VOTEDPLAYER表不存在");
    return false;
  }
...
void CVoteItem::writeDatabase()
{
  ...
  {
    Xlogger->error("投票详细数据保存失败,VOTEITEM表不存在");
    return;
  }  
}
...
bool CVoteItem::insertDatabase()
{
  ...
  {
    Xlogger->error("添加投票记录失败,VOTEITEM表不存在");
    return false;
  }
