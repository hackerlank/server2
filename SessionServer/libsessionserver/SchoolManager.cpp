...
void CSendSchoolCallback::exec(CSchoolMember *member,const BYTE tag)
{
  ...
  //  Xlogger->debug("tag = [%u] name = [%s] online = [%u]",temp.tag,temp.name,temp.online);
}
...
bool CSchoolM::init()
{
  ...
  {
    Xlogger->error("不能生成师门管理器对象！");
    return false;
  }


  Xlogger->debug("loadSchoolFromDB()");
  if (!loadSchoolFromDB()) return false;
  Xlogger->debug("loadSchoolMemberFromDB()");
...
bool CSchoolM::loadSchoolFromDB()
{
  ...
  {
    Xlogger->error("门派数据初始化失败,exeSelect 返回无效buf指针");
  }
  return false;
}
...
bool CSchoolM::createSchoolFromDB(const stSchoolInfo &info)
{
  ...
  if (pSchool)
  {
    //    Xlogger->debug("创建门派对象id=[%u]name=[%s]",info.dwSchoolID,info.name);
                ...
                case Cmd::TEACHER_ANSWER_YES:
                  {
                    ...
                    if (isClean(pUser->name))
                    {
                      if (master)
                      {
                        Xlogger->info("[师徒]收到%s招收%s的指令",master->name,pUser->name);
          ...
          case Cmd::FIRE_MEMBER_FROM_SCHOOL_PARA: //开除某人
            {
              ...
              Xlogger->info("[师徒]收到%s开除%s的指令",pUser->name,ptCmd->memberName);
...
void CSchoolM::processLeaveGroup(UserSession *pUser)
{
  ...
  if (member)
  {
    ...
    {
      Xlogger->error("CSchoolM::processLeaveGroup(): %s对应节点无法取到节点管理器的对象",member->name);
    }
  }
  else
  {
  ...
  if (tIterator != memberIndex.end())
  {
    ...
    if (pSchool)
    {
      ...
      if (pSchool->insertSchoolToDB())
      {
        ...
        {
          Xlogger->error("CSchoolM::createNewSchool 无法取得门派管理器");
        }
      }
      SAFE_DELETE(pSchool);
    }
...
void CSchoolM::addMember(UserSession *master, UserSession *prentice)
{
	...
	else
	{
		...
		if (masterMember)
		{
			...
			{
				//Xlogger->trace("[师徒]增加徒弟节点[%s]失败删除师傅节点[%s]", master->name, prentice->name);
		...
		{
			master->sendSysChat(Cmd::INFO_TYPE_FAIL, "请通知GM查证，暂时无法为你办理徒弟招收事宜");
			//Xlogger->error("无法在School模块中再添加节点addMember方法失败");
			return;
		}
	}
}
...
int CSchoolM::fireSchoolMember(const char* roleName,bool find)
{
  struct findList : public execEntry<CSchool>
  {
    ...
    void removeSchoolMember()
    {
      if (_status == 1)
      {
        Xlogger->info("%s 是师门成员,解除其社会关系",_roleName);
      ...
      if (_status == 3)
      {
        Xlogger->debug("%s 是师尊,解除其社会关系",_roleName);
    ...
    bool exec(CSchool *pSchool)
    {
      if (pSchool)
      {
        ...
        {
#ifdef _DEBUG
          Xlogger->debug("%s是师尊,解除社会关系",_roleName);
#endif
          _pSchool = pSchool;
          _status = 3;
          return false;
        ...
        else
        {
          ...
          {
#ifdef _DEBUG
            Xlogger->debug("%s 是 %s 师门成员,能解除社会关系",_roleName,pSchool->getSchoolName());
#endif
            _pSchool = pSchool;
            _status = 1;
            return false;
          ...
          {
#ifdef _DEBUG
            Xlogger->debug("%s 不是 %s 的师门成员",_roleName,pSchool->getSchoolName());
#endif
            _status = 2;
          }
        }
          ...
          case Cmd::SCHOOL_BULLETIN_SET:
            {
              ...
              bzero(buf,x_socket::MAX_DATASIZE);
...
bool CSchool::addNode(const stSchoolMemberInfo &info)
{
  ...
  if (member)
  {
    ...
    if (member->havePreNode())
    {
      ...
      if (pPreNode)
      {
        ...
        {
          SAFE_DELETE(member);
          Xlogger->error("%s节点管理器加入%s节点失败id为%u",name,info.name,info.dwSerialID);
        }
      }
      else
      {
        SAFE_DELETE(member);
        Xlogger->error("%s节点管理器中%s节点寻找师傅节点%u失败",name,info.name,info.dwMasterID);
      }
    }
    else
    {
  ...
  {
    Xlogger->error("初始化的时候内存分配失败 CSchool::addNode");
  }
  return false;
}
...
bool CSchool::insertSchoolToDB()
{
  ...
  if ((DWORD)-1 == retcode)
  {
    Xlogger->error("插入SCHOOL表数据库出错 name = %s",schooldata.name);
    return false;
  }
  else
  {
...
bool CSchool::deleteSchoolFromDB()
{
  ...
  if ((DWORD)-1 == retcode)
  {
    Xlogger->error("删除SCHOOL记录失败 SCHOOLID=%u",id);
    return false;
  }
  destroy = true;
  return true;
...
CSchoolMember * CSchool::addTeacher(UserSession *master)
{
  ...
  {
    Xlogger->error("CSchool::addTeacher():内存分配失败,无法创建新的成员节点");
  }
  return member;
}
...
bool CSchool::processLeaveSchool(CSchoolMember * member,bool deleteTeacher)
{
  if (member)
  {
    ...
    if (member->getSchoolID() == 0)
    {
      Xlogger->info("[师徒]processLeaveSchool(%s)",member->name);
  ...
  if (prenticeList.size() > getPrenticeCount())
  {
    ...
    if (member)
    {
      ...
      if (member->updateRecord())
      {
        ...
        if (!retval.second)
        {
          Xlogger->error("[师徒]: %s 添加 %s 徒弟节点失败(%u)--A",this->name,member->name,member->getJoinTime());
        }
    ...
    {
      Xlogger->error("CSchoolMember::addPrentice():%s的无效下级节点存在问题,请查证",name);
    }
  }
  else
  {
    ...
    if (member)
    {
      ...
      if (member->insertRecord())
      {
        ...
        if (!retval.second)
        {
          Xlogger->error("[师徒]: %s 添加 %s 徒弟节点失败(%u)--B",this->name,member->name,member->getJoinTime());
          member->deleteRecord();
          SAFE_DELETE(member);
        }
        else
    ...
    {
      Xlogger->error("CSchoolMember::addPrentice():内存分配失败,无法创建新的成员节点");
    }
  }
...
{
  Xlogger->info("[师徒]节点被移除数据库记录被删掉 Name=[%s] CharID=[%u] MasterID=[%u] PreSerialID=[%u]",this->name,this->dwCharID,this->dwMasterID,this->dwPreSerialID);
...
bool CSchoolMember::insertRecord()
{
  ...
  if ((DWORD)-1 == retcode)
  {
    Xlogger->error("插入SCHOOLMEMBER数据库出错 %u,%s",dwCharID,name);
    return false;
  }
...
bool CSchoolMember::updateRecord(bool locked)
{
  ...
  if (retcode > 1)
  {
    Xlogger->error("修改门派成员档案失败：serialid = %u,retcode = %u",id,retcode);
    return false;
  }
  return true;
}
...
bool CSchoolMember::deleteRecord()
{
  ...
  if ((DWORD)-1 == retcode)
  {
    Xlogger->error("删除门派SCHOOLMEMBER记录失败 SERIALID=%u",id);
    return false;
  }
  return true;
}
...
bool CSchoolMember::addNextLevelNode(CSchoolMember *member)
{
  ...
  if (!retval.second)
  {
    Xlogger->error("[师徒]: %s 添加 %s 徒弟节点失败(%u)",this->name,member->name,member->getJoinTime());
  }
    ...
    if (member)
    {
      ...
      if (member->getTeacher() == this) // 如果此人的师傅是本节点
      {
        if (school->processLeaveSchool(member,false))
        {
          ...
          //school->sendCmdToSchool(&send,sizeof(send));
          Xlogger->info("[师徒]%s被%s逐出师门,剩余徒弟数Count=[%u] Size=[%u]",prenticeName,name,
