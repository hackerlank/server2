...
{
  Xlogger->info("用户%s(%u)游戏时间:%u分钟",name,id,(regTime.elapse(SessionTimeTick::currentTime)/1000)/60); 
  ...
  Xlogger->debug("Session目前在线人数:%u",--user_count);
    ...
    if (NULL != rel)
    {
      ...
      switch(rel->type)
      {
        ...
        case Cmd::RELATION_TYPE_FRIEND:
          ...
          break;
        default:
          Xlogger->error("发送好友关系有不能识别的类型 %u",rel->type);
          break;
      }
