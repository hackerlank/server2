    ...
    case PARA_SCENE_SENDMAIL:
      {
        ...
        {
          Xlogger->error("[邮件]doMailCmd(PARA_SCENE_SENDMAIL): 发送邮件时未找到发送者 %s",rev->mail.fromName);
          return true;
        }
        */
        ...
        if (sendMail(* rev))
        {
          ...
          Xlogger->info("[邮件]邮件发送成功 %s->%s(%u)",rev->mail.fromName,rev->mail.toName,rev->mail.toID);
    ...
    case PARA_SCENE_GET_MAIL_LIST:
      {
        ...
        if (!pUser)
        {
          Xlogger->error("[邮件]doMailCmd(PARA_SCENE_GET_MAIL_LIST): 取得邮件列表时未找到玩家");
          return true;
        }
        ...
        {               
          ...
          Xlogger->error("[邮件]doMailCmd(PARA_SCENE_GET_MAIL_LIST): 得到数据库句柄失败");
          return true;
        }
    ...
    case PARA_SCENE_OPEN_MAIL:
      {
        ...
        if (!pUser)
        {
          Xlogger->error("[邮件]doMailCmd(PARA_SCENE_OPEN_MAIL): 打开邮件时未找到玩家");
          return true;
        }
        ...
        {               
          ...
          Xlogger->error("[邮件]doMailCmd(PARA_SCENE_OPEN_MAIL): 得到数据库句柄失败");
          return true;
        }
...
#ifdef _DEBUG
        //Xlogger->debug("[邮件]%s 打开邮件 id=%u 物品:thisID=%u ObjectID=%u",pUser->name,cm.mailID,cm.item.qwThisID,cm.item.dwObjectID);
#endif
    ...
    case PARA_SCENE_GET_MAIL_ITEM:
      {
        ...
        if (!pUser)
        {
          Xlogger->error("[邮件]doMailCmd(PARA_SCENE_GET_MAIL_ITEM): 获取附件时未找到玩家");
          return true;
        }
        ...
        {               
          ...
          Xlogger->error("[邮件]doMailCmd(PARA_SCENE_GET_MAIL_ITEM): 得到数据库句柄失败");
          return true;
        }
    ...
    case PARA_SCENE_GET_MAIL_ITEM_CONFIRM:
      {
        ...
        if (!pUser)
        {
          Xlogger->error("[邮件]doMailCmd(PARA_SCENE_GET_MAIL_ITEM_CONFIRM): 确认获取附件时未找到玩家");
          return true;
        }
        */
        ...
        if ((connHandleID)-1 == handle)
        {               
          Xlogger->error("[邮件]doMailCmd(PARA_SCENE_GET_MAIL_ITEM_CONFIRM): 得到数据库句柄失败");
          return true;
        }
        ...
        {
          if (retcode!=0)
        Xlogger->error("[邮件]%s 确认获取附件错误 mailID=%u retCode=%d",mail.toName,rev->mailID,retcode);
          SessionService::dbConnPool->putHandle(handle);
          return true;
        }
        ...
        if (mail.recvMoney)
          ...
          {
            Xlogger->error("[邮件]%s 支付银子失败 mailID=%u",mail.toName,rev->mailID);
          }
        //BYTE i = 1;
    ...
    case PARA_SCENE_DEL_MAIL:
      {
        ...
        if (!pUser)
        {
          Xlogger->error("[邮件]doMailCmd(PARA_SCENE_DEL_MAIL): 删除邮件时未找到玩家");
          return true;
        }
        ...
        if ((connHandleID)-1 == handle)
        {               
          Xlogger->error("[邮件]doMailCmd(PARA_SCENE_DEL_MAIL): 得到数据库句柄失败");
          return true;
        }
        ...
        if (1 != retcode)
          Xlogger->error("[邮件]删除邮件失败：mailID=%u,retcode=%u",rev->mailID,retcode);
        stDelMail dm;
        dm.mailID = rev->mailID;
        pUser->sendCmdToMe(&dm,sizeof(dm));
        Xlogger->info("[邮件]%s 删除邮件 mailID=%u",pUser->name,rev->mailID);
        SessionService::dbConnPool->putHandle(handle);
    ...
    case PARA_SCENE_TURN_BACK_MAIL:
      {
        ...
        if (!pUser)
        {
          Xlogger->error("[邮件]doMailCmd(PARA_SCENE_TURN_BACK_MAIL): 退回邮件时未找到玩家");
          return true;
        }
        ...
        if ((connHandleID)-1 == handle)
        {
          ...
          Xlogger->error("[邮件]doMailCmd(PARA_SCENE_TURN_BACK_MAIL): 得到数据库句柄失败");
          return true;
        }
        ...
        if (turnBackMail(rev->mailID))
        {
          ...
          dm.mailID = rev->mailID;
          pUser->sendCmdToMe(&dm,sizeof(dm));
        Xlogger->info("[邮件]%s 退回 %s 的邮件 mailID=%u",pUser->name,info.fromName,rev->mailID);
        }
        return true;
      }
      break;
...
bool MailService::sendMail(DWORD h,Cmd::Session::t_sendMail_SceneSession & sm)
{
  ...
  {
    Xlogger->error("[邮件]sendMoneyMail 收件人为空 fromName=%s money=%u text=%s type=%u",sm.mail.fromName,sm.mail.sendMoney,sm.mail.text,sm.mail.type);
    return false;
  }
  ...
  if ((connHandleID)-1 == handle)
  {               
    Xlogger->error("[邮件]sendMail: 无效的数据库句柄");
    Xlogger->error("[邮件]%s->%s 丢失 money=%u item=%s",sm.mail.fromName,sm.mail.toName,sm.mail.sendMoney,sm.item.object.strName);
    return false;
  }
  ...
  if ((DWORD)-1 == retcode)
  {
    Xlogger->error("[邮件]sendMail: 插入新邮件数据库出错 retcode=%d",retcode);
    Xlogger->error("[邮件]%s->%s 丢失 money=%u item=%s",sm.mail.fromName,sm.mail.toName,sm.mail.sendMoney,sm.item.object.strName);
    return false;
  }
...
bool MailService::sendMail(Cmd::Session::t_sendMail_SceneSession & sm)
{
  ...
  if ((connHandleID)-1 == handle)
  {               
    Xlogger->error("[邮件]sendMail: 得到数据库句柄失败");
    Xlogger->error("[邮件]%s->%s 丢失 money=%u item=%s",sm.mail.fromName,sm.mail.toName,sm.mail.sendMoney,sm.item.object.strName);
    return false;
  }
  DWORD retcode = SessionService::dbConnPool->exeInsert(handle,"`MAIL`",mail_define,(const BYTE *)&sm.mail);
...
void MailService::checkDB()
{
  ...
  if ((connHandleID)-1 == handle)
  {               
    Xlogger->error("[邮件]MailService::checkDB: 得到数据库句柄失败");
    return;
  }
  ...
  if ((DWORD)-1 == retcode)
  {
    //Xlogger->error("[邮件]MailService::checkDB: 转发过期邮件失败 retCode=%d",retcode);
    SessionService::dbConnPool->putHandle(handle);
    return;
  }
  else
    //Xlogger->error("[邮件]MailService::checkDB: 转发 %d 封过期邮件",retcode);
  */
  ...
  if ((DWORD)-1 == retcode)
  {
    Xlogger->error("[邮件]MailService::checkDB: 删除过期邮件失败 retCode=%d",retcode);
    SessionService::dbConnPool->putHandle(handle);
    return;
  }
  else
    if (retcode) Xlogger->info("[邮件]MailService::checkDB: 删除 %d 封过期邮件",retcode);
  ...
  if ((DWORD)-1 == retcode)
  {
    Xlogger->error("[邮件]MailService::checkDB: 常规检查失败 retCode=%d",retcode);
    SessionService::dbConnPool->putHandle(handle);
    return;
  }
  ...
  if (retcode) Xlogger->debug("[邮件]删除 %u 封10天之前的邮件记录",retcode);
...
bool MailService::turnBackMail(DWORD mailID)
{
  ...
  if ((connHandleID)-1 == handle)
  {               
    Xlogger->error("[邮件]turnBackMail: 得到数据库句柄失败");
    return false;
  }
  ...
  if (1 != retcode
      ...
      //|| 0==mail.fromID)
  {
    Xlogger->error("[邮件]不能退还邮件 mailID=%u retCode=%d",mailID,retcode);
    SessionService::dbConnPool->putHandle(handle);
    return false;
  }
  ...
  if (1 != retcode)
  {
    Xlogger->error("[邮件]返还邮件Update失败：mailID=%u,retcode=%d",mailID,retcode);
    return false;
  }
...
void MailService::delMailByNameAndID(char * userName,DWORD id)
{
  ...
  if ((connHandleID)-1 == handle)
  {
    Xlogger->error("[邮件]delMailbyName: 得到数据库句柄失败 name=%s",userName);
    return;
  }
  ...
  if ((DWORD)-1 == retcode)
  {
    Xlogger->error("[邮件]删除玩家所有邮件失败 retCode=%d",retcode);
    SessionService::dbConnPool->putHandle(handle);
    return;
  }
  ...
  SessionService::dbConnPool->putHandle(handle);
  if ((DWORD)-1 == retcode)
    Xlogger->error("[邮件]delMailbyName:删除角色所有邮件Update失败：userName=%s,retcode=%d",userName,retcode);
  else
    Xlogger->info("[邮件]delMailbyName:删除角色所有邮件：userName=%s,retcode=%d",userName,retcode);
}
...
void MailService::loadNewMail()
{
  ...
  if ((connHandleID)-1 == handle)
  {
    Xlogger->error("[邮件]loadNewMail: 得到数据库句柄失败");
    return;
  }
  ...
  Xlogger->debug("[邮件]loadNewMail %u组 %u个",newMailMap.size(),retcode);
}
