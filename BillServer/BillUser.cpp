...
BillUser::BillUser(DWORD acc,DWORD logintemp,const char *count,const DWORD ip,shared_ptr<BillTask> gate)
{
	id=acc;
	tempid=logintemp;
...
void BillUser::increaseGoldListNum()
{
  ...
  if (goldlistNum <10)
  {
    ...
    if ((connHandleID)-1 != handle)
    {
      ...
      //Xlogger->debug("%u交易金子单数量:%d",this->id,goldlistNum);
...
void BillUser::increaseMoneyListNum()
{
  ...
  if (moneylistNum <10)
  {
    ...
    if ((connHandleID)-1 != handle)
    {
      ...
      //Xlogger->debug("%u交易银子单数量:%d",this->id,moneylistNum);
      BillService::dbConnPool->putHandle(handle);
    }
  }
  // */
...
bool BillUser::checkStockLogin()
{
	...
	{
		Cmd::stReturnPasswordStockUserCmd ret; 
		ret.byReturn = Cmd::STOCK_LOGIN_NOTLOGIN;
		this->sendCmdToMe(&ret,sizeof(ret));
		Xlogger->debug("%s(%d)please login first",this->account,this->id);
	}
	return false;
}
  ...
  if ((connHandleID)-1 != handle)
  {
    ...
    if (restore)
    {
      ...
      if (recordset && !recordset->empty())
      {
        ...
        {
          Xlogger->debug("BillUser::restorecard 数据库错误!");
        }      
        else
        {
  ...
  if ((connHandleID)-1 != handle)
  {
    ...
    if (restore)
    {
      ...
      if (recordset && !recordset->empty())
      {
        ...
        if ((DWORD)-1 == BillService::dbConnPool->exeDelete(handle,restore,&where))
        {
          Xlogger->debug("BillUser::restoregold数据库错误!");
        }      
        else
        {
          strncpy(send.account,account,Cmd::UserServer::ID_MAX_LENGTH);
...
bool BillUser::check_tid(const char *t)
{
  ...
  {
    Xlogger->debug("%d兑换金币返回时没有正确的tid(%s,%s),可能该玩家已经退出",this->id,this->tid,t);
    return false;
  }
  return true;
}
...
bool BillUser::redeem_object_card_err(const BillData* bd)
{
  ...
  if ((connHandleID)-1 != handle)
  {
    ...
    if (bd->result == Cmd::UserServer::RET_OK)
    {
      ...
      {
        Xlogger->debug("%d,%s,添加到补偿道具表失败",bd->uid,bd->tid);
      }
      else
      {
...
bool BillUser::redeem_object_card(const BillData* bd)
{
  ...
  if (bd->result == Cmd::UserServer::RET_OK)
  {
    //Xlogger->debug("bd->subatt=%d",bd->subat);
...
bool BillUser::redeem_gold_err(const BillData* bd)
{
  ...
  if ((connHandleID)-1 != handle)
  {
    ...
    if (bd->result == Cmd::UserServer::RET_OK)
    {
      ...
      if ((DWORD)-1 == BillService::dbConnPool->exeInsert(handle,restore,&column))
      {
        Xlogger->debug("%d,%s,添加到补偿金币表失败",bd->uid,bd->tid);
      }
      else
      {
          ...
          case PARA_STOCK_SAVE:
            {
              ...
              if ((connHandleID)-1 != handle)
              {
                ...
                if (balance)
                {
                  ...
                  Xlogger->debug("%s(%d)股票冲值,金币%d,银币%d",this->account,this->id,rev->dwGold,rev->dwMoney);
                }
                BillService::dbConnPool->putHandle(handle);
              }
              // */
    ...
    case STOCK_BILL_USERCMD:
      {
        switch(pNullCmd->para)
        {
          case PASSWORD_STOCKPARA:
            {
              ...
              if (this->password[0])
              {
                if (!strncmp(password,rev->byPawword,sizeof(rev->byPawword)))
                {
                    ...
                    if ((connHandleID)-1 != handle)
                    {
                      ...
                      if (balance)
                      {
                              ...
                              balance,&column,&where))
                        {
                          ...
                          Xlogger->debug("%d,%s,修改密码服务器错误",this->id,this->account);
                        }
                        else
                        {
                          ...
                          Xlogger->debug("%d,%s,修改密码成功",this->id,this->account);
                        }
                      }
                      else
                      {
                        ret.byReturn = STOCK_SERVER_WRONG;
                        Xlogger->debug("%d,%s,修改密码得到句柄错误",this->id,this->account);
                      }
                      BillService::dbConnPool->putHandle(handle);
                    }
                  }
                  else
                  {
                    if (rev->byNew1[0])
                    {
                      ...
                      Xlogger->debug("%d,%s,修改密码两次输入不一致",this->id,this->account);
                    }
                    else
                    {
                      ret.byReturn = STOCK_LOGIN_OK;
                      Xlogger->debug("%d,%s,登陆成功",this->id,this->account);
                    }
                  }
                }
                else
                {
                  ...
                  Xlogger->debug("%d,%s,登陆密码错误",this->id,this->account);
                }
              }
              else/// 新帐号
              {// 没有兑换记录,插入新的记录
                  ...
                  if (rev->byNew1[0] && !strncmp(rev->byNew1,rev->byNew2,sizeof(rev->byNew1)))
                  {
                    if (balance)
                    {
                      ...
                      if (!recordset || (recordset && recordset->empty()))
                      {
                        ...
                        {
                          ret.byReturn = STOCK_SERVER_WRONG;
                          Xlogger->debug("%d,%s,启用帐号服务器数据库插入错误",this->id,this->account);
                        }
                        ret.byReturn = STOCK_OPEN_OK;
                        strncpy(this->password,rev->byNew1,sizeof(rev->byNew1));
                        Xlogger->debug("%d,%s,启用帐号成功",this->id,this->account);
                      }
                      else
                      {
                        ret.byReturn =STOCK_EXIST;
                        Xlogger->debug("%d,%s,帐号已经启用",this->id,this->account);
                      }
                      SAFE_DELETE(recordset)
                    }
                    else
                    {
                      ret.byReturn = STOCK_SERVER_WRONG;
                      Xlogger->debug("%d,%s,启用帐号服务器错误",this->id,this->account);
                    }
                  }
                  else
                  {
                    ...
                    {
                      ret.byReturn = STOCK_DIFF;
                      Xlogger->debug("%d,%s,启用帐号密码不一致",this->id,this->account);
                    }
                    else
                    {
                      ret.byReturn = STOCK_NONE;
                      Xlogger->debug("%d,%s,启用帐号密码不能为空",this->id,this->account);
                    }
                  }
                  BillService::dbConnPool->putHandle(handle);
                }
                ...
                {
                  ret.byReturn = STOCK_SERVER_WRONG;
                  Xlogger->debug("%d,%s,启用帐号未得到服务器句柄",this->id,this->account);
                }
              }
          ...
          case QUERY_FUND_STOCKPARA:
            {
              ...
              send.dwMoney=money;
              sendCmdToMe(&send,sizeof(send));
              Xlogger->debug("%s(%d)请求股票帐号余额",this->account,this->id);
              return true;
            }
            break;
          case TRANSFER_FUND_FETCH_STOCKPARA:
            {
              ...
              {
                Xlogger->debug("%s(%d)股票套现数值错误,可能是外挂,金币%d,银币%d",this->account,this->id,rev->dwGold,rev->dwMoney);
                return true;
              }
              /*
                 connHandleID handle = BillService::dbConnPool->getHandle();
              ...
              send.dwMoney=money;
              sendCmdToMe(&send,sizeof(send));
              //Xlogger->debug("%s(%d)股票套现,金币%d,银币%d",this->account,this->id,rev->dwGold,rev->dwMoney);
              return true;
            }
            break;
          case PUT_LIST_STOCKPARA:
            {
              ...
              Xlogger->debug("%s(%d)委托单,类型%s,数量%d,价格:%d",this->account,this->id,rev->byType==STOCK_MONEY?"银币":"金币",rev->dwNum,rev->dwPrice);
              return true;
            }
            break;
            /// 请求自己未成交委托单
          ...
          case REQUEST_SELF_HISTORY_STOCKPARA:
            {
              Xlogger->debug("%u请求自己历史数据",this->id);
              if (!checkStockLogin())
              {
                return true;
              }
...
{
  Xlogger->debug("%s:提交%s单,数量%d,价格:%d",this->account,type?"money":"gold",num,price);
  if (!num || !price)
  {
    return false;
  }
  ...
  if ((connHandleID)-1 != handle)
  {
    ...
    if (bret)
    {
      ...
      if (balance)
      {
        ...
        else
        {
          ...
          {
            Xlogger->debug("%s(%d)未识别的股票类型%d",this->account,this->id,type);
          }
        }
      }
    }
    ...
    {
      Xlogger->debug("%s(%d)扣除费用失败或者交易单位不正确",this->account,this->id);
    }
    BillService::dbConnPool->putHandle(handle);
  }
  if (bret)
...
bool BillUser::removeGold(DWORD num,const char *disc,bool transfer,bool tax)
{
  ...
  if (num > this->gold)
  {
    Xlogger->debug("%s(%d)扣除金币失败,需要%d,现有%d,描述:%s",this->account,this->id,num,this->gold,disc);
    return bret;
  }
  connHandleID handle = BillService::dbConnPool->getHandle();
  if ((connHandleID)-1 != handle)
...
bool BillUser::removeMoney(DWORD num,const char *disc,bool transfer,bool tax)
{
  ...
  if (num > this->money)
  {
    Xlogger->debug("%s(%d)扣除银币失败,需要%d,现有%d,描述:%s",this->account,this->id,num,this->money,disc);
    return bret;
  }
  connHandleID handle = BillService::dbConnPool->getHandle();
  if ((connHandleID)-1 != handle)
...
{
  //Xlogger->debug("ClientIp:%s",client_ip);
  return client_ip;
}
