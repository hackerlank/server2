      ...
      if (history)
      {
        ...
        if (recordset && !recordset->empty())
        {
          for(DWORD i=0; i < recordset->size(); i++)
          {
            ...
            //Xlogger->debug("%s(%d)未成交的个人金币委托单:%d,%d,%d,%d",pUser->name,pUser->id,waitgold->list[waitgold->size].id,waitgold->list[waitgold->size].dwNum,waitgold->list[waitgold->size].dwPrice,waitgold->list[waitgold->size].dwTime);
            waitgold->size++;
          }
          if (waitgold->size)
          {
      ...
      if (history)
      {
        ...
        if (recordset && !recordset->empty())
        {
          for(DWORD i=0; i < recordset->size(); i++)
          {
            ...
            //Xlogger->debug("%s(%d)未成交的个人银币委托单:%d,%d,%d,%d",pUser->name,pUser->id,waitmoney->list[waitmoney->size].id,waitmoney->list[waitmoney->size].dwNum,waitmoney->list[waitmoney->size].dwPrice,waitmoney->list[waitmoney->size].dwTime);
            waitmoney->size++;
          }
          if (waitmoney->size)
          {
...
bool Consign::updateHistory(DWORD id,DWORD acc,DWORD num,DWORD commitprice,DWORD price,DWORD comtime,bool type,DWORD sysmoney)
{
  ...
  if ((connHandleID)-1 != handle)
  {
    ...
    {
      Xlogger->debug("Consign::updateHistory数据库错误!");
      BillService::dbConnPool->putHandle(handle);
      return false;
    }      
    else
    ...
    if (canceltbl)
    {
      ...
      if (cancelset && !cancelset->empty())
      {
        for(DWORD i=0; i < cancelset->size() ; i++)
        {
          ...
          {
            Xlogger->debug("Consign::cancelListAll数据库错误!");
            BillService::dbConnPool->putHandle(handle);
            return false;
          }      
          else
    ...
    if ((connHandleID)-1 != handle)
    {
      ...
      if (canceltbl)
      {
        ...
        if (cancelset && !cancelset->empty())
        {
          ...
          if ((DWORD)-1 == BillService::dbConnPool->exeDelete(handle,canceltbl,&where))
          {
            Xlogger->debug("ConsignGoldManager::cancelList 数据库错误!");
            BillService::dbConnPool->putHandle(handle);
            return false;
          }      
          else
        ...
        {
          Xlogger->debug("%s(%d)撤销不存在的金币委托单:%d",pUser->account,pUser->id,listid);
        }
      }
      BillService::dbConnPool->putHandle(handle);
      ConsignHistoryManager::getInstance()->update();
    ...
    if (goldtbl)
    {
      ...
      if (goldset && !goldset->empty())
      {
        ...
        if (list.num && list.num < changegold)
        {
          ...
          {
            Xlogger->debug("ConsignGoldManager::trade 数据库错误!");
          }
          else
          {
    ...
    if ((connHandleID)-1 != handle)
    {
      ...
      if (canceltbl)
      {
        ...
        if (cancelset && !cancelset->empty())
        {
          ...
          if ((DWORD)-1 == BillService::dbConnPool->exeDelete(handle,canceltbl,&where))
          {
            Xlogger->debug("ConsignMoneyManager::cancelList 数据库错误!");
            BillService::dbConnPool->putHandle(handle);
            return false;
          }      
          else
    ...
    if (moneytbl)
    {
      ...
      if (moneyset && !moneyset->empty())
      {
        ...
        if (list.num && list.num < changemoney && list.num >= list.price)
        {
          ...
          {
            Xlogger->debug("ConsignMoneyManager::trade 数据库错误!");
          }
          else
          {
    ...
    if (history)
    {
      ...
      if (recordset && !recordset->empty())
      {
        for(DWORD i=0; i < recordset->size() && i < 1024000; i++)
        {
          ...
          //Xlogger->debug("历史金币数据:(时间:%d,价格:%d,成交量:%d)",trait.dwTime,trait.dwPrice,trait.dwTotal);
    ...
    if (history)
    {
      ...
      if (recordset && !recordset->empty())
      {
        for(DWORD i=0; i < recordset->size() && i < 1024000; i++)
        {
          ...
          trait.dwPrice = recordset->get(i)->get("price");
          trait.dwTime = recordset->get(i)->get("oktime");
          //Xlogger->debug("历史银币数据:(时间:%d,价格:%d,成交量:%d)",trait.dwTime,trait.dwPrice,trait.dwTotal);
    ...
    if ((connHandleID)-1 != handle)
    {
      ...
      if (history)
      {
        ...
        if (recordset && !recordset->empty())
        {
          ...
          //Xlogger->debug("最新历史数据:(时间:%d,价格:%d,成交量:%d",newHistoryGold.dwTime,newHistoryGold.dwPrice,newHistoryGold.dwTotal);
        }
      }
      SAFE_DELETE(recordset);
      history = BillService::metaData->getFields("CONSIGNMONEYHISTORY");
      ...
      if (history)
      {
        ...
        if (recordset && !recordset->empty())
        {
          ...
          //Xlogger->debug("最新历史数据:(时间:%d,价格:%d,成交量:%d",newHistoryMoney.dwTime,newHistoryMoney.dwPrice,newHistoryMoney.dwTotal);
        }
      }
      SAFE_DELETE(recordset);
      BillService::dbConnPool->putHandle(handle);
  ...
  if (pUser)
  {
    ...
    if (num && num<=720)
    {
      ...
      for(DWORD i = 0; i < num ; i++)
      {
        ...
        {
          //Xlogger->debug("玩家请求金币历史数据:(时间:%d,价格:%d,成交量:%d",iter->second.dwTime,iter->second.dwPrice,iter->second.dwTotal);
      ...
      for(DWORD i = 0; i < num ; i++)
      {
        ...
        for(iter = range.first;iter !=range.second;iter++)
        {
          //Xlogger->debug("玩家请求银币历史数据:(时间:%d,价格:%d,成交量:%d",iter->second.dwTime,iter->second.dwPrice,iter->second.dwTotal);
...
bool ConsignHistoryManager::sendSelfDataToUser(BillUser *pUser,DWORD begintime,DWORD num)
{
  if (pUser)
  {
        ...
        if (history)
        {
          ...
          if (recordset && !recordset->empty())
          {
            for(DWORD i=0; i < recordset->size() && i < num * 60; i++)
            {
              ...
              //Xlogger->debug("玩家%s(%d)请求自己金币历史数据:%d,%d,%d,%d,%d",pUser->account,pUser->id,sendgold->list[sendgold->size].dwID,sendgold->list[sendgold->size].wdNum,sendgold->list[sendgold->size].wdCommitPrice,sendgold->list[sendgold->size].wdPrice,sendgold->list[sendgold->size].dwCommitTime,sendgold->list[sendgold->size].dwOkTime);
              sendgold->size++;
            }
            SAFE_DELETE(recordset);
          }
    ...
    {
      Xlogger->debug("%u请求自己历史数据数量错误:%d",pUser->id,num);
    }
  }
  return true;
}
