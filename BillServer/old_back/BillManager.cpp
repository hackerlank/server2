...
void BillManager::update()
{
  ...
  if (!infoMap.empty())
  {
    ...
    for(BillInfoMap_iterator it = infoMap.begin(); it != infoMap.end();)
    {
      ...
      {
        Xlogger->debug("等待网关登陆信息超时：%u,%u,%u",it->second.accid,it->second.loginTempID,it->second.wdGatewayID);
...
void BillManager::updateByGatewayID(const WORD wdGatewayID)
{
  ...
  if (!infoMap.empty())
  {
    for(BillInfoMap_iterator it = infoMap.begin(); it != infoMap.end();)
    {
      ...
      {
        Xlogger->debug("网关关闭,清除登陆数据：%u,%u,%u",it->second.accid,it->second.loginTempID,it->second.wdGatewayID);
        BillInfoMap_iterator tmp = it;
        it++;
        infoMap.erase(tmp);
      }
