...
BillClient::BillClient(
    ...
    const WORD port,
    BillCallback &bc,
    const DWORD my_id) : x_tcp_clientTask(ip,port),bc(bc),netType(NetType_near),my_id(my_id),gameZone()
{
...
int BillClient::checkRebound()
{
  ...
  if (retcode > 0)
  {
    ...
    else
    {
      ...
      {
        Xlogger->debug("登陆平台计费服务器成功,收到区的编号：%u(%u,%u),%s,%u",
            ptCmd->gameZone.id,
            ptCmd->gameZone.game,
            ptCmd->gameZone.zone,
            ptCmd->name,
        ...
        Xlogger->debug("%s",gameZone_str);
        return 1;
      }
      else
      {
        Xlogger->debug("登陆平台计费服务器失败");
        return -1;
      }
    }
  }
...
bool BillClient::cmdMsgParse(const Cmd::t_NullCmd *pNullCmd,const DWORD nCmdLen)
{
    ...
    case CMD_USER_RET:
      {
        ...
        {
          //TODO FIXME
          Xlogger->debug("tradeSN not found,%s",pCmd->tid);
        }
        return retval;
      }
      break;
  ...
  Xlogger->debug("BillClient::cmdMsgParse(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
  return false;
}
