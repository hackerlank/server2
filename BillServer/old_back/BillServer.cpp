    ...
    case PARA_BILL_NEWSESSION:
      {
        ...
        if (!task)
        {
          Xlogger->error("账号%d登陆时网关已经关闭",ptCmd->session.accid);
          t_idinuse_Bill ret;
        ...
        {
          //重复登陆验证
          Xlogger->error("账号已经登陆%s(%d,%d)",ptCmd->session.account,ptCmd->session.accid,ptCmd->session.loginTempID);
          t_idinuse_Bill ret;
          ret.accid = ptCmd->session.accid;
          ret.loginTempID = ptCmd->session.loginTempID;
          ret.wdLoginID = ptCmd->session.wdLoginID;
        ...
        Xlogger->info("帐号登陆%s(%d,%d)",pUser->account,pUser->id,pUser->tempid);
...
bool redeem_moth_card(const BillData* bd)
{
  ...
  if (!bs.accid)
  {
    Xlogger->debug("%s兑换金币返回时没有正确的BillSession,可能该玩家已经退出",bd->tid);
    return false;
  }
