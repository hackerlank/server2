...
void GateUser::refreshCharInfo()
{
	...
	//if (ret)
	//{
	//	if (size >  0 && size <= (int)(x_socket::MAX_DATASIZE - sizeof(stUserInfoUserCmd) - 100))
	//	{
	//		//Xlogger->debug("生成图形验证码：%s",jpegPassport);
	//		cmd->size = size;
...
bool GateUser::beginSelect()
{
	...
	if (!logout && isInitState())
	{
		...
		Xlogger->error("添加帐号(%ld)失败",accid);
	...
	case CHAT_USERCMD:
		if (ALL_CHAT_USERCMD_PARAMETER == cmd->byParam)
		{
			...
			*/
			...
			bcopy(ret,cmd,cmdLen,x_socket::MAX_DATASIZE);
		}
		break;
	...
	if (!hasPacked)
	{
		...
		case CHAT_USERCMD:
			if (ALL_CHAT_USERCMD_PARAMETER == cmd->byParam)
			{
				...
				*
				*/
				BYTE buf[x_socket::MAX_DATASIZE];//={0};
...
void GateUser::reg(int charno)
{
	...
	Xlogger->fatal("reg OldMap:%s", send.OldMap);
...
void GateSelectUserSession::putSelectUserInfo(const Cmd::SelectUserInfo &info)
{
	for(int i = 0; i < Cmd::MAX_CHARINFO; i++)
	{
		if (userinfo[i].id==0 || userinfo[i].id==(DWORD)-1)
		{
			...
			Xlogger->info("id=%d name=%s level=%d mapName=%s",
