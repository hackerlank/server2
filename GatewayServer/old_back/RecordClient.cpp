...
{
#ifdef _MSGPARSE_
  Xlogger->error("?? RecordClient::msgParse(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
#endif
      ...
      case PARA_GATE_DELCHAR_RETURN:
        {
          ...
          if (pUser && 1 == rev->retcode)
          {
            ...
            Xlogger->info("删除角色成功：%u,%u",rev->accid,rev->id);
          }
          else
            Xlogger->warn("删除角色失败：%u,%u",rev->accid,rev->id);
          return true;
        }
        break;
      case PARA_GATE_CHECKNAME_RETURN:
        {
          ...
          if (pUser)
          {
            ...
            Xlogger->debug("检查角色名:%d",ret.err_code);
      ...
      case PARA_GATE_RET_SELECTINFO:
        {
          ...
          if (pUser && pUser->tempid == 0)
          {
            ...
            if (empty)
            {
              //Xlogger->debug("帐号没有角色信息 %u",rev->accid);
              pUser->noCharInfo();
            }
            else
            {
