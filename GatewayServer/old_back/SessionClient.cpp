			...
			case PARA_GATE_UNREGUSER:
				{
					...
					if (pUser)
					{
						if (rev->retcode==UNREGUSER_RET_LOGOUT)
						{
							...
							{
								//pUser->final();
								Xlogger->info("Session请求用户(%ld,%ld,%s)注销",pUser->accid,pUser->id,pUser->name);
								pUser->Terminate();
								//SAFE_DELETE(pUser);
							}
							return true;
						...
						{
							Xlogger->error("Session请求用户%s注册失败注销",pUser->name);
							//pUser->final();
							pUser->Terminate();
							//SAFE_DELETE(pUser);
							return true;
					...
					{
						Xlogger->warn("注销时无此用户(%ld),scenetempid=%ld,注销类型%d",
...
#ifdef _DEBUG
					Xlogger->debug("删除角色状态为:%d",rev->status);
#endif            
					if (rev->status == 0)
					{
...
#define country_info GatewayService::getInstance().country_info
			...
			case PARA_FORWARD_USER:
				{
					...
					{
						//Xlogger->debug("转发会话服务器帐号%ld的%u %u消息失败",rev->dwID,((Cmd::stNullUserCmd *)rev->data)->byCmd,((Cmd::stNullUserCmd *)rev->data)->byParam);
					}
					else
					{
						//Xlogger->debug("转发会话服务器帐号%ld的%u %u消息成功",rev->dwID,((Cmd::stNullUserCmd *)rev->data)->byCmd,((Cmd::stNullUserCmd *)rev->data)->byParam);
					}
					return true;
				}
				break;
			case PARA_FORWARD_WORLD:
				{
					...
					struct WorldExec :public execEntry<GateUser>
					{
						...
						WorldExec(BYTE *data,DWORD len):_data(data),_len(len),_type(0)
						{
							...
							_sendLen = x_socket::packetPackZip(_data,_len,cmd_queue); 
						}
	...
	Xlogger->error("SessionClient::msgParse(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
	return false;
}
