...
{
  Xlogger->debug("SceneClientManager::init");
  const Cmd::Super::ServerEntry *serverEntry = GatewayService::getInstance().getServerEntryByType(SCENESSERVER);
  sceneClientPool = new x_tcp_clientTaskPool(atoi(Seal::global["threadPoolClient"].c_str()),8000);
  ...
  while(serverEntry)
  {
    ...
    if (NULL == sceneClient)
    {
      Xlogger->error("没有足够内存,不能建立场景服务器客户端实例");
      return false;
    }
