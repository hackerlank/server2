...
*
*/
class SessionClient : public x_tcp_buffer_client
{
...
*
*/
class GateSelectUserSession:private boost::noncopyable
{
...
private:
	...
	*
	*/
	GatewayService() : x_subnetservice("网关服务器",GATEWAYSERVER)
	{
		rolereg_verify = true;
		taskPool = NULL;
		startUpFinish = false;
...
*
*/
class RecordClient : public x_tcp_buffer_client
{
...
*
*/
class MiniClient : public x_tcp_buffer_client
{
...
*
*/
class BillClient : public x_tcp_buffer_client
{
...
public:
	...
	{
		//Xlogger->debug("previous:%ld,current:%ld,interval:%d,must_interval:%d",check.previous().msecs(),current.msecs(),check.previous().elapse(current),MIN_INTERVAL);
	...
	static bool check_count(const zRTime& current,CheckInfo& check)
	{
		...
		//time ok
		//Xlogger->debug("last:%ld,current:%ld,elpase:%d,must_elpase:%d",check.last().msecs(),current.msecs(),check.last().elapse(current),COUNT_TIME);
...
*
*/
class ScreenIndex :private boost::noncopyable
{
private:
	//读写锁
	zRWLock wrlock;
...
* \brief 定义场景服务器连接客户端类
**/
class SceneClient : public x_tcp_clientTask
{
...
public:
	...
	SceneClient( const std::string &name,const Cmd::Super::ServerEntry *serverEntry)
		: x_tcp_clientTask(serverEntry->pstrIP,serverEntry->wdPort)
	{
		wdServerID=serverEntry->wdServerID;
	};
	~SceneClient()
	{
		Xlogger->debug("SceneClient析构");
	}
...
*
*/
class SceneClient : public x_tcp_buffer_client
{
...
public:
	...
	*/
	SceneClient( const std::string &name,const Cmd::Super::ServerEntry *serverEntry)
		: x_tcp_buffer_client(name,serverEntry->pstrIP,serverEntry->wdPort,false,8000)
	{
		wdServerID=serverEntry->wdServerID;
	};
...
private:
	...
	** \brief 客户端连接管理池
	**/
	x_tcp_clientTaskPool *sceneClientPool;
	/**
	** \brief 进行断线重连检测的时间记录
	**/
	zTime actionTimer;
