...
{
#ifdef _MSGPARSE_
  Xlogger->error("?? MiniClient::msgParse(%u,%u,%u)",pNullCmd->cmd,pNullCmd->para,nCmdLen);
#endif
      ...
      case PARA_MINI_FORWARD_USER:
        {
          ...
          if (!pUser || !pUser->sendCmd(rev->data,rev->size))
            Xlogger->debug("转发MINI服务器 %u的%u %u消息失败",rev->id,((Cmd::stNullUserCmd *)rev->data)->byCmd,((Cmd::stNullUserCmd *)rev->data)->byParam);
          return true;
        }
        break;
        /*
           ...
           {
           Xlogger->debug("转发MINI服务器帐号%ld的场景%u %u消息失败",rev->id,
