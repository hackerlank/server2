		...
		if (countryNode)
		{
			...
			while(subnode)
			{
				if (strcmp((char*)subnode->name,"country") == 0)
				{
					...
					Xlogger->info("重新读取国家名称(%d,%s,%d,%d)",info.id,info.name,info.mapid,info.function);
					CountryMap_iter iter = country_dic.find(info.id);
					if (iter !=country_dic.end())
					{
						Xlogger->debug("reload国家字典成功%s",info.name);
					...
					for(StrVec_iterator iter_1 = country_info.begin() ; iter_1 != country_info.end() ; iter_1 ++)
					{
						...
						{
							Xlogger->debug("reload国家信息成功%s",info.name);
		...
		if (mapNode)
		{
			...
			while(subnode)
			{
				if (strcmp((char*)subnode->name,"map") == 0)
				{
					...
					Xlogger->info("加载地图名称(%d,%s,%s,%d)",info.id,info.name,info.filename,info.backto);
		...
		if (countryNode)
		{
			...
			while(subnode)
			{
				if (strcmp((char*)subnode->name,"country") == 0)
				{
					...
					Xlogger->info("加载国家名称(%d,%s,%d,%d,%d)",info.id,info.name,info.mapid,info.function,info.type);
					...
					if (info.mapid)
					{
						...
						{
							Xlogger->error("得到%d地图名称失败",info.mapid);
							continue;
						}
						inited = true;
