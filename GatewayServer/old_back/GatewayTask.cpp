...
lastCheckTime(GatewayTimeTick::currentTime)
{
	...
	Xlogger->debug("GatewayTask::GatewayTask");
	recycle_wait = 5000;
	...
	//mSocket->enc.set_key_rc5((const BYTE *)Seal::global["rc5_key"].c_str(),16,12);
...
{
	Xlogger->debug("GatewayTask::verifyVersion");
	using namespace Cmd;
		...
		&& USER_VERIFY_VER_PARA == pNullCmd->byParam)
	{
		...
		{
			Xlogger->debug("客户端连接通过版本号验证[%u]",ptCmd->version);
			return true;
		}
		else
		{
			Xlogger->debug("客户端连接没有通过版本号验证[%u][%u]",ptCmd->version,GatewayService::getInstance().verify_client_version);
			return false;
		}
	}

	Xlogger->error("客户端连接没有通过版本号验证");
	return false;
}
		...
		&& PASSWD_LOGON_USERCMD_PARA == pNullCmd->byParam)
	{
		...
		if (LoginSessionManager::getInstance().verify(ptCmd->loginTempID,ptCmd->dwUserID,numPassword,&des_key))
		{
			...
			if (numPassword[0])
			{
				...
				Xlogger->debug("[财保]: %s 数字密码为: %u",account,numPwd);
			}
			...
			//Xlogger->debug("设置des密钥：%u %u %u %u %u %u %u %u",des_key[0],des_key[1],des_key[2],des_key[3],des_key[4],des_key[5],des_key[6],des_key[7]);

			//Xlogger->debug("客户端连接同步验证成功,等待计费服务器返回");
			return true;
		}
	}

	Xlogger->error("客户端连接同步验证失败");
	return false;
}
...
{
	Xlogger->debug("GatewayTask::verifyConn");
...
void GatewayTask::Terminate(const TerminateMethod method)
{
	if (pUser)
	{
		Xlogger->info("用户Terminate(%ld,%ld,%s)注销,ip=%s",pUser->accid,pUser->id,pUser->name,getIP());
...
void GatewayTask::addToContainer()
{
	...
	if (pUser)
	{
		...
		cmd.qwGameTime = Seal::qwGameTime;
		if (!qwGameTime)
		{
			qwGameTime = Seal::qwGameTime; 
			GameTimeSyn.now();
		}
...
bool GatewayTask::forwardBillScene(const Cmd::stNullUserCmd *pNullCmd,const DWORD nCmdLen)
{
	if (pUser)
	{
		...
		//Xlogger->debug("转发%ld的消息到%ld场景",pUser->id,pUser->sceneTempID);
		return true;
	}
	return false;
}
...
bool GatewayTask::forwardMini(const Cmd::stNullUserCmd *pNullCmd,const DWORD nCmdLen)
{
	if (pUser)
	{
		...
		Xlogger->debug("转发%ld的消息到Mini服务器",pUser->id);
		return true;
	}   
	return false;
}
...
bool GatewayTask::forwardSession(const Cmd::stNullUserCmd *pNullCmd,const DWORD nCmdLen)
{
	if (pUser)
	{
		...
		//Xlogger->debug("转发%ld的消息到会话服务器",pUser->id,pUser->sceneTempID);
		return true;
	}
	return false;
}
...
bool GatewayTask::msgParse_Select(const Cmd::stNullUserCmd *pNullCmd,const DWORD nCmdLen)
{
	...
	case CHECKNAME_SELECT_USERCMD_PARA:
		{
			...
			Xlogger->info("CHECKNAME_SELECT_USERCMD_PARA");
	...
	case CREATE_SELECT_USERCMD_PARA:
		{
			...
			Xlogger->info("CREATE_SELECT_USERCMD_PARA %u,%s,%u",pUser->accid,rev->strUserName,rev->charType);
			if (!pUser->charInfoFull())
			{
				if (strlen(rev->strUserName) > 16)
				{
					Xlogger->warn("名字长度太长(%d)",strlen(rev->strUserName));
					return true;
				}
				//角色数量没有满,还可以再建立角色
				...
				//if (rev->country >11)
				//Xlogger->debug("国家信息数量%u",GatewayService::getInstance().country_info.getCountrySize());
				if ( pUser->CountryID !=GatewayService::getInstance().country_info.getCountryID(pUser->CountryID))
				{
					Xlogger->warn("国家数据不合法");
					return true;
				}
	...
	case LOGIN_SELECT_USERCMD_PARA: 
		{
			Xlogger->info("LOGIN_SELECT_USERCMD_PARA");
			...
			if (!pUser->isWaitUnregState())
			{
				...
				Xlogger->info("收到%ld的第%d个登录用户指令",pUser->accid,rev->charNo);
				if (!userinfo)
				{
					Xlogger->warn("收到不存在的角色,可能是恶意攻击!");
					return true;
				}
				if (userinfo->bitmask & CHARBASE_FORBID)
				{
					Xlogger->warn("收到已经被封号的角色,可能是恶意攻击!");
					return true;
				}

				if (!GatewayService::getInstance().country_info.isEnableLogin(userinfo->country))
				{
					Xlogger->warn("国家不允许登陆 %u,%u",pUser->accid,userinfo->country);
					Cmd::stDisableLoginCountryCmd send;
				...
				if (pUser->checkPassport(rev->jpegPassport))
				{
					...
					{
						//Xlogger->debug("%u,%u,%u",pUser->systemstate,pUser->tempid,userinfo->id);
						...
						{
							Xlogger->info("注册 %u,%s",pUser->accid,userinfo->name);
							//注册角色
						...
						{
							Xlogger->warn("选择%s时添加用户失败",userinfo->name);
						}
					}
					else
						Xlogger->warn("用户%s(%ld)重复选择角色",userinfo->name,userinfo->id);
				}
				else
					Xlogger->warn("验证图形验证码失败 %u",pUser->accid);
			}
			else
			{
				Xlogger->warn("用户%s(%ld)正在退出状态",pUser->name,pUser->id);
			}
			return true;
		}
		break;
	case DELETE_SELECT_USERCMD_PARA:
		{
			...
			Xlogger->info("DELETE_SELECT_USERCMD_PARA");
			SelectUserInfo *userinfo=pUser->getSelectUserInfo(rev->charNo);
			if (!userinfo)
			{
				Xlogger->warn("收到不存在的角色,可能是恶意攻击!");
			...
			if (pUser->tempid == 0 && userinfo && userinfo->id!=0)
			{
				Xlogger->info("用户(%s,%d)删除角色",userinfo->name,userinfo->id);
				...
				Xlogger->warn("用户%s(%ld)删除角色失败",userinfo->name,userinfo->id);
			return true;
		}
		break;
	case REQUEST_COUNTRY_SELECT_USERCMD_PARA:
		{
			Xlogger->info("REQUEST_COUNTRY_SELECT_USERCMD_PARA");

			if (Seal::global["countryorder"] == "2")
			{
				Cmd::Record::t_request_Country_GateRecord reqRecord; 
				recordClient->sendCmd(&reqRecord,sizeof(reqRecord));
			}
			else  if (Seal::global["countryorder"] == "1")
			{
	...
	Xlogger->error("GatewayTask::msgParse_Select(%u,%u,%u)",pNullCmd->byCmd,pNullCmd->byParam,nCmdLen);
	return false;
}
...
bool GatewayTask::msgParse_Time(const Cmd::stNullUserCmd *pNullCmd,const DWORD nCmdLen)
{
	...
	case USERGAMETIME_TIMER_USERCMD_PARA:
		{
			...
			{   
				if (this->pUser)
				{
					Xlogger->info("客户端游戏时间太快,使用了加速器,需要断开连接(accid = %u,%u,%I64u)",this->pUser->accid,pNullCmd->dwTimestamp,rev->qwGameTime - (qwGameTime + (GatewayTimeTick::currentTime.sec() - GameTimeSyn.sec())));
				}
				Xlogger->debug("游戏时间:(accid=%u,%I64u,%I64u)",accid,rev->qwGameTime,qwGameTime + (GatewayTimeTick::currentTime.sec() - GameTimeSyn.sec()));
				Terminate();
			}
			if (!haveCheckTime)
				haveCheckTime = true;
			else
				Xlogger->warn("校对时间标记错误(accid=%u)",accid);
			return true;
		}
		break;
	...
	Xlogger->error("GatewayTask::msgParse_Time(%u,%u,%u)",pNullCmd->byCmd,pNullCmd->byParam,nCmdLen);
	return false;
}
...
bool GatewayTask::checkTime(const zRTime &ct)
{
	if (accountVerified == ACCOUNTVERIFY_SUCCESS)
	{
		if (ct >= lastCheckTime)
		{
			...
			{
				Xlogger->info("客户端指定时间内没有返回校对时间指令（accid = %u,snd_queue_size = %u）,注销",accid,mSocket->snd_queue_size());
				TerminateWait();
			}
...
bool GatewayTask::checkUserCmd(const Cmd::stNullUserCmd *pCmd,const zRTime &ct)
{
	if (dwTimestampServer)
	{
		...
		{
			Xlogger->info("客户端指令时间太快,使用了加速器,需要断开连接（accid = %u,%d,%I64u）",accid,(dwTimestampServer + pCmd->dwTimestamp) - ct.msecs(),initTime.elapse(ct));
		...
		if (pCmd->dwTimestamp >= v_lastSampleTime)
		{
			...
			{
			Xlogger->debug("%d毫秒收到指令个数%d",sampleInterval,v_samplePackets);
			}
			// */
			v_samplePackets = 0;
		}
		...
		{
			Xlogger->info("客户端指令发送过快,需要断开连接（accid = %u,%u/%u毫秒）",accid,v_samplePackets,sampleInterval);
			return false;
		}
	}
	return true;
...
#ifdef _MSGPARSE_
	Xlogger->error("?? GatewayTask::msgParse(%u,%u,%u)",pNullCmd->byCmd,pNullCmd->byParam,nCmdLen);
#endif
	...
	if (pUser)
	{
		...
		case LOGON_USERCMD:
			if (pNullCmd->byParam==BACKSELECT_USERCMD_PARA)
			{
				...
				{
					Xlogger->info("%s(%ld)小退",pUser->name,pUser->id);
		...
		case SELECT_USERCMD:
			if (pUser->isSelectState())
			{
				...
				Xlogger->warn("%s(%ld)不再选择用户状态,发送选择用户系列指令",pUser->name,pUser->id);
			return true;
		case TIME_USERCMD:
		...
		case CHAT_USERCMD:
			{
				if (pUser && pUser->isPlayState())
				{
					...
					case QUESTION_OBJECT_USERCMD_PARA:
						...
						{
#ifdef _DEBUG
							Xlogger->debug("收到交易物品查询命令,转发到会话");
#endif              
							return true;
						}
						break;
				...
				{
					Xlogger->error("连接状态不正确,忽略这条指令(%d,%d)",pNullCmd->byCmd,pNullCmd->byParam);
				}
				return true;
			}
			break;
		...
		case SAFETY_USERCMD:
			{
				...
				{
					Xlogger->error("连接状态不正确,忽略这条指令(%d,%d)",
		...
		case MAGIC_USERCMD:
			{
#ifdef _DEBUG
				Xlogger->debug("攻击指令时间戳:%ld",pNullCmd->dwTimestamp);
#endif
				zRTime ctv;
		...
		case DATA_USERCMD:
			{
				...
				default:
					{
						Xlogger->error("无效的DATA_USERCMD从客户端发送过来byParam=%u",pNullCmd->byParam);
					}
					break;
				}
				return true;
...
#ifdef _DEBUG
			if (pNullCmd->byCmd == PROPERTY_USERCMD) Xlogger->error("收到人物属性操作指令！");
#endif
			//注意这里没有break
		case RELIVE_USERCMD:
			//注意这里没有break
						...
						case REQUEST_REDEEM_GOLD_PARA:
							{
								...
								if ((int)cmd->dwNum < 0)
								{
									Xlogger->error("请求兑换金币(accid=%d),数量%d,是负数,外挂所致",this->accid,cmd->dwNum);
									return true;
								}
								...
								//Xlogger->debug("请求兑换金币(accid=%d),数量%d",this->accid,cmd->dwNum);
							}
							break;
							//请求兑换月卡
						case REQUEST_REDEEM_MONTH_CARD_PARA:
							{
								...
								accountClient->sendCmd(&send,sizeof(send));  
								//Xlogger->debug("请求兑换月卡(accid=%d)",this->accid);
							}
							break;
							//请求查询点卡
						case REQUEST_POINT_PARA:
							{
								...
								accountClient->sendCmd(&send,sizeof(send));  
								//Xlogger->debug("请求查询剩余点数(accid=%d)",this->accid);
							}
							break;
						case CONSUME_CARD_PARA:
							{
								...
								send.accid = this->accid;
								accountClient->sendCmd(&send,sizeof(send));  
								Xlogger->debug("%s(%u)请求消费道具卡:%s,类型:%d",pUser->name,pUser->id,rev->cardid,rev->type);
								return true;
							}
							break;
						default:
